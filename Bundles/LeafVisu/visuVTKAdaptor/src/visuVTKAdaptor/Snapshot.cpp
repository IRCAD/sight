...
void Snapshot::doUpdate( ::fwServices::ObjectMsg::csptr msg) throw(fwTools::Failed)
{
    ...
    if ( compositeMsg && compositeMsg->hasEvent( "SNAP"))
    {
        ...
        SLM_ASSERT("sceneID missing", dataInfo->getField_NEWAPI("sceneID"));
        ::fwData::String::sptr sceneID = dataInfo->getField_NEWAPI< ::fwData::String >("sceneID");
        SLM_ASSERT("sceneID empty!", sceneID);

        SLM_ASSERT("filename missing", dataInfo->getField_NEWAPI("filename"));
        ::fwData::String::sptr filename = dataInfo->getField_NEWAPI< ::fwData::String >("filename");
