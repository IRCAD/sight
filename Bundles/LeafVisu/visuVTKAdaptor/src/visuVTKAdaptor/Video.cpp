...
void Video::doUpdate() throw(fwTools::Failed)
{
    ...
    if(!bText_init)
    {
        ...
        ::boost::filesystem::path pathShaderVP ( "Bundles/visuVTKAdaptor_" + std::string(
                                                     VISUVTKADAPTOR_VER) + "/vp.glsl" );
        ::boost::filesystem::path pathShaderFP ( "Bundles/visuVTKAdaptor_" + std::string(
                                                     VISUVTKADAPTOR_VER) + "/fp.glsl" );
