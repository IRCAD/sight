...
namespace sight::modules::filter::video
{
 ...
 *
 * @code{.xml}
     <service uid="..." type="::sight::modules::filter::video::SOpticalFlow" worker="ofWorker  >
 ...
 *  usually you don't need to change the value.
 */
class MODULE_IO_VIDEO_CLASS_API SOpticalFlow : public services::IController
{
public:
    ...
    /// Constructor
    MODULE_IO_VIDEO_CLASS_API SOpticalFlow() noexcept;

    /// Destructor
    MODULE_IO_VIDEO_CLASS_API ~SOpticalFlow() noexcept override;

    /// Connects ::arData:FrameTL::s_OBJECT_PUSHED_SIG to s_UPDATE_SLOT
    MODULE_IO_VIDEO_CLASS_API services::IService::KeyConnectionsMap getAutoConnections() const override;

protected:

    /// Does nothing.
    MODULE_IO_VIDEO_CLASS_API void configuring() override;

    /// Does nothing.
    MODULE_IO_VIDEO_CLASS_API void starting() override;

    /// Computes Optical flow.
    MODULE_IO_VIDEO_CLASS_API void updating() override;

    /// Does nothing.
    MODULE_IO_VIDEO_CLASS_API void stopping() override;
