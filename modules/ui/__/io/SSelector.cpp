...
SSelector::SSelector() :
    ...
    m_slotForwardJob(new_slot(FORWARD_JOB_SLOT, &SSelector::forwardJob, this))
{
}
        ...
        if(!extensionSelectionIsCanceled)
        {
            ...
            if(m_mode == READER_MODE)
            {
                auto reader = service::add<io::service::reader>(extensionId);
                {
                    ...
                    reader->setInOut(obj.get_shared(), io::service::s_DATA_KEY);
                }
            ...
            else
            {
                ...
                {
                    auto obj = m_data.lock();
                    writer->setInput(obj.get_shared(), io::service::s_DATA_KEY);
                }
...
void SSelector::forwardJob(core::jobs::base::sptr job)
{
