...
bool list_view::eventFilter(QObject* _watched, QEvent* _event)
{
    ...
    if(_watched == m_listWidget && _event->type() == QEvent::KeyRelease)
    {
        auto* event_key = static_cast<QKeyEvent*>(_event);
        if(event_key->key() == Qt::Key_Delete)
        {
            const QList<QListWidgetItem*>& list_item = m_listWidget->selectedItems();
            if(!list_item.empty())
            {
                this->removeItem(m_listWidget->row(list_item.first()));
                return true;
            }
        }
    }
...
//------------------------------------------------------------------------------
void list_view::removeItem(int _index)
{
    // remove item at index position
    delete m_listWidget->takeItem(_index);
...
void list_view::onItemDoubleClicked(QListWidgetItem* _item)
{
    const int index = m_listWidget->row(_item);
