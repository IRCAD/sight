...
{
    new_slot(slots::RESET_TIMELINE_SLOT, &SSynchronizer::resetTimeline, this);
    new_slot(slots::SYNCHRONIZE_SLOT, &SSynchronizer::synchronize, this);
    new_slot(slots::SET_FRAME_BINDING_SLOT, &SSynchronizer::setFrameBinding, this);
    new_slot(slots::SET_MATRIX_BINDING_SLOT, &SSynchronizer::setMatrixBinding, this);
    new_slot(slots::SET_DELAY_SLOT, &SSynchronizer::setDelay, this);
...
service::connections_t SSynchronizer::getAutoConnections() const
{
    return {
        {configKey::s_FRAMETL_INPUT, data::TimeLine::CLEARED_SIG, slots::RESET_TIMELINE_SLOT},
        {configKey::s_MATRIXTL_INPUT, data::TimeLine::CLEARED_SIG, slots::RESET_TIMELINE_SLOT},
        {configKey::s_FRAMETL_INPUT, data::TimeLine::OBJECT_PUSHED_SIG, service::slots::UPDATE},
        {configKey::s_MATRIXTL_INPUT, data::TimeLine::OBJECT_PUSHED_SIG, service::slots::UPDATE}
    };
}
...
)
{
    ...
    if(buffer)
    {
        for(const outVarParameter outputVarParam : getFrameTlOutputVarIndex(frameTLIndex))
        {
            ...
            if(auto imageSeries = std::dynamic_pointer_cast<sight::data::ImageSeries>(frame.get_shared()); imageSeries)
            {
