...
void SVoxelPicker::starting()
{
    ...
    const auto interactor = std::dynamic_pointer_cast< ::fwRenderOgre::interactor::IInteractor >(this->getSptr());
...
void SVoxelPicker::pick(MouseButton _button, Modifier _mod, int _x, int _y, bool _pressed)
{
    if(_button == MouseButton::LEFT)
    {
        ...
        const auto* const camera = sceneManager->getCamera(::fwRenderOgre::Layer::s_DEFAULT_CAMERA_NAME);
        const auto vpPos         = ::fwRenderOgre::helper::Camera::convertFromWindowToViewportSpace(*camera, _x, _y);
