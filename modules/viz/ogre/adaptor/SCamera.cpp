...
#include "modules/viz/ogre/adaptor/SCamera.hpp"
...
void SCamera::starting()
{
    ...
    m_layerConnection.connect(this->getLayer(), sight::viz::ogre::Layer::s_CAMERA_RANGE_UPDATED_SIG,
                              this->getSptr(), s_CALIBRATE_SLOT);
    m_layerConnection.connect(this->getLayer(), sight::viz::ogre::Layer::s_RESIZE_LAYER_SIG,
                              this->getSptr(), s_CALIBRATE_SLOT);
...
void SCamera::calibrateMonoCamera(const data::Camera::csptr& _cam)
{
    ...
    {
        ::Ogre::Matrix4 m = sight::viz::ogre::helper::Camera::computeProjectionMatrix(*_cam, width, height,
                                                                                      nearClip, farClip);
...
void SCamera::calibrateCameraSeries(const data::CameraSeries::csptr& _cs)
{
    ...
    if(layer->getStereoMode() == sight::viz::ogre::compositor::Core::StereoModeType::NONE && nbCams > 0)
    {
        this->calibrateMonoCamera(_cs->getCamera(0));
    }
    else
    {
        sight::viz::ogre::Layer::CameraCalibrationsType calibrations;
        ...
        for(size_t i = 0; i < nbCams; ++i)
        {
            ...
            if(camera->getIsCalibrated())
            {
                const auto intrinsicProjMx =
                    sight::viz::ogre::helper::Camera::computeProjectionMatrix(*camera, width, height,
                                                                              nearClip, farClip);
