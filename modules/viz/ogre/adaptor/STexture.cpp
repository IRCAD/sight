...
const core::com::Signals::SignalKeyType modules::viz::ogre::adaptor::STexture::s_TEXTURE_SWAPPED_SIG = "textureSwapped";
...
void STexture::updating()
{
    ...
    if(image->getAllocatedSizeInBytes() != 0)
    {
        // Loads the new image
        this->getRenderService()->makeCurrent();
        sight::viz::ogre::Utils::loadOgreTexture(image.get_shared(), m_texture, ::Ogre::TEX_TYPE_2D, m_isDynamic);
...
void STexture::stopping()
{
    ...
    ::Ogre::TextureManager::getSingleton().remove(m_textureName, sight::viz::ogre::RESOURCE_GROUP);
