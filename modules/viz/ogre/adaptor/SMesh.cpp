...
void SMesh::starting()
{
    ...
    m_meshGeometry = ::std::make_shared< sight::viz::ogre::Mesh>(this->getID());
    ...
    if(!m_useNewMaterialAdaptor)
    {
        ...
        auto mtlAdaptors = this->getRenderService()->getAdaptors< modules::viz::ogre::adaptor::SMaterial>();

        auto result =
            std::find_if(mtlAdaptors.begin(), mtlAdaptors.end(),
                         [this](const modules::viz::ogre::adaptor::SMaterial::sptr& srv)
            {
...
void modules::viz::ogre::adaptor::SMesh::setVisible(bool _visible)
{
    if(m_entity)
    {
...
void SMesh::updateMesh(const data::Mesh::sptr& _mesh)
{
    ...
    for(auto renderable : r2vbRenderables.second)
    {
        ...
        if(r2vbRenderables.first)
        {
            if(adaptor)
            {
                auto r2vbMtlAdaptor = modules::viz::ogre::adaptor::SMaterial::dynamicCast(adaptor);
                ...
                r2vbMtlAdaptor->slot(modules::viz::ogre::adaptor::SMaterial::s_UPDATE_SLOT)->run();
            }
            else
            {
...
{
    auto materialAdaptor = this->registerService< modules::viz::ogre::adaptor::SMaterial >(
...
void SMesh::updateNewMaterialAdaptor(const data::Mesh::sptr& _mesh)
{
    if(!m_materialAdaptor)
    {
        if(m_entity)
        {
            ...
            m_entity->setMaterialName(m_materialAdaptor->getMaterialName(), sight::viz::ogre::RESOURCE_GROUP);
        }
    }
    ...
    else
    {
        m_entity->setMaterialName(m_materialAdaptor->getMaterialName(), sight::viz::ogre::RESOURCE_GROUP);
        m_meshGeometry->updateMaterial(m_materialAdaptor->getMaterialFw(), false);
        m_materialAdaptor->slot(modules::viz::ogre::adaptor::SMaterial::s_UPDATE_SLOT)->run();
    }
}
...
void SMesh::modifyVertices()
{
    ...
    //m_materialAdaptor->slot(modules::viz::ogre::adaptor::SMaterial::s_UPDATE_SLOT)->asyncRun();
...
void SMesh::requestRender()
{
    ...
    sight::viz::ogre::IAdaptor::requestRender();
}
