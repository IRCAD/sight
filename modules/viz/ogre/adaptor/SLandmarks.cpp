...
#include "modules/viz/ogre/adaptor/SLandmarks.hpp"
...
void SLandmarks::starting()
{
    ...
    m_materialAdaptor = this->registerService< modules::viz::ogre::adaptor::SMaterial >(
        "::sight::modules::viz::ogre::adaptor::SMaterial");
    m_materialAdaptor->registerInOut(m_material, modules::viz::ogre::adaptor::SMaterial::s_MATERIAL_INOUT, true);
    ...
    m_materialAdaptor->setMaterialTemplateName(sight::viz::ogre::Material::DEFAULT_MATERIAL_TEMPLATE_NAME);
...
void SLandmarks::insertPoint(std::string _groupName, size_t _index)
{
...

    {
        ...
        // Create the label.
        sight::viz::ogre::Text* text = nullptr;
        if(m_enableLabels)
        {
            // Get necessary data.
            ...
            // Create the label.
            text = sight::viz::ogre::Text::New(textName, sceneMgr, overlay, m_fontSource, m_fontSize, dpi, cam);
    ...
    if(_button == LEFT)
    {
        ...
        if(found)
        {
            ...
            if(pickedPos.has_value())
            {
                ...
                const auto landmarks              = this->getLockedInOut< data::Landmarks >(s_LANDMARKS_INPUT);
                data::Landmarks::PointType& point = landmarks->getPoint(m_pickedData->m_groupName,
