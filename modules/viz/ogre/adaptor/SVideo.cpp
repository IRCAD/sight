...
#include "modules/viz/ogre/adaptor/SVideo.hpp"
...
void SVideo::starting()
{
    ...
    if(plW.lock())
    {
        ...
        m_pointListAdaptor = this->registerService< sight::viz::ogre::IAdaptor >(
            "::sight::modules::viz::ogre::adaptor::SPointList");
...
void SVideo::updating()
{
    ...
    if (!m_isTextureInit || type != m_previousType )
    {
        ...
        if(!m_texture)
        {
            auto texture = ::Ogre::TextureManager::getSingletonPtr()->createOrRetrieve(
                ...
                sight::viz::ogre::RESOURCE_GROUP, true).first;
        ...
        if(tf)
        {
            ...
            {
                defaultMat = mtlMgr.getByName(s_VIDEO_WITHTF_MATERIAL_NAME, sight::viz::ogre::RESOURCE_GROUP);
            }
            else
            {
                defaultMat = mtlMgr.getByName(s_VIDEO_WITHTF_INT_MATERIAL_NAME, sight::viz::ogre::RESOURCE_GROUP);
            }
        }
        else
        {
            defaultMat = mtlMgr.getByName(s_VIDEO_MATERIAL_NAME, sight::viz::ogre::RESOURCE_GROUP);
        }
    ...
    sight::viz::ogre::Utils::loadOgreTexture(imageSight.get_shared(), m_texture, ::Ogre::TEX_TYPE_2D, true);
    ...
    if (!m_isTextureInit || size[0] != m_previousWidth || size[1] != m_previousHeight )
    {
        ...
        m_mesh = meshManager.createPlane(videoMeshName, sight::viz::ogre::RESOURCE_GROUP,
...
void SVideo::clearEntity()
{
    ...
    {
        ::Ogre::MeshManager& meshManager = ::Ogre::MeshManager::getSingleton();
        meshManager.remove(m_mesh->getName(), sight::viz::ogre::RESOURCE_GROUP);
        m_mesh.reset();
    }
}
