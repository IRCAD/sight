 ...
 * @code{.xml}
    <service uid="..." type="::sight::modules::viz::ogre::adaptor::SFragmentsInfo">
 ...
 */
class MODULE_VIZ_OGRE_CLASS_API SFragmentsInfo final :
...
public:
    ...
    MODULE_VIZ_OGRE_API ~SFragmentsInfo() noexcept override;

    /**
     * @brief Resizes the global render target, called by the related viewport since this adaptor is a listener.
     *
     * Call @ref modules::viz::ogre::adaptor::destroyCompositor() and modules::viz::ogre::adaptor::createCompositor(int,
     * int).
     *
     * @param _viewport related layer's viewport.
     */
    MODULE_VIZ_OGRE_API void viewportDimensionsChanged(::Ogre::Viewport* _viewport) override;

    /// Calls updating(). This is called right after the layer render target has been rendered.
    MODULE_VIZ_OGRE_API void postRenderTargetUpdate(const ::Ogre::RenderTargetEvent&) override;
...
protected:
    ...
    /// Destroys adaptor, only calls modules::viz::ogre::adaptor::destroyCompositor().
