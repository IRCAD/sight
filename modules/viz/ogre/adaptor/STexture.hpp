...
{
/**
 * @brief This adaptor maps a texture on a mesh. This is done via modules::viz::ogre::adaptor::SMaterial. In the
 * configuration we
 *  don't specify the material adaptor since it is automatically created by the modules::viz::ogre::adaptor::SMesh
 * adaptor.
 ...
 * @code{.xml}
    <service type="::sight::modules::viz::ogre::adaptor::STexture">
        <in key="image" uid="..." />
 ...
 */
class MODULE_VIZ_OGRE_CLASS_API STexture final : public sight::viz::ogre::IAdaptor
{
...
public:
    ...
    MODULE_VIZ_OGRE_API STexture() noexcept;

    /// Destroys the adaptor.
    MODULE_VIZ_OGRE_API ~STexture() noexcept override;

    /// Gets the Ogre texture.
    MODULE_VIZ_OGRE_API ::Ogre::TexturePtr getTexture() const;

    /// Gets the textures name.
    MODULE_VIZ_OGRE_API std::string getTextureName() const;

    /// Sets the texture name.
    MODULE_VIZ_OGRE_API void setTextureName(std::string _texName);

    /// Gets if 1.0 is used as the alpha value.
    MODULE_VIZ_OGRE_API bool getUseAlpha() const;

    /// Return true if a valid image is stored in this texture
    MODULE_VIZ_OGRE_API bool isValid() const;
