...
#include "modules/viz/ogre/adaptor/SNegato3D.hpp"
...
static constexpr std::uint8_t s_NEGATO_WIDGET_RQ_GROUP_ID = sight::viz::ogre::compositor::Core::s_SURFACE_RQ_GROUP_ID -
                                                            1;
...
void SNegato3D::configuring()
{
    ...
    if(config.count(s_FILTERING_CONFIG))
    {
        ...
        sight::viz::ogre::Plane::FilteringEnumType filtering(sight::viz::ogre::Plane::FilteringEnumType::LINEAR);

        if(filteringValue == "none")
        {
            filtering = sight::viz::ogre::Plane::FilteringEnumType::NONE;
        }
        else if(filteringValue == "anisotropic")
        {
            filtering = sight::viz::ogre::Plane::FilteringEnumType::ANISOTROPIC;
        }
    ...
    const std::string transformId =
        config.get<std::string>(sight::viz::ogre::ITransformable::s_TRANSFORM_CONFIG, this->getID() + "_transform");
...
void SNegato3D::starting()
{
    ...
    m_gpuTF = std::make_unique< sight::viz::ogre::TransferFunction>();
    ...
    for(auto& plane : m_planes)
    {
        ...
        plane = std::make_shared< sight::viz::ogre::Plane >(this->getID(), m_negatoSceneNode,
...
void SNegato3D::newImage()
{
    ...
    int sagittalIdx = 0;
    {
        ...
        const auto [spacing, origin] = sight::viz::ogre::Utils::convertSpacingAndOrigin(image.get_shared());
...
{
    const auto notNull =
        std::bind(std::not_equal_to< sight::viz::ogre::Plane::sptr >(), std::placeholders::_1, nullptr);
...
void SNegato3D::buttonReleaseEvent(MouseButton, Modifier, int, int)
{
    if(m_pickedPlane)
    {
        m_pickedPlane->setRenderQueuerGroupAndPriority(sight::viz::ogre::compositor::Core::s_SURFACE_RQ_GROUP_ID, 0);
        m_pickedPlane.reset();
    }
    m_pickingCross->setVisible(false);
...
std::optional< ::Ogre::Vector3 > SNegato3D::getPickedSlices(int _x, int _y)
{
    ...
    sight::viz::ogre::picker::IPicker picker;
    picker.setSceneManager(sceneManager);
    picker.executeRaySceneQuery(_x, _y, m_queryFlags);

    const auto isPicked = [&picker](const sight::viz::ogre::Plane::sptr& _p)
                          {
...
void SNegato3D::updatePickingCross(const ::Ogre::Vector3& _pickedPos, const ::Ogre::Vector3& _imgOrigin)
{
    ...
    {
        case sight::viz::ogre::Plane::OrientationMode::X_AXIS:
            ...
            break;
        case sight::viz::ogre::Plane::OrientationMode::Y_AXIS:
            ...
            break;
        case sight::viz::ogre::Plane::OrientationMode::Z_AXIS:
            m_pickingCross->position(0 + _imgOrigin.x, _pickedPos.y, _pickedPos.z);
            m_pickingCross->position(w + _imgOrigin.x, _pickedPos.y, _pickedPos.z);
            m_pickingCross->position(_pickedPos.x, 0 + _imgOrigin.y, _pickedPos.z);
            m_pickingCross->position(_pickedPos.x, h + _imgOrigin.y, _pickedPos.z);
