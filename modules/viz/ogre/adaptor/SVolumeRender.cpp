...
#include "modules/viz/ogre/adaptor/SVolumeRender.hpp"

#include "modules/viz/ogre/adaptor/STransform.hpp"
...
void SVolumeRender::starting()
{
    ...
    m_gpuVolumeTF = std::make_shared< sight::viz::ogre::TransferFunction>();
    ...
    sight::viz::ogre::Layer::sptr layer = renderService->getLayer(m_layerID);

    m_volumeRenderer = new sight::viz::ogre::vr::RayTracingVolumeRenderer(this->getID(),
                                                                          layer,
                                                                          m_volumeSceneNode,
                                                                          m_3DOgreTexture,
                                                                          m_gpuVolumeTF,
...
void SVolumeRender::newImage()
{
    auto renderService = this->getRenderService();
    {
        if(m_dynamic)
        {
            ...
            m_bufferingWorker = std::unique_ptr< sight::viz::ogre::IGraphicsWorker >(newWorker);
        }
        ...
        renderService->makeCurrent();
        {
            ...
            sight::viz::ogre::Utils::convertImageForNegato(m_3DOgreTexture.get(), image.get_shared());
        }
        this->updateVolumeTF();
    }
        ...
        auto bufferingFn = [this]()
                           {
                               ...
                               sight::viz::ogre::Utils::convertImageForNegato(m_bufferingTexture.get(),
                                                                              image.get_shared());
...
void SVolumeRender::updateImage()
{
    ...
    const data::TransferFunction::sptr volumeTF = m_helperVolumeTF.getTransferFunction();
    {
        ...
        if(m_ambientOcclusion || m_colorBleeding || m_shadows)
        {
            ...
            {
                m_ambientOcclusionSAT = std::make_shared< sight::viz::ogre::vr::IllumAmbientOcclusionSAT>(
...
void SVolumeRender::toggleAmbientOcclusion(bool _ambientOcclusion)
{
    ...
    this->toggleVREffect(modules::viz::ogre::adaptor::SVolumeRender::VR_AMBIENT_OCCLUSION);
}
...
void SVolumeRender::toggleColorBleeding(bool _colorBleeding)
{
    ...
    this->toggleVREffect(modules::viz::ogre::adaptor::SVolumeRender::VR_COLOR_BLEEDING);
}
...
{
    m_shadows = _shadows;
    this->toggleVREffect(modules::viz::ogre::adaptor::SVolumeRender::VR_SHADOWS);
}
...
void SVolumeRender::setFocalDistance(int _focalDistance)
{
    if(this->getRenderService()->getLayer(m_layerID)->getStereoMode() !=
       sight::viz::ogre::compositor::Core::StereoModeType::NONE)
    {
...
void SVolumeRender::createWidget()
{
    ...
    if(clippingMatrix)
    {
        ogreClippingMx = sight::viz::ogre::Utils::convertTM3DToOgreMx(clippingMatrix.get_shared());
    }

    const sight::viz::ogre::Layer::sptr layer = this->getLayer();

    this->destroyWidget(); // Destroys the old widgets if they were created.
    m_widget = std::make_shared< sight::viz::ogre::interactor::ClippingBoxInteractor>(layer, m_layerOrderDependant,
...
void SVolumeRender::destroyWidget()
{
    if(m_widget)
    {
        sight::viz::ogre::Layer::sptr layer = this->getLayer();
...
void SVolumeRender::toggleVREffect(modules::viz::ogre::adaptor::SVolumeRender::VREffectType _vrEffect)
{
    this->getRenderService()->makeCurrent();
    ...
    if(isValid)
    {
        ...
        else if(m_ambientOcclusionSAT)
        {
            switch(_vrEffect)
            {
                case modules::viz::ogre::adaptor::SVolumeRender::VR_AMBIENT_OCCLUSION:
                case modules::viz::ogre::adaptor::SVolumeRender::VR_COLOR_BLEEDING:
                    m_ambientOcclusionSAT->setAO(m_ambientOcclusion || m_colorBleeding);
                    break;
                case modules::viz::ogre::adaptor::SVolumeRender::VR_SHADOWS:
...
void SVolumeRender::updateClippingTM3D()
{
    ...
    if(clippingMatrix)
    {
        sight::viz::ogre::Utils::copyOgreMxToTM3D(m_widget->getClippingTransform(), clippingMatrix.get_shared());
