...
#include "modules/viz/ogre/adaptor/SNegato2D.hpp"
...
void SNegato2D::starting()
{
    ...
    m_gpuTF = std::unique_ptr< sight::viz::ogre::TransferFunction>(new sight::viz::ogre::TransferFunction());
    ...
    m_plane = std::make_unique< sight::viz::ogre::Plane >(this->getID(), m_negatoSceneNode, getSceneManager(),
...
void SNegato2D::changeSliceType(int _from, int _to)
{
    ...
    if(planeOrientation != newOrientation)
    {
        ...
        const auto origin = sight::viz::ogre::Utils::convertSpacingAndOrigin(image.get_shared()).second;
        m_plane->setOriginPosition(origin);
