 ...
 * @code{.xml}
    <service type="::sight::modules::viz::ogre::adaptor::SNegato3D">
        <inout key="image" uid="..." />
 ...
 */
class MODULE_VIZ_OGRE_CLASS_API SNegato3D final :
...
public:
    ...
    fwCoreServiceMacro(SNegato3D, ::sight::viz::ogre::IAdaptor)

    /// Creates slots.
    MODULE_VIZ_OGRE_API SNegato3D() noexcept;

    /// Destroys the adaptor.
    MODULE_VIZ_OGRE_API ~SNegato3D() noexcept override;
...
protected:
     ...
     */
    MODULE_VIZ_OGRE_API void swapping(const KeyType& key) override;
...
private:
    ...
    std::unique_ptr< sight::viz::ogre::TransferFunction> m_gpuTF { nullptr };

    /// Stores the planes on which we will apply our texture.
    std::array< sight::viz::ogre::Plane::sptr, 3> m_planes;

    /// Contains the plane that the user is currently interacting with.
    sight::viz::ogre::Plane::sptr m_pickedPlane { nullptr };
