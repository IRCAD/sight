...
#include "modules/viz/ogre/adaptor/SVoxelPicker.hpp"
...
void SVoxelPicker::starting()
{
    ...
    const auto interactor = std::dynamic_pointer_cast< sight::viz::ogre::interactor::IInteractor >(this->getSptr());
...
void SVoxelPicker::pick(MouseButton _button, Modifier _mod, int _x, int _y, bool _pressed)
{
    if(_button == MouseButton::LEFT)
    {
        ...
        const auto* const camera = sceneManager->getCamera(sight::viz::ogre::Layer::s_DEFAULT_CAMERA_NAME);
        const auto vpPos         = sight::viz::ogre::helper::Camera::convertFromWindowToViewportSpace(*camera, _x, _y);
