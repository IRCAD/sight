...
void negato::updateBufferFromImage(QImage* _img)
{
    ...
    if(m_orientation == orientation_t::SAGITTAL) // sagittal
    {
        ...
        for(std::size_t z = 0 ; z < size[2] ; ++z)
        {
            ...
            {
                const QRgb val =
                    sight::module::viz::scene2d::adaptor::negato::getQImageVal(imgBuff[zxOffset + y * size[0]], *tf);
    ...
    else if(m_orientation == orientation_t::FRONTAL) // frontal
    {
        ...
        for(std::size_t z = 0 ; z < size[2] ; ++z)
        {
            ...
            {
                const QRgb val =
                    sight::module::viz::scene2d::adaptor::negato::getQImageVal(imgBuff[zyOffset + x], *tf);
...
QRgb negato::getQImageVal(const std::int16_t value, const data::transfer_function& tf)
{
    const data::transfer_function::color_t color = tf.sample(value);
