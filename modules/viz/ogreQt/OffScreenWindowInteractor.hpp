...
class MODULE_VIZ_OGREQT_CLASS_API OffScreenWindowInteractor final :
    public sight::viz::ogre::IWindowInteractor
{

public:

    fwCoreClassMacro(OffScreenWindowInteractor, sight::viz::ogre::IWindowInteractor)
     ...
     */
    MODULE_VIZ_OGREQT_API OffScreenWindowInteractor(
        sight::viz::ogre::IWindowInteractor::OffscreenMgrKey,
        unsigned int _width,
        unsigned int _height);

    /// Destroys the render window.
    MODULE_VIZ_OGREQT_API virtual ~OffScreenWindowInteractor() final;

    /// Renders immediately the frame in the render window.
    MODULE_VIZ_OGREQT_API virtual void renderNow() override;

    /// Renders immediately the frame in the render window.
    MODULE_VIZ_OGREQT_API virtual void requestRender() override;

    /// Creates the render window in the global OpenGL context, @see modules::viz::ogreQt::OpenGLContext.
    MODULE_VIZ_OGREQT_API virtual void createContainer(ui::base::container::fwContainer::sptr, bool, bool) override;

    /// Does nothing.
    MODULE_VIZ_OGREQT_API virtual void connectToContainer() override;

    /// Deletes the render window and the render texture.
    MODULE_VIZ_OGREQT_API virtual void disconnectInteractor() override;

    /// Returns the unique identifier of the widget.
    MODULE_VIZ_OGREQT_API virtual int getWidgetId() const override;

    /// Returns current frame number of the render window.
    MODULE_VIZ_OGREQT_API virtual int getFrameId() const override;

    /// Makes the OpenGL context as current one on this thread against this window.
    MODULE_VIZ_OGREQT_API virtual void makeCurrent() override;

    /// Gets the Ogre render target.
    MODULE_VIZ_OGREQT_API virtual ::Ogre::RenderTarget* getRenderTarget() override;

    /// Gets the Ogre render texture attached to the render target.
    MODULE_VIZ_OGREQT_API virtual ::Ogre::TexturePtr getRenderTexture() override;

    /// Creates a worker able to handle resources from the window's OpenGL context.
    MODULE_VIZ_OGREQT_API sight::viz::ogre::IGraphicsWorker* createGraphicsWorker() override;

    /// Returns a DPI of 220 to permit offscreen font rendering.
    /// @warning Doesn't make much sense when not rendering on a screen.
    MODULE_VIZ_OGREQT_API virtual float getLogicalDotsPerInch() const override
    {
