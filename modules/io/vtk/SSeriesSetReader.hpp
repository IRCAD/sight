 ...
 *
 * @code{.xml}
   <service type="sight::module::io::vtk::SSeriesSetReader">
      <inout key="data" uid="..." />
      <file>...</file>
      <file>...</file>
      <file>...</file>
 ...
 * - \b data [sight::data::SeriesSet]: SeriesSet to load.
...
public:
     ...
     */
    MODULE_IO_VTK_API SSeriesSetReader() noexcept;

    ~SSeriesSetReader() noexcept
    {
    }
...
private:
    ...
    /**
     * @brief Load VTK mesh or image and push it into SeriesSet.
     * @param[in] vtkFiles vector of std::filesystem::path.
     * @param[out] series_set std::shared_ptr< data::SeriesSet >.
     * @return bool.
     *
    ...
    void loadSeriesSet(
        ...
        const SPTR(data::SeriesSet)& series_set
    );
