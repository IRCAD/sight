 ...
 * @code{.xml}
        <service type="sight::module::geometry::SPointToLandmarkVector">
            <in key="landmark" uid="..." />
 ...
 */
class MODULE_GEOMETRY_CLASS_API SPointToLandmarkVector final : public service::base
{
public:

    /// Generates default methods like New, dynamicCast, ...
    SIGHT_DECLARE_SERVICE(SPointToLandmarkVector, service::base);

    /// Initializes signals.
    MODULE_GEOMETRY_API SPointToLandmarkVector() noexcept;

    /// Destroys the service.
    MODULE_GEOMETRY_API ~SPointToLandmarkVector() noexcept override;
