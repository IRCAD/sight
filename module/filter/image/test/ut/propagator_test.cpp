...
TEST_SUITE("sight::module::filter::image::propagator")
{
    TEST_CASE("propagate")
    {
        ...
        for(std::size_t x = 3 ; x < 6 ; ++x)
        {
            for(std::size_t y = 0 ; y < 10 ; ++y)
            {
                for(std::size_t z = 40 ; z < 70 ; ++z)
                {
                    ...
                    image->set_pixel(index, reinterpret_cast<sight::data::image::buffer_t*>(&value));
                }
            ...
            for(std::size_t x = 0 ; x < 10 ; ++x)
            {
                for(std::size_t y = 0 ; y < 20 ; ++y)
                {
                    for(std::size_t z = 0 ; z < 90 ; ++z)
                    {
                        ...
                        {
                            CHECK_EQ(static_cast<std::uint8_t>(50), value_out);
                        }
                        else
                        {
                            CHECK_EQ(static_cast<std::uint8_t>(0), value_out);
        ...
        srv->update().wait();
        {
            ...
            CHECK_EQ(std::size_t(1200), samples_out->size()[0]);
            ...
            for(std::size_t i = 0 ; i < 1200 ; ++i)
            {
                ...
                CHECK((static_cast<std::int16_t>(2554) == voxel || static_cast<std::int16_t>(456) == voxel));
                count_1 = (static_cast<std::int16_t>(2554) == voxel) ? count_1 + 1 : count_1;
                count_2 = (static_cast<std::int16_t>(456) == voxel) ? count_2 + 1 : count_2;
            }

            CHECK_EQ(static_cast<std::size_t>(900), count_1);
            CHECK_EQ(static_cast<std::size_t>(300), count_2);
        ...
        // Clear the output image
        {
            ...
            CHECK_EQ(false, mask_filled_out->value());
        }
...

        {
                ...
                for(std::size_t y = 0 ; y < 20 ; ++y)
                {
                    for(std::size_t z = 0 ; z < 90 ; ++z)
                    {
                        ...
                        else
                        {
                            std::stringstream ss;
                            ss << x << " " << y << " " << z;
                            CHECK_MESSAGE(value_out == static_cast<std::uint8_t>(0), ss.str());
                        }
                    }
                }
            }
            ...
            const auto voxels_lock = samples_out->dump_lock();
            CHECK_EQ(std::size_t(302), samples_out->size()[0]);
            ...
            CHECK_EQ(static_cast<std::size_t>(300), count);
            CHECK_EQ(true, mask_filled_out->value());
        }
