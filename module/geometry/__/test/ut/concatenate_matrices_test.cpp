...
static void check_matrix(const matrix_t& _expected, const sight::data::matrix4& _actual)
{
    for(size_t i = 0 ; i < _expected.size() ; ++i)
    {
        CHECK(_expected[i] == doctest::Approx(_actual[i]).epsilon(DELTA));
    }
}

TEST_SUITE("sight::module::geometry::concatenate_matrices")
{
    TEST_CASE("identity")
    {
    ...
    TEST_CASE("multiply_inverse")
    {
        ...
        *in4 = {-0.866, -0.5, 0., 14.94,
        ...
        for(int i = 0 ; i < 500 ; ++i)
        {
            ...
            const matrix_t expected =
            {
                -0.433038, 0.250022, -0.866038, -20.9134,
