...
DicomImage::DicomImage(): m_zSpacing(0)
{
...
void DicomImage::read() throw(::gdcmIO::exception::Failed)
{
    SLM_TRACE_FUNC();
    ...
    if ( imageFiles.size() > 1 )
    {
        ...
        else
        {
            ...
            if ( !gSorter.StableSort( imageFiles ) )
            {
                ...
                {
                    // Give up
                    throw ::gdcmIO::exception::Failed("Files sort failed");
                }
            }
    ...
    if (gImageReader.Read())
    {
        try
        {
            ...
            this->readImage(this->getConcreteObject());
        }
    ...
    {
        throw ::gdcmIO::exception::Failed("DICOM image reading failed");
    }
}

//------------------------------------------------------------------------------

void DicomImage::readImage( ::fwData::Image::sptr img ) throw(::gdcmIO::exception::Failed)
{
    SLM_TRACE_FUNC();
    ...
    if ( dim == 3 && !isMultiFrame )   // 3D image from several files
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    // Let's give up
                    throw ::gdcmIO::exception::Failed("Failed to get depth spacing");
                }
            }
        }
    }
...
void DicomImage::setImageBuffer( ::gdcm::Image & gImg,
                                 ...
                                 ::boost::shared_ptr< ::gdcmIO::container::DicomInstance > dicomInstance ) throw(::gdcmIO::exception::Failed)
{
    ...
    {// Read 2D or 3D image from one file
        ...
        if ( !gImg.GetBuffer( (char*) gdcmBuffer ) )
            throw ::gdcmIO::exception::Failed("Failed to get image buffer");
    }
...
void DicomImage::rescaleImageBuffer(::gdcm::Image & gImg,
                                    ::fwData::Image::sptr img,
    ...
    if ( isMultiFrame )
    {
        ...
        if(scanOk)
        {
            ...
            for(int  i= 0; it != keys.end(); ++it, i++)
            {
                ...
                {
                    throw ::gdcmIO::exception::Failed("Image could not be rescale.");
                }
...
void * DicomImage::read2DImages() throw(::gdcmIO::exception::Failed)
{
    ::boost::shared_ptr< ::gdcm::ImageReader >  gImageReader    = ::boost::static_pointer_cast< ::gdcm::ImageReader >( this->getReader() );
    ...
    if ( !gImg.GetBuffer( gdcmGlobalBuffer ) )
        throw ::gdcmIO::exception::Failed("Failed to get first frame buffer");
