...
void DicomLandmark::setFromData(::fwData::Image::csptr a_image) throw (::gdcmIO::exception::Failed)
{
    SLM_TRACE_FUNC();
    ...
    if (a_image->getNumberOfDimensions() > 2)
    {
        ...
        zOrigin = a_image->getOrigin()[2];
    }

    for (iter = vLandmarks.begin() ; iter != vLandmarks.end() ; ++iter)
    {
        // Compute the max legal coord z
        refFrame = floor(((*iter)->getCoord()[2]-zOrigin) * zSpacingInverse + 0.5) + 1;    // +1 because frame number start at 1 and point at 0
...
void DicomLandmark::addSCoord(const ::gdcmIO::container::DicomSCoord & a_point)
{
