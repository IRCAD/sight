...
{   //TODO : Complete the test
    if(get(0,3)!=0.0 || get(1,3)!=0.0 || get(2,3)!=0 || get(3,3)!=1.0)
    {
        return false;
    }
...
double arlCore::vnl_rigid_matrix::distance2( const vnl_rigid_matrix& T, ARLCORE_VRM_DISTANCE_METHOD method,
                                             std::vector<double>& errors2 ) const
{
    errors2.clear();
    ...
    if(errors2.size()>0)
    {
        return distance2/(double)errors2.size();
    }
    else
    {
        return -1;
...
void arlCore::vnl_rigid_matrix::compare( const vnl_rigid_matrix& T, double& errorTranslation,
                                         double& errorRotation ) const
{
...
void arlCore::vnl_rigid_matrix::compareInverse( const vnl_rigid_matrix& T, double& errorTranslation,
                                                double& errorRotation ) const
{
...
double arlCore::vnl_rigid_matrix::getAngleOXY( void )
{
    ...
    const double Angle =
        fmod(vgl_homg_operators_3d<double>::plane_plane_angle(Plane1, Plane2), vnl_math::pi/2.0)*arlCore::RadToDeg;
    return Angle;
}
...
bool arlCore::vnl_rigid_matrix::chgPlaneOXZ(  arlCore::Point::csptr O,  arlCore::Point::csptr X,
                                              arlCore::Point::csptr Z )
{
...
bool arlCore::vnl_rigid_matrix::trf( Point::csptr pt1, Point::sptr pt2 ) const
{
    ...
    vnl_matrix_fixed<double,3,3> C = this->getRotation() *  pt1->getCovMatrix() * vnl_transpose(
        this->getRotation() );
...
unsigned int arlCore::vnl_rigid_matrix::trf( const std::vector< Point::csptr >& l1,
                                             std::vector< Point::csptr >& l2 ) const
{
    unsigned int i;
    for( i = 0; i<l1.size(); ++i )
    {
...
bool arlCore::vnl_rigid_matrix::trf( const vgl_line_3d_2_points<double>& line1,
                                     vgl_line_3d_2_points<double>& line2 ) const
{
    ...
    if(!trf(Point::New(P1Line.x(), P1Line.y(), P1Line.z()), p1))
    {
        return false;
    }
    if(!trf(Point::New(P2Line.x(), P2Line.y(), P2Line.z()), p2))
    {
        return false;
    }
...
bool arlCore::vnl_rigid_matrix::trf( const vgl_line_segment_3d<double>& line1,
                                     vgl_line_segment_3d<double>& line2 ) const
{
    const vgl_point_3d<double> P1Line = line1.point1();
    const vgl_point_3d<double> P2Line = line1.point2();
    Point::sptr p1                    = Point::New(3);
...
bool arlCore::vnl_rigid_matrix::registerICP(PointList::csptr pointsListA,PointList::csptr pointsListB, double& firstRMS,
                                            double& lastRMS, unsigned int& iterations, bool justVisible, double RMSMax,
                                            unsigned int iterationsMax )
...
//  http://ieeexplore.ieee.org/iel1/34/3469/00121791.pdf?tp=&arnumber=121791&isnumber=3469
    ...
    if(justVisible)
    {
        modelSize = pointsListA->visibleSize();
    }
    else
    {
        modelSize = pointsListA->size();
    }
    if(modelSize<1)
    {
        return false;
    ...
    if(justVisible)
    {
        acquisitionSize = pointsListB->visibleSize();
    }
    else
    {
        acquisitionSize = pointsListB->size();
    }
    ...
    for( i = 0; i<acquisitionSize; ++i )
    {
        for( j = 0; j<3; ++j )
        {
            Pi[i][j] = Pk[i][j] = Rot[j][0]*dataPoints[i][0]+Rot[j][1]*dataPoints[i][1]+Rot[j][2]*dataPoints[i][2]+
                                  Tra[j];
            gravityP[j] += Pi[i][j];
        }
    }
    ...
    if(Verbose)
    {
        ...
        std::cout<<"firstRMS="<<firstRMS<<" lastRMS="<<lastRMS<<" Iterations="<<iterations<<" PtsModel="<<modelSize<<
            " PtsAcquisition="<<acquisitionSize<<"\n";
...
bool arlCore::vnl_rigid_matrix::oldRegisterICP(PointList::csptr pointsListA,PointList::csptr pointsListB, double& RMS,
                                               unsigned int& iterations, double RMSMax, unsigned int iterationsMax )
{
    return false;
#ifdef ANN
//  To be continued : Automatic initialization or using the current matrix for initialization
...
bool arlCore::vnl_rigid_matrix::register3D3DUncertainty(PointList::csptr a,PointList::csptr b,
                                                        arlCore::ARLCORE_REGISTER3D3D methode,
                                                        std::vector<double> optimiserParameters,
                                                        std::vector<double>& log /*, unsigned int numberOfPoints*/ )
{
...
//  }
    ...
    {
        arlCore::register3D3DUncertainty_LS_cost_function reg3D3D_LM(a, b, 6, /*numberOfPoints*/ 0,
...
bool arlCore::vnl_rigid_matrix::register3D3DwithoutMatching( PointList::csptr a, PointList::csptr b, bool computeRMS,
                                                             double gaussianError, double decimage )
...
vnl_matrix_fixed<double,4,4> arlCore::rotVecFromPoint(double a, double b, double c, double u, double v, double w,
                                                      double angle)
...
unsigned int arlCore::filter( const std::vector<const arlCore::vnl_rigid_matrix*>& list, ARLCORE_TRF_FILTER_TYPE type,
                              vnl_rigid_matrix& average )
{
    ...
    for( i = 0; i<Size; ++i )
    {
        if(type==ARLCORE_TRF_FILTER_CONSTANT || RangeTime==0.0 )
        {
            coeff = 1.0;
        }
        else
