...
unsigned int arlCore::fillGrid( arlCore::PointList::sptr pl, unsigned int nbPointsX, double squareSizeX,
                                unsigned int nbPointsY, double squareSizeY, unsigned int nbPointsZ, double squareSizeZ )
{
...
bool arlCore::plot( const std::vector< const arlCore::PointList* >& pl, const std::string& options )
{
    ...
    for( i = 0; i<pl.size(); ++i )
    {
        ...
        if(pl[i])
        {
            pl[i]->save(ss.str(), ARLCORE_POINT_SAVE_GNUPLOT, JustVisible, Overwrite);
        }
        if(i==0)
        {
            file<<" ";
...
bool arlCore::computePolynomial( unsigned int degree, const vnl_vector<double>& parameters,
                                 const vnl_vector<double>& data, vnl_vector<double>& result )
{
    if(degree<1)
    {
        return false;
    ...
    if(NbParameters!=parameters.size())
    {
        return false;
    }
    if(data.size()!=NbEquations)
    {
        return false;
    }
...
unsigned int arlCore::computeNeedleROI( Point::csptr Tip, Point::csptr Top, unsigned int width,
                                        std::vector<arlCore::Point::sptr >& corners, unsigned int shape )
...
//  vnl_powell Optimizer( &OVR );
    ...
    arlCore::OptimiseVideoRobotX_LS OVR( M06, Z, model3D, cams, points2DList, number_of_residuals,
...
//  vnl_powell Optimizer( &OVR );
    ...
    arlCore::OptimiseVideoRobotZ_LS OVR( M06, X, model3D, cams, points2DList, number_of_residuals,
                                         vnl_least_squares_function::use_gradient );
    vnl_levenberg_marquardt Optimizer( OVR );
    Optimizer.set_trace(true);
//  Optimizer.set_check_derivatives(1);
...
//  vnl_powell Optimizer( &OVR );
    ...
    arlCore::OptimiseVideoRobotXZ_LS OVR( M06, model3D, cams, points2DList, number_of_residuals,
                                          vnl_least_squares_function::use_gradient );
    vnl_levenberg_marquardt Optimizer( OVR );
    Optimizer.set_trace(true);
//  Optimizer.set_check_derivatives(1);
...
double arlRandom::Random::privateRand( void )
{
    if(m_randomTable.size()>0)
    {
        ...
        ++m_index;
        if(m_index>=(long int)m_randomTable.size())
        {
            m_index = 0;
        }
        return Value;
...
bool arlString::splitFilename( const std::string& path, std::string& folder, std::string& fullFileName,
                               std::string& ext )
{
    size_t found;
    ...
    {
        ext = fullFileName.substr(found+1);
    }
    return true;
}
...
void arlString::getFullDateString( long int date, std::string& s, char separator )
{
    ...
    sprintf(name,"%04d%c%02d%c%02d%c%02d%c%02d%c%02d", t->tm_year+1900, separator, t->tm_mon+1, separator, t->tm_mday,
            separator, t->tm_hour, separator, t->tm_min, separator, t->tm_sec);
    s = name;
}
...
unsigned int arlString::extractDoubles( const std::string& s, std::vector<double>& v, char separator )
{
    ...
    if( pos>0 )
    {
        extractDoubles(s.substr(0,pos-1),v,separator);
    }
    // TODO Use convert
    double d;
    if( pos<s.length() && convert(s.substr(pos),d) )
    {
        v.push_back(d);
    }
...
bool arlString::extractIP( const std::string& s, unsigned int& IP1, unsigned int& IP2, unsigned int& IP3,
                           unsigned int& IP4 )
{
...
unsigned int arlCore::save( const std::vector< vnl_vector<double> >& q, vnl_vector<double> ratio,
                            const std::vector< std::string >& names, const std::string& fileName, bool overwrite )
{
    if(q.size()<1)
    {
        return 0;
...
unsigned int arlCore::load( std::vector< vnl_vector<double> >& v, std::vector<std::string>& names,
        ...
        for( j = 0; j<NbCams; ++j )
        {
            if(videoCams[j]!=0)
            {
                if(i==0)
                {
                    ...
                    if(points2DList[j].size()!=models3DList[j].size())
                    {
                        std::cerr<<
                            "AxxBReproj_cost_function : assert(points2DList[j].size()==models3DList[j].size());\n";
                    }
                }
                assert( (points2DList[j][i])->size()== (models3DList[j][i])->size() );
                if(  (points2DList[j][i])->size()!= (models3DList[j][i])->size() )
                {
                    std::cerr<<
                        "AxxBReproj_cost_function : assert(points2DList[j][i].size()==models3DList[j][i].size());\n";
                }
                for( k = 0; k<(models3DList[j][i])->size(); ++k )
                {
                    T.trf( (*(points2DList[j][i]))[k], pt3D);
                    if(!videoCams[j]->project3DPoint( pt3D, pt2D ))
                    {
                        std::cerr<<"AxxBReproj_cost_function : Reprojection Error\n";
                    }
                    else
                    {
...
unsigned int arlCore::statistic( const std::vector<double>& v, double& min, double& max, double& rms, double& mean,
                                 double& stddev, bool square )
{
    unsigned int i;
    ...
    for( i = 0; i<Size; ++i )
    {
        if(square)
        {
            value2 = v[i], value = sqrt(v[i]);
        }
        else
        {
            value = v[i], value2 = v[i]*v[i];
        }
        if(i==0)
        {
            min = value, max = value;
    ...
    for( i = 0; i<Size; ++i )
    {
        if(square)
        {
            stddev += (sqrt(v[i])-mean)*(sqrt(v[i])-mean);
        }
        else
        {
...
unsigned int arlCore::computeStat( const vnl_vector_fixed<double,5>& stat, double& min, double& max, double& mean,
                                   double& stdDev, double& RMS )
{   // stat = { n ; min ; max ; sum(error) ; sum(error²) }
...
bool arlCore::needleCalibration( const std::vector< arlCore::vnl_rigid_matrix >& tip,
                                 arlCore::vnl_rigid_matrix& calibration,
                                 const std::vector< arlCore::vnl_rigid_matrix >& orientation, double& RMS )
{
    ...
    if(tip.size()>3)
    {
        ...
        if(!pl.sphereCenterEstimation(center, radius, methode, optimiser_parameter, log))
        {
            return false;
        }
        if(log.size()>0)
        ...
        if(Verbose)
        {
            std::cout<<"Radius = "<<radius<<" Error = "<<RMS<<" center = "<<center->x()<<" "<<center->y()<<" "<<
                center->z()<<"\n";
        }
    }
...
bool arlCore::vgl_fit_line_3d( const std::vector< vgl_point_3d<double> >& points, vgl_line_3d_2_points<double>& line,
                               double& RMS )
{
    RMS = -1;
    if(points.size()<2)
