...
bool arlCore::Mesh::generateX( double ySize, double zSize, double yStep, double zStep, double degree,
                               double gaussianNoise, PointList::sptr summits )
{
    return generate( 0, ySize, zSize, yStep, zStep, degree, gaussianNoise, summits );
}

bool arlCore::Mesh::generateY( double xSize, double zSize, double xStep, double zStep, double degree,
                               double gaussianNoise, PointList::sptr summits )
{
    return generate( 1, xSize, zSize, xStep, zStep, degree, gaussianNoise, summits );
}

bool arlCore::Mesh::generateZ( double xSize, double ySize, double xStep, double yStep, double degree,
                               double gaussianNoise, PointList::sptr summits )
{
    return generate( 2, xSize, ySize, xStep, yStep, degree, gaussianNoise, summits );
}

bool arlCore::Mesh::generate( unsigned int axis, double width, double length, double stepW, double stepL, double degree,
                              double gaussianNoise, PointList::sptr summits )
{
    clear();
    ...
    for( i = 0; i<Width; ++i )
    {
        ...
        for( j = 0; j<Length; ++j )
        {
            ...
            for( k = 0; k<summits->size() && !end; ++k )
            {
                ...
                {
                    case 0:
                        distance2 = (Wi-(*summits)[k]->y())*(Wi-(*summits)[k]->y())+(Li-(*summits)[k]->z())*
                                    (Li-(*summits)[k]->z());
                        break;
                    case 1:
                        distance2 = (Wi-(*summits)[k]->x())*(Wi-(*summits)[k]->x())+(Li-(*summits)[k]->z())*
                                    (Li-(*summits)[k]->z());
                        break;
                    case 2:
                        distance2 = (Wi-(*summits)[k]->x())*(Wi-(*summits)[k]->x())+(Li-(*summits)[k]->y())*
                                    (Li-(*summits)[k]->y());
                        break;
...
bool arlCore::Mesh::load( const std::string& fileName, double gaussianNoise )
{
            ...
            while(config.getToken(token))
            {
                    ...
                    if(arlCore::load( vnlPoints, names, ss.str() ))
                    {
                        ...
                        for( i = 0; i<vnlPoints.size(); ++i )
                        {
                            if(arlString::valueOf(names[i], index))
                            {
                                ...
                                arlCore::Point::sptr pt = arlCore::Point::New(vnlPoints[i][0], vnlPoints[i][1],
                                                                              vnlPoints[i][2]);
                        ...
                        {
                            planeN[i] =
                                PL[i].plane( planes[i], planeRMS[i], planeMin[i], planeMax[i], scalar, outliers );
                        }
                    }
                }
                if(token=="Point")
                ...
                {   // FIXME Find another way to display this informations
                    ...
                    if(token=="PlaneAngle")
                    {
                        ...
                        {
                            double angle = fmod(vgl_homg_operators_3d<double>::plane_plane_angle(planes[p1],
                                                                                                 planes[p2]),
                                                vnl_math::pi/2.0);
                            if(angle>=vnl_math::pi/4.0)
                            {
                                angle = vnl_math::pi/2.0 - angle;
                            }
                            std::cout<<"Plane Angle "<<p1<<"-"<<p2<<" = "<<angle*arlCore::RadToDeg<<" deg";
                            if(Compare)
                            {
                                std::cout<<" (Delta="<<angle*arlCore::RadToDeg-compareValue<<")";
                            }
                            std::cout<<"\n";
                        }
                    }
                    if(token=="PointDistance")
                    {
                        ...
                        if(points.size()>p1 && points.size()>p2)
                        {
                            ...
                            if(Compare)
                            {
                                std::cout<<" (Delta="<<Distance-compareValue<<")";
                            }
                            std::cout<<"\n";
                        }
                    }
    ...
    if(filetype=="Surface")
    {
        ...
        if(Version>0.0)
        {
            ...
            while(config.getToken(token))
            {
                ...
                if(token=="XSize" || token=="Width")
                {
                    config.get(xSize);
                }
                if(token=="YSize" || token=="Length")
                {
                    config.get(ySize);
                }
                if(token=="XStep")
    ...
    if(Raw)
    {
        while(!file.eof())
        {
            ...
            if((i+1)%3==0)
            {
                addTriangle(index-2, index-1, index);
            }
            ++i;
        }
        file.close();
...
unsigned int arlCore::Mesh::addPoint( Point::csptr p, double gaussianNoise )
{
    ...
    if(gaussianNoise>0.0)
    {
        (*m_pointList)[index]->addGaussianNoise(gaussianNoise);
    }
    return index;
}
...
bool arlCore::Mesh::addTriangle( unsigned int a, unsigned int b, unsigned int c )
{
    ...
    {
        m_triangles.push_back(vnl_vector_fixed<unsigned int, 3>(a,b,c));
    }
    else
    {
