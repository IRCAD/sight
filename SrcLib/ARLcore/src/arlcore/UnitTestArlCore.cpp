...
void printHeader( const std::string& name, unsigned int nbIterations, double tolerance )
{
    ...
    {
        std::cerr<<"* Random seed = "<<arlRandom::Random::initRandom(Seed)<<"\n";
    }
    std::cerr<<"*\n";
...
bool printErrors( const std::string& name, const vnl_vector<double>& error, long int robustnessOK,
                  long int robustnessTotal, double tolerance, bool Verbose = false)
{
    ...
    if(tolerance>=0)
    {
        test = error.max_value()<=tolerance;
    }
    ...
    double p = 0;
    if(robustnessTotal!=0)
    {
        p = (double)robustnessOK*100.0/(double)robustnessTotal;
    }
    std::cerr<<"*  Robustness = "<<robustnessOK<<" / "<<robustnessTotal<<" = "<<p<<"%\n";
    return test;
}

bool printErrors( const std::string& name, const vnl_vector_fixed<double,5>& stat, long int robustnessOK,
                  long int robustnessTotal, double tolerance, bool Verbose = false)
{
...
bool printFooter( unsigned int nbIterations, bool test )
{
    ...
    if(test)
    {
        std::cerr<<"** OK OK OK OK OK OK OK OK";
    }
    else
    {
...
bool arlCore::testRegistration3D3D( long int nbIterations, double tolerance )
{
    ...
    bool test =
        printErrors("Erreur sur la translation",errorTranslation,robustnessOK,robustnessTotal,tolerance, Verbose);
...
bool arlCore::testICPRegistration( long int nbIterations, double tolerance )
{
    ...
    for( compteur = 0; compteur<nbIterations; ++compteur )
    {
        ...
        if(compteur==0)
        {
            meshModel.load("G:/Mouse/ToSort/19052008-sd-ext.raw", GaussianNoise3D);
        }
        if(compteur>0 || meshModel.getPointList()->size()<1 )
        {
            for( i = 0; i<nbSummits; ++i )
            {
                ...
                if(i<4)
                {
                    Y = arlRandom::Random::uniformDoubleRnd( 0, altitude/2 );
                }
        ...
        if(compteur==0)
        {
            fringes.save(cloudName2.str());
        }
        else
        {
            meshCloud.save(cloudName.str());
        }
        if(Verbose)
        {
            std::cout<<"Cloud ("<<cloud->size()<<") = "<<cloudName.str()<<"\n";
        }
        if(Verbose)
        {
            std::cout<<"Model ("<<model->size()<<") = "<<modelName.str()<<"\n";
        }
        T2 = T1;
...
bool arlCore::testRegistration3D3DUncertainty( long int nbIterations, double tolerance )
{
    ...
    for( compteur = 0; compteur<nbIterations; ++compteur )
    {
        ...
        if(rangeGaussianNoise3D_x < 1e-3)
        {
            rangeGaussianNoise3D_x = 0.1;
        }
        if(rangeGaussianNoise3D_y < 1e-3)
        {
            rangeGaussianNoise3D_y = 0.1;
        }
        if(rangeGaussianNoise3D_z < 1e-3)
        {
            rangeGaussianNoise3D_z = 0.1;
        }
        ...
        if(T2.register3D3DUncertainty(worldPoints, modelPoints, arlCore::ARLCORE_REGISTER3D3D_GC, opt_param,
                                      log /*, worldPoints.size()*/) )
        {
            ++robustnessOK;
            T1.compare(T2, errorTranslation[compteur], errorRotation[compteur]);
        }
        ...
        for(enum_3D2D = arlCore::ARLCORE_PR_UNKNOWN+1,i = 0; enum_3D2D <arlCore::ARLCORE_PR_NBTYPES; ++enum_3D2D,++i)
        {
            if( enum_3D2D!=ARLCORE_PR_ISPPC_CG && enum_3D2D!=ARLCORE_PR_OSPPC_CG && enum_3D2D!=ARLCORE_PR_EPPC_CG )
            {
                ...
                if(arlCore::multiViewPointRegistration3D2D( cameras.getList(), reprojection2D, modelPoints,
                                                            T2, (arlCore::ARLCORE_PROJECTIVE_REGISTRATION)enum_3D2D,
                                                            param_optimisation, log, 0))
                {
...
bool arlCore::testHomographyRegistration( long int nbIterations, double tolerance )
{
    ...
    for(compteur = 0; compteur<nbIterations; ++compteur)
    {
        ...
        for( i = 0; i<cameras.size(); ++i)
        {
            ...
            if( arlCore::planarHomographyRegistration_3D_2D(cameras[i], reprojection2D, tmp,  T2, param_optimisation,
                                                            log, true) )                                                            //VAG FIXME PERF getListCopy()
            {
                T1.compareInverse(T2, errorT, errorR);
                addValue(errorTranslation, errorT);
                addValue(errorRotation, errorR);
...
bool arlCore::testReconstructionPolyscopic( long int nbIterations, double tolerance)
{
    ...
    for( compteur = 0; compteur<nbIterations; ++compteur)
    {
        ...
        for(enum_recons = arlCore::ARLCORE_R3D_UNKNOWN +1,k = 0; enum_recons<arlCore::ARLCORE_R3D_NBTYPE;
            ++enum_recons,++k)
        {
            if(Verbose)
            {
                std::cerr << "Changement de methode\n";
            }
            if( compteur==0 )
            {
            ...
            for( i = 0; i<NbPoints; ++i )
            {
                ...
                if( arlCore::reconst3D( pointList2D, cameras, point_reconstruit,
                                        (arlCore::ARLCORE_RECONSTRUCTION3D) enum_recons, plane, log) )
                {
                    const double Error = point_reconstruit->distance( (*worldPoints)[i] );
                    addValue(errorReproj[k], Error);
                    if(Verbose)
                    {
                        std::cerr << "Point reconstruit = "<< (*point_reconstruit)[0] <<" "<< (*point_reconstruit)[1] <<
                            " "<<(*point_reconstruit)[2] <<"\n";
                    }
                    ++robustnessOK[k];
                }
...
bool arlCore::testReconstructionPolyscopicUncertainty( long int nbIterations, double tolerance)
{
    ...
    for( compteur = 0; compteur<nbIterations; ++compteur)
    {
        ...
        double GaussianNoise2D   = 0.1;
        std::cerr << "GaussianNoise2D["<<compteur<<"] =" << GaussianNoise2D<<std::endl;

        for(enum_recons = arlCore::ARLCORE_R3D_TWO_LINES_APPROX,k = 0;
            enum_recons<=arlCore::ARLCORE_R3D_REPROJECTION_OPTIMIZATION; ++enum_recons,++k)
        {
            if( compteur==0 )
            {
            ...
            if(enum_recons == arlCore::ARLCORE_R3D_TWO_LINES_APPROX ||
               enum_recons == arlCore::ARLCORE_R3D_REPROJECTION_OPTIMIZATION)
            {
                if(enum_recons == arlCore::ARLCORE_R3D_TWO_LINES_APPROX)
                {
                    for( j = 0; j<nbCameras; ++j)
                    {
                        for( i = 0; i<nbPoints; ++i )
                        {
                            ...
                            reprojection2D[j][i]->set(0,
                                                      reprojection2D_nonoise[j][i]->x() +
                                                      arlRandom::Random::gaussianRnd(GaussianNoise2D));
                            reprojection2D[j][i]->set(1,
                                                      reprojection2D_nonoise[j][i]->y() +
                                                      arlRandom::Random::gaussianRnd(GaussianNoise2D));
                ...
                if(enum_recons == arlCore::ARLCORE_R3D_REPROJECTION_OPTIMIZATION_UNCERTAINTY)
                {
                    for( j = 0; j<nbCameras; ++j)
                    {
                        for( i = 0; i<nbPoints; ++i )
                        {
                            //*reprojection2D[j][i] = *reprojection2D_nonoise[j][i];
                            //std::cerr<<"reprojection2D_nonoise ="<<reprojection2D_nonoise[j][i]->x()<<" "<<reprojection2D_nonoise[j][i]->y()<<"\n";
                            ((Point::sptr)reprojection2D[j][i])->set(0,
                                                                     reprojection2D_nonoise[j][i]->x() +
                                                                     arlRandom::Random::gaussianRnd(GaussianNoise2D_x));
                            ((Point::sptr)reprojection2D[j][i])->set(1,
                                                                     reprojection2D_nonoise[j][i]->y() +
                                                                     arlRandom::Random::gaussianRnd(GaussianNoise2D_y));
                            //std::cerr<<"reprojection2D="<<reprojection2D[j][i]->x()<<" "<<reprojection2D[j][i]->y()<<"\n";
                            arlCore::vnl_covariance_matrix& cov_mat =
                                ((Point::sptr)reprojection2D[j][i])->getCovMatrix();
...
//                      if(i==0)
                        {
                            ...
                            tt[0] = (*worldPoints)[i]->x(); tt[1] = (*worldPoints)[i]->y();
                            tt[2] = (*worldPoints)[i]->z();
...
//                          index_mu[compteur] = (soustraction.transpose() * vnl_matrix_inverse<double> (point_reconstruit.getCovMatrix()) * soustraction)(0,0);
                            index_mu[k].push_back( (soustraction.transpose() *
                                                    vnl_matrix_inverse<double> (point_reconstruit->getCovMatrix()) *
                                                    soustraction)(0,0) );
...
//                          std::cerr << "index_mu["<<compteur<<"] =" << index_mu[compteur]<<std::endl;
                        ...
                        if(Verbose)
                        {
                            std::cerr << "Point reconstruit = "<< (*point_reconstruit)[0] <<" "<<
                            (*point_reconstruit)[1] <<" "<<(*point_reconstruit)[2] <<"\n";
                        }
                        ++robustnessOK[k];
                        ++nbpt;
                    }
        ...
        enum_recons<=arlCore::ARLCORE_R3D_REPROJECTION_OPTIMIZATION; ++enum_recons,++k)
    {
           ...
           enum_recons == arlCore::ARLCORE_R3D_REPROJECTION_OPTIMIZATION)
        {
            ...
            if(enum_recons == arlCore::ARLCORE_R3D_TWO_LINES_APPROX)
            {
                arlCore::WriteTableau ( (char*)"PredictionSigma3D_ARLCORE_R3D_TWO_LINES_APPROX", index_mu[k],
                                        nbIterations);
            ...
            {
                arlCore::WriteTableau (   (char*)"PredictionSigma3D_ARLCORE_R3D_REPROJECTION_OPTIMIZATION",
                                          index_mu[k], nbIterations);
...
bool arlCore::testEpipolarMatching( long int nbIterations, double tolerance )
{
    ...
    for( compteur = 0; compteur<nbIterations; ++compteur )
    {
        ...
        worldPoints->shapeRandom(NbPointsModel, arlCore::ARLCORE_SHAPE_SPHERE,arlCore::Point::New(0.0,0.0,
                                                                                                  0.0), VolumeSize);         //arlCore::ARLCORE_SHAPE_PLAINSQUARE
        const std::vector<arlCore::Camera>& cameras = scene.getCameras().getList();
        if(cameras.size()>=2)
    ...
    printErrors("Proportion faux appariements/total",element_quotient(nonCorrects,total),robustnessOK,robustnessTotal,
                -1);
...
bool arlCore::testInitIntrinsicCalibration( long int nbIterations, double tolerance )
{
    ...
    for( compteur = 0; compteur<nbIterations; ++compteur)
    {
        ...
        if(arlCore::initIntrinsicCalibration(listeWorldPoints, reprojection2D, camera_init, extrinsic_init,
                                             optimiser_parameters, log))
        {   //Calcul des erreurs
            ...
            {   //verification du calcul des matrices extrinseques
                arlCore::vnl_rigid_vector realT(cam.getExtrinsic()*liste_extrinsic_alea[i]),
                estimateT(extrinsic_init[i]);
    ...
    bool test = printErrors("Erreurs sur la translation extrinseque",errorTranslation,robustnessOK,robustnessTotal,
                            tolerance);
    test =
        printErrors("Erreur sur la rotation extrinseque",errorRotation,robustnessOK,robustnessTotal,tolerance) && test;
    test =
        printErrors("Erreur sur les parametres intrinseques",errorIntrinsic,robustnessOK,robustnessTotal,
                    tolerance) && test;
    return printFooter(nbIterations, test);
}
...
bool arlCore::testIntrinsicCalibration( long int nbIterations, double tolerance )
{
    ...
    for( compteur = 0; compteur<nbIterations; ++compteur)
    {
        ...
        std::copy( reprojection2DNC.begin(),  reprojection2DNC.end(),  reprojection2D.begin() );
        //VAG FIXME PERF END
        if(arlCore::initIntrinsicCalibration(listeWorldPoints, reprojection2D_bruitee, camera_init, extrinsic_init,
                                             optimiser_parameters, log))
        {
            if(arlCore::refineIntrinsicCalibration(listeWorldPoints, reprojection2D, nbParameters, camera_init,
                                                   extrinsic_init, optimiser_parameters, log))
            {   // Calcul des erreurs
                double errorI, errorR = 0,errorT = 0;
                errorI = fabs(camera_init.getfx()-cam.getfx())+fabs(camera_init.getfy()-cam.getfy())
                         + fabs(camera_init.getcx()-cam.getcx())+fabs(camera_init.getcy()-cam.getcy())
                ...
                for( k = 0; k<nbPoses; ++k )
                {   //verification du calcul des matrices extrinseques
                    arlCore::vnl_rigid_vector realT(cam.getExtrinsic()*liste_extrinsic_alea[k]), estimateT(
                        extrinsic_init[k]);
                    errorR += fabs(realT[0]- estimateT[0])+fabs(realT[1]- estimateT[1])+fabs(realT[2]- estimateT[2]);
                    errorT += fabs(realT[3]- estimateT[3])+fabs(realT[4]- estimateT[4])+fabs(realT[5]- estimateT[5]);
                }
...
//        std::copy( reprojection2DNC.begin(),  reprojection2DNC.end(),  reprojection2D.begin() );
//        //VAG FIXME PERF END
        arlCore::initExtrinsicCalibration( listeWorldPoints, reprojection2D, liste_camera_identity, init_rigid_trsf,
                                           optimiser_parameters, log_init );
    ...
    bool test = printErrors("Erreurs sur les translations extrinseques",errorTranslation,robustnessOK,robustnessTotal,
                            tolerance);
    test =
        printErrors("Erreur sur les rotations extrinseques",errorRotation,robustnessOK,robustnessTotal,
                    tolerance) && test;
    return printFooter(nbIterations, test);
}
...
// pour forcer l'algorithme a s'arreter lorsque les erreurs stagnent
    ...
    for( compteur = 0; compteur<nbIterations; ++compteur)
    {
        ...
        //INITIALISATION DU CALIBRAGE
        arlCore::initExtrinsicCalibration(listeWorldPoints, reprojection2D_bruitee, liste_camera_identity,
                                          init_rigid_trsf, optimiser_parameters, log_init );
        arlCore::refineExtrinsicCalibration(listeWorldPoints, reprojection2D, liste_camera_identity, init_rigid_trsf,
                                            optimiser_parameters, log_init );
        std::vector<arlCore::vnl_rigid_matrix> solution((nbCameras-1)+nbPoses);
        for( i = 0; i<nbPoses; ++i )
        {
...
bool arlCore::testRigidTransfo( unsigned int nbIterations, double tolerance )
{
    ...
    for( i = 0; i<nbIterations; ++i )
    {
        ...
        if(error && Verbose)
        {
            std::cerr<<m1.as_matrix()<<"\n";
        }
        if(error && Verbose)
        {
            std::cerr<<m3.as_matrix()<<"\n";
        }
        if(error && Verbose)
        {
            std::cerr<<"----------------------------\n";
...
bool arlCore::testRegistrationCriteriaComparison( long int nbIterations, std::vector<double> parameters,
                                                  std::vector<double> staticStatus,
                                                  std::vector< vnl_vector<double>* > erreur_recalage_methode,
                                                  std::vector< vnl_vector<double>* > duree_recalage_methode,
                                                  std::vector< vnl_vector<double>* > robustesse_recalage_methode )
{
    ...
    const double TransfVolumeSize       = 500.0;
    const double rangeGaussianNoise3D_X = parameters[9], rangeGaussianNoise3D_Y = parameters[10],
                 rangeGaussianNoise3D_Z = parameters[11];
    const double rangeGaussianNoise2D_X = parameters[12], rangeGaussianNoise2D_Y = parameters[13];
    double GaussianNoise3D_X, GaussianNoise3D_Y, GaussianNoise3D_Z, GaussianNoise2D_X, GaussianNoise2D_Y, RSB_3D = 0.0,
           RSB_2D              = 0.0;
    const unsigned int nbTypes = arlCore::ARLCORE_PR_NBTYPES;
    ...
    for( compteur = 0; compteur<nbIterations; ++compteur)
    {
        ...
        if(Verbose)
        {
            std::cerr<<"GaussianNoise3D_X ="<<GaussianNoise3D_X<<std::endl<<"GaussianNoise3D_Y ="<<GaussianNoise3D_Y<<
                std::endl<<"GaussianNoise3D_Z ="<<GaussianNoise3D_Z<<std::endl;
        }
        ...
        if(Verbose)
        {
            std::cerr<<"GaussianNoise2D_X ="<<GaussianNoise2D_X<<std::endl<<"GaussianNoise2D_Y ="<<GaussianNoise2D_Y<<
                std::endl;
        }
        ...
        if(parameters[14] == 0)
        {
            ...
            trf_initialisation.setTranslation( trf_initialisation(0,3)+arlRandom::Random::uniformDoubleRnd(0,
                                                                                                           3),
                                               trf_initialisation(1,3)+arlRandom::Random::uniformDoubleRnd(0,
                                                                                                           3),
                                               trf_initialisation(2,
                                                                  3)+
                                               arlRandom::Random::uniformDoubleRnd(0,3));
        }
        if(parameters[14] == 1)
        {
        ...
        for( i = 0; i<modelPoints->size(); ++i )
        {
            ...
            if(Verbose)
            {
                std::cerr<<"modelPointsNoisy["<<i<<"]"<< (*modelPointsNoisy)[i]->getCoordinates()<<std::endl;
            }
            ...
            if(Verbose)
            {
                std::cerr<<"cov mat 3D["<<i<<"] ="<< tmp->getCovMatrix()<<std::endl;
            }
        ...
        for( j = 0; j<(long int)nbCameras; ++j)
        {
            ...
            for( i = 0; i<nbModelPoints; ++i )
            {
                ...
                if(Verbose)
                {
                    std::cerr<<"reprojection2D["<<j<<"]["<<i<<"]"<<reprojection2D[j][i]->getCoordinates()<<std::endl;
                }
                if(Verbose)
                {
                    std::cerr<<"reprojection2DNoisy["<<j<<"]["<<i<<"]"<<reprojection2DNoisy[j][i]->getCoordinates()<<
                        std::endl;
                }
                arlCore::vnl_covariance_matrix& cov_mat = tmp->getCovMatrix();
                cov_mat.fill(0.0);
                ...
                if(Verbose)
                {
                    std::cerr<<"cov mat 2D["<<i<<"] ="<< tmp->getCovMatrix()<<std::endl;
                }
        ...
        for( i = 0; i<regPoints->size(); ++i )
        {
            ...
            reconst3D( tmp_2D_point_list, cameras, (*reconstructedPoints_Lines)[i],
                       arlCore::ARLCORE_R3D_MULTI_LINES_APPROX, plane_reconstruction, log_reconstruction);
            (*duree_recalage_methode[0])[compteur] = registrationDuration.getLap();
            if(Verbose)
            {
                std::cerr<<"(*reconstructedPoints_Lines)["<<i<<"]"<<(*reconstructedPoints_Lines)[i]->getCoordinates()<<
                    std::endl;
            }
            registrationDuration.startLap();
            reconst3D( tmp_2D_point_list, cameras, (*reconstructedPoints_Opt)[i],
                       arlCore::ARLCORE_R3D_REPROJECTION_OPTIMIZATION_UNCERTAINTY, plane_reconstruction,
                       log_reconstruction, -1, true);
            ...
            if(Verbose)
            {
                std::cerr<<"reconstructedPoints_Opt["<<i<<"]"<< (*reconstructedPoints_Opt)[i]->getCoordinates()<<
                    std::endl;
            }
        }
        ...
        if(Verbose)
        {
            std::cerr<<"T_list[0] ="<<T_list[0]<<std::endl;
        }
        T_list.push_back(trf_initialisation);
        registrationDuration.startLap();
        ...
        if(Verbose)
        {
            std::cerr<<"Recalage 3D3D : methode directe"<<std::endl;
        }

        for(enum_3D3D = arlCore::ARLCORE_REGISTER3D3D_UNKNOWN+1,i = 0; enum_3D3D <arlCore::ARLCORE_REGISTER3D3D_GC;
            ++enum_3D3D,++i)
        {
            ...
            T_list.push_back(trf_initialisation);
            registrationDuration.startLap();
            T_list[2+i].register3D3DUncertainty(modelPointsNoisy, reconstructedPoints_Opt,
                                                (arlCore::ARLCORE_REGISTER3D3D)enum_3D3D, optimise_param,
                                                log_3D3D /*, modelPoints->size()*/ );
        ...
        if(Verbose)
        {
            std::cerr<<"T_list.size() ="<<(unsigned int)T_list.size()<<std::endl;
        }
        ...
        //for(enum_3D2D=arlCore::ARLCORE_PR_UNKNOWN+1,i=0; enum_3D2D <arlCore::ARLCORE_PR_EPPC; ++enum_3D2D,++i)
        {
            ...
            arlCore::multiViewPointRegistration3D2D( cameras, reprojection2DNoisy,  modelPointsNoisy,
                                                     T_list[offset+i],
                                                     (arlCore::ARLCORE_PROJECTIVE_REGISTRATION)enum_3D2D,
                                                     param_optimisation, log, 1);
        ...
        if(Verbose)
        {
            std::cerr<< "CALCUL DES ERREURS "<<std::endl;
        }
    ...
    for(enum_3D3D = arlCore::ARLCORE_REGISTER3D3D_UNKNOWN+1,i = 0; enum_3D3D <arlCore::ARLCORE_REGISTER3D3D_GC; ++i)
    {
        ...
        for( j = 0; j<nbIterations; ++j )
        {
            if(Verbose)
            {
                std::cerr<< "Erreur de recalage methode ["<<i<<"]="<<(*erreur_recalage_methode[i])[j] <<std::endl;
            }
        ...
        std::cerr<<std::setprecision(10)<< "Duree MOYENNE de recalage methode ["<<i<<"]="<<
            duree_recalage_methode[i]->mean() <<std::endl<<std::endl;
    ...
    for(enum_3D2D = arlCore::ARLCORE_PR_UNKNOWN+1,i = 0; enum_3D2D <arlCore::ARLCORE_PR_ISPPC_CG; ++enum_3D2D,++i)
    {
        ...
        for( j = 0; j<nbIterations; ++j)
        {
            if(Verbose)
            {
                std::cerr<< "Erreur de recalage methode ["<<i<<"]="<<(*erreur_recalage_methode[i+offset2])[j] <<
                    std::endl;
            }
            sub[j] = (*erreur_recalage_methode[i+offset2])[j] - erreur_recalage_methode[i+offset2]->mean();
        }
        std::cerr<< "Erreur MOYENNE de recalage methode ["<<i<<"]="<<erreur_recalage_methode[i+offset2]->mean() <<
            std::endl;
        std::cerr<< "Erreur MAX de recalage methode ["<<i<<"]="<<erreur_recalage_methode[i+offset2]->max_value() <<
            std::endl;
        std::cerr<< "Ecart-type de recalage methode ["<<i<<"]="<<sub.rms() <<std::endl;
        std::cerr<<std::setprecision(10)<< "Duree MOYENNE de recalage methode ["<<i<<"]="<<
            duree_recalage_methode[i+offset2]->mean() <<std::endl<<std::endl;
        std::cerr<< "Robustesse methode ["<<i<<"]="<<robustesse_recalage_methode[i+offset2]->sum() <<std::endl<<
            std::endl;
    }
    bool test = true;
...
bool arlCore::testSphereCenterEstimation( long int nbIterations, double tolerance )
{
    ...
    for(enum_method = arlCore::ARLCORE_SCE_UNKNOWN+1,i = 0; enum_method <arlCore::ARLCORE_SCE_NBMETHOD;
        ++enum_method, ++i)
    {
        for( compteur = 0; compteur<nbIterations; ++compteur )
        {
            ...
            unsigned int nbPoints =
                (unsigned int)floor( arlRandom::Random::uniformDoubleRnd(4, 50) );
            ...
            if(Verbose)
            {
                std::cerr<< "nbPoints = "<<nbPoints <<std::endl;
            }
            if(Verbose)
            {
                std::cerr<< "diameter/2 = "<<diameter/2 <<std::endl;
            }
            ...
            if( surface_points_rotated->sphereCenterEstimation( center_estimation, radius_estimation, type,
                                                                optimiser_parameter, log) )
            {
                ++robustnessOK[i];
...
bool arlCore::testPolynomialFieldDistortion( long int nbIterations, double tolerance )
{
    ...
    for( interpolationDegree = interpolationDegreeMin; interpolationDegree<=interpolationDegreeMax;
         ++interpolationDegree, ++n )
    {
        ...
        for( compteur = 0; compteur<nbIterations; ++compteur )
        {
            ...
            for( i = 2; i<CubeRadius/ResGrid[0]; ++i )
            {
                for( j = 2; j<CubeRadius/ResGrid[1]; ++j )
                {
                    ...
                    {
                        real->push_back(arlCore::Point::New( ResGrid[0]*(double)i, ResGrid[1]*(double)j,
                                                             ResGrid[2]*(double)k));
                    }
                }
            }
