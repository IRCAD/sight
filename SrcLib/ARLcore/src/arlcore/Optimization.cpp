...
double arlCore::CostFunction::observe( const vnl_vector<double>& x )
{
    ...
    if(m_observer)
    {
        ...
        {
            m_errorList.push_back((double)(now-m_time), m_error);
        }
    }
    ++m_iterations;
    return m_error;
...
bool arlCore::CostFunction::verify( const vnl_vector<double>& x, double endError )
{
    ...
    if(!b1)
    {
        std::cout<<"Distorsion error/solution : "<<endError<<"*"<<getCriterion(x)<<"\n";
    }
    bool b2 = (endError==m_errorMin);
    if(!b2)
    {
        std::cout<<"Distorsion error min : "<<endError<<"*"<<m_errorMin<<"\n";
    }
    bool b3 = (x==m_solution);
    if(!b3)
    {
        std::cout<<"Distorsion solution : "<<x<<"*"<<m_solution<<"\n";
    }
    return b1 && b2 && b3;
}
...
// OptimiseVideoRobot1
arlCore::OptimiseVideoRobot1::OptimiseVideoRobot1( const std::vector< arlCore::vnl_rigid_matrix >& M06,
                                                   PointList::csptr Tip ) :
    vnl_cost_function(6),
    m_tip(Tip),
    m_M06(M06),
...
bool arlCore::OptimiseVideoRobot::criterion( const arlCore::vnl_rigid_matrix& X, const arlCore::vnl_rigid_matrix& invZ,
                                             vnl_vector<double>& fx )
{
    fx.fill(0.0);
...
arlCore::OptimiseReprojection::OptimiseReprojection(const vector<const arlCore::Camera*>& cameras,
...
arlCore::Dornaika_cost_function::Dornaika_cost_function(const std::vector< vnl_rigid_matrix >& A,
...
arlCore::Dornaika_LS_cost_function::Dornaika_LS_cost_function(const std::vector< vnl_rigid_matrix >& A,
                                                              const std::vector< vnl_rigid_matrix >& B,
...
arlCore::Polynomial_cost_function::Polynomial_cost_function( PointList::csptr real, PointList::csptr distorded,
                                                             unsigned int degree ) :
...
double arlCore::Polynomial_cost_function::f( vnl_vector<double > const& x )
{
    ...
    for( i = 0; i<m_distorded->size(); ++i )
    {
        ...
        else
        {
            for( j = 0; j<m_nbEquations; ++j )
            {
                m_error += (distorded[j]-distorsion[j]-(*m_real)[i]->get(j))*
                           (distorded[j]-distorsion[j]-(*m_real)[i]->get(j));
            }
        }
    }
    return observe(x);
...
arlCore::OptimiseReprojectionUncertainty::OptimiseReprojectionUncertainty(const vector<const arlCore::Camera*>& cameras,
                                                                          const vector<arlCore::Point::csptr>& points2D)
    :
      vnl_cost_function(3),
      m_cameras(cameras),
        ...
        for(j = 0; j<m_points2DList[i].size(); ++j)
        {
            if(m_points2DList[i][j]!=0)
            {
                ...
                if(m_points2DList[i][j]->isVisible())
                {
                    ...
                    point2D(m_points2DList[i][j]->x(),m_points2DList[i][j]->y());
        ...
        for(j = 0; j<m_points3D.size(); ++j)
        {
            if(m_points2D[i][j]!=0)
            {
                ...
                {   //Ce critere calcule la distance entre les demi-droites [Cimij) et les points T*Mj
                    ...
                    crossProduct =
                        vnl_cross_3d( m_extPoints2D[i][j], extPoints3D.extract(3) )/ m_extPoints2D[i][j].two_norm();
...
{
    if(nbParameters==0 || nbParameters>8)
    {
        m_nbParameters = 8;
    }
    else
...
arlCore::Intrinsic_cost_function::~Intrinsic_cost_function()
{
    ...
    {
        if(m_2DpatternsList[i]!=0)
        {
            delete m_2DpatternsList[i];
        }
    }
    for( i = 0; i<m_3DpatternsList.size(); ++i )
    {
        if(m_3DpatternsList[i]!=0)
        {
            delete m_3DpatternsList[i];
        }
    }
    if(m_camera!=0)
    {
...
unsigned int arlCore::Intrinsic_cost_function::addPattern( const std::vector<Point::sptr >& points2D,
...
// INTRINSIC LS
arlCore::IntrinsicLS_cost_function::IntrinsicLS_cost_function(unsigned int number_of_unknowns,
                                                              unsigned int number_of_residuals, UseGradient g) :
    vnl_least_squares_function(number_of_unknowns, number_of_residuals, g)
...
unsigned int arlCore::IntrinsicLS_cost_function::addPattern( const std::vector<Point::sptr >& points2D,
                                                             const std::vector<Point::sptr >& model3D )
...
// EXTRINSIC
arlCore::Extrinsic_cost_function::Extrinsic_cost_function(const std::vector<const arlCore::Camera*>& a,
                                                          unsigned int nou) :
...
unsigned int arlCore::Extrinsic_cost_function::addPattern(const std::vector<std::vector<Point::csptr > >& points2D,
                                                          const std::vector<Point::csptr >& model3D)
{// faire addPattern pour chaque pattern rigide et chaque pose
 //assert(points2D.size()==model3D.size());
    unsigned int i, j;
    ...
    for( i = 0; i<NbPoses; ++i )
    {
        for( j = 0; j<NbCameras; ++j )
        {
            for( k = 0; k<m_3DpatternsList[i].size(); ++k )
            {
                if(j==0)
                {
                    points3DRepereCam = listTransfo[i] * m_3DpatternsList[i][k]->getHCoordinates();
                }
                else
                {
                    points3DRepereCam = listTransfo[NbPoses-1+j] * listTransfo[i] *
                                        m_3DpatternsList[i][k]->getHCoordinates();
                }
...
// EXTRINSIC LS
arlCore::ExtrinsicLS_cost_function::ExtrinsicLS_cost_function(const std::vector<const arlCore::Camera*>& a,
                                                              unsigned int number_of_unknowns,
                                                              unsigned int number_of_residuals, UseGradient g) :
    vnl_least_squares_function(number_of_unknowns, number_of_residuals, g),
    m_cameras(a)
...
unsigned int arlCore::ExtrinsicLS_cost_function::addPattern( const std::vector<std::vector<Point::csptr > >& points2D,
                                                             const std::vector<Point::csptr >& model3D )
{// faire addPattern pour chaque pattern rigide et chaque pose
 //assert(points2D.size()==model3D.size());
    unsigned int i, j;
...
double arlCore::AX_XB_cost_function::f(vnl_vector< double > const& x)
{
    ...
    if(Verbose)
    {
        cerr<<"error AX XB = "<<m_error/m_A_matrix.size()<<endl;
    }
    return observe(x);
}
...
double arlCore::AX_XB_cost_function::criterion( const arlCore::vnl_rigid_matrix& X )
{
    ...
    for( i = 0; i<m_A_matrix.size(); ++i )
    {
        ...
        if(Method==1)
        {
            error += (AX-XB).frobenius_norm();
        }
        if(Method==2)
        {
            error += RErr2 + m_alpha*TErr2;
        }
    }
    return error;
}
...
arlCore::register3D3DUncertainty_LS_cost_function::register3D3DUncertainty_LS_cost_function(
    PointList::csptr points3D_source,
    PointList::csptr points3D_cible,
    unsigned int number_of_unknowns,
