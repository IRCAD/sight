...
bool arlCore::propagateCovarianceToReconst3D( const std::vector<arlCore::Point::csptr>& list2D,
                                              const std::vector<const arlCore::Camera*>& camsList,
                                              arlCore::Point::sptr point3D, arlCore::ARLCORE_RECONSTRUCTION3D methode,
                                              std::vector<double>& log, const bool pixelFrame)
{
    unsigned int i, l, perfect_reproj2D_nb = 0;
    if(methode == arlCore::ARLCORE_R3D_TWO_LINES_PERFECT || methode == arlCore::ARLCORE_R3D_MULTI_LINES_APPROX ||
       methode == arlCore::ARLCORE_R3D_MULTI_LINES_PERFECT)
    {
        const bool verbose = false;
        if(verbose)
        {
            std::cerr<<"point3D cov matrix= \n"<< point3D->getCovMatrix() <<std::endl;
        }
    ...
    if(methode == arlCore::ARLCORE_R3D_REPROJECTION_OPTIMIZATION ||
       methode == arlCore::ARLCORE_R3D_REPROJECTION_OPTIMIZATION_UNCERTAINTY)
    {
        for(l = 0; l<camsList.size(); ++l)
        {
            if( list2D[l]->getCovMatrix().is_zero() )
        ...
        for(l = 0; l<camsList.size(); ++l)
        {
            ...
            dml_dM[l] = (camsList[l]->getQint().as_matrix() - tmp_mat_2_1 * mat_Cint) *
                        camsList[l]->getExtrinsic().getRotation();
            dml_dM[l] /= inner_product_Cl_Pil_M;
            if(verbose)
            {
                std::cerr<<"dml_dM[l]= "<<dml_dM[l]<<std::endl;
            }
            ...
            if(methode == arlCore::ARLCORE_R3D_REPROJECTION_OPTIMIZATION)
            {
                ...
                if(verbose)
                {
                    std::cerr<<"mat cov="<<  list2D[l]->getCovMatrix()(0,0) <<" "<<list2D[l]->getCovMatrix()(1,1)<<"\n";
                }
                // calcul de U + U.transpose NEGLIGEABLE !!
                U = (camsList[l]->getExtrinsic().getRotation()).transpose().as_matrix() *
                    (mat_Cint.transpose().as_matrix()) * tilde_mi_sous_mi.as_matrix() * dml_dM[l];
                ...
                H          += 1/sigma2D_2 * dml_dM[l].transpose() * dml_dM[l] + 1/sigma2D_2/inner_product_Cl_Pil_M *
                              (U+U.transpose());
            ...
            if(methode == arlCore::ARLCORE_R3D_REPROJECTION_OPTIMIZATION_UNCERTAINTY)
            {
                ...
                // calcul de U + U.transpose NEGLIGEABLE !!
                U = (camsList[l]->getExtrinsic().getRotation()).transpose().as_matrix() *
                    (mat_Cint.transpose().as_matrix()) * tilde_mi_sous_mi.as_matrix() * sigma2D_inv * dml_dM[l];
                //std::cerr<<"U + U^t/sigma2D/<Cl|Pil*M>= "<<1/sigma2D_2/inner_product_Cl_Pil_M *(U+U.transpose())<<std::endl;
                dPhi_dml[l] = -dml_dM[l].transpose() * sigma2D_inv;
                H          += dml_dM[l].transpose()* sigma2D_inv * dml_dM[l] + 1/inner_product_Cl_Pil_M *
                              (U+U.transpose());
                //std::cerr<<"H^l="<<H<<std::endl;
        ...
        if(verbose)
        {
            std::cerr<< "Hessian matrix ="<< H<<std::endl;
        }
        Hinv = vnl_matrix_inverse<double>(H);
        if(verbose)
        {
            std::cerr<< "Hessian inv ="<< Hinv <<std::endl;
            std::cerr<< "Gamma="<< Gamma <<std::endl;
        }
        Sigma_Point_Rec = Hinv * Gamma.as_matrix() * Hinv.transpose();
        if(verbose)
        {
            std::cerr<< "matrice covariance 3D = "<< Sigma_Point_Rec << std::endl;
        }
        arlCore::vnl_covariance_matrix& cov_mat = point3D->getCovMatrix();
        for(i = 0; i<3; ++i)
        {
...
//      tmp = vnl_transpose(P.getRotation()) * tmp * P.getRotation();
        ...
        arlCore::Point::sptr m1C1 = arlCore::Point::New((-1.0)*(*focal_m1)[0], (-1.0)*(*focal_m1)[1],
                                                        (-1.0)*(*focal_m1)[2]);
        if(verbose)
        {
            std::cerr<<"m1C1 ="<<m1C1->print()<<"\n";
        }
...
//              cov_mat_J.put(i,j, landa2[0]*landa2[0]*m1C1.getCovMatrix()[i][j]);
        ...
        if(verbose)
        {
            std::cerr<<"point3D_rep_cam="<<point3D_rep_cam->print()<<"\n";
        }
...
//   "IncompleteGammaP", "Invalid arguments (must be positive)");
    if(x < (a+1.0))
    {
        return IGammaSeries(a,x);
    }
    else
    {
        return 1.0-IGammaCFraction(a,x);
    }
}
