    ...
    m_endError(-1)
{
    ...
    if(justVisible)
    {
        m_modelSize = model->getPointList()->visibleSize();
    }
    else
    {
        m_modelSize = model->getPointList()->size();
    }
    if(justVisible)
    {
        m_cloudSize = cloud->visibleSize();
...
double arlCore::ICP::computeCriterion( const arlCore::vnl_rigid_matrix& M, vnl_vector< double >& fx )
{
    ...
    {   // Points to points
        ...
        {   // Search the matching point for every point of cloud
            for( j = 0; j<3; ++j )
            {
                Pt[j] = rotInit[j][0]*m_cloudPoints[i][0]+rotInit[j][1]*m_cloudPoints[i][1]+rotInit[j][2]*
                        m_cloudPoints[i][2]+traInit[j];
            }
            m_ANNtree->annkSearch( Pt, m_nbNN, m_nn_idx, m_squaredDists, Epsilon );
            if(fx.size()>i)
            {
                fx[i] = m_squaredDists[0];
            }
...
{
#ifdef ANN
    if(m_cloudSize<1 || !m_point2PointMode)
    {
        return false;
    }
    unsigned int i, j;
    ...
    for( i = 0; i<m_cloudSize; ++i )
    {
        for( j = 0; j<3; ++j )
        {
            m_Pi[i][j] = m_Pk[i][j] = m_rotInit[j][0]*m_cloudPoints[i][0]+m_rotInit[j][1]*m_cloudPoints[i][1]+
                                      m_rotInit[j][2]*m_cloudPoints[i][2]+m_traInit[j];
...
//  http://ieeexplore.ieee.org/iel1/34/3469/00121791.pdf?tp=&arnumber=121791&isnumber=3469
    ...
    for( i = 0; i<3; ++i )
    {
        Tend[i] = m_cloudGravity[i]-
                  (Rend[i][0]*m_modelGravity[0]+Rend[i][1]*m_modelGravity[1]+Rend[i][2]*m_modelGravity[2]);
    }
