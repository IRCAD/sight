...
#ifndef __ARLCORE_SCENE_H__
#define __ARLCORE_SCENE_H__
...
public:
     ...
     */
    ARLCORE_API virtual bool addCameras(unsigned int nbCams, CSPTR(
                                            Point ) centre, double sphereRadius, double distMin,
                                        const std::vector<double> &intrinsicParam,
                                        const std::vector<double> &intrinsicRange);
     ...
     *
     */
    ARLCORE_API unsigned int detection( unsigned int cam, unsigned int tagNo, SPTR(
                                            SmartPointList ) spl, double gaussianNoise = 0 );

protected:
    ARLCORE_API Scene( PlaneSystem&, Tags::sptr );
    ARLCORE_API virtual bool dynamicBehavior( unsigned int step );
    ARLCORE_API virtual unsigned int detectionBehavior( unsigned int cam, SPTR( SmartPointList ), std::vector< CSPTR(
                                                                                                                   Tag) > &tags, std::map< CSPTR(
                                                                                                                                               Tag),
                                                                                                                                           unsigned int >&allTags );
