...
#ifndef __ARLCORE_RECONST3D_H__
#define __ARLCORE_RECONST3D_H__
...
enum ARLCORE_RECONSTRUCTION3D { ARLCORE_R3D_UNKNOWN,
                                ...
                                ARLCORE_R3D_REPROJECTION_OPTIMIZATION,
                                ARLCORE_R3D_REPROJECTION_OPTIMIZATION_UNCERTAINTY,
                                ARLCORE_R3D_NBTYPE };

static std::string ARLCORE_RECONSTRUCTION3D_NAMES[ARLCORE_R3D_NBTYPE] = { "ARLCORE_R3D_UNKNOWN",
                                                                          "ARLCORE_R3D_HORAUD_APPROX",
                                                                          "ARLCORE_R3D_HORAUD_PERFECT",
                                                                          "ARLCORE_R3D_TWO_LINES_APPROX",
                                                                          "ARLCORE_R3D_TWO_LINES_PERFECT",
                                                                          "ARLCORE_R3D_MULTI_LINES_APPROX",
                                                                          "ARLCORE_R3D_MULTI_LINES_PERFECT",
                                                                          "ARLCORE_R3D_REPROJECTION_OPTIMIZATION",
                                                                          "ARLCORE_R3D_REPROJECTION_OPTIMIZATION_UNCERTAINTY"};
 ...
 */
ARLCORE_API bool reconst3D( const std::vector<Point::csptr> &points2D, const std::vector< const Camera*> &camsList, SPTR(
                                Point ) point3D,
                            ARLCORE_RECONSTRUCTION3D, unsigned int& plane, std::vector<double> &log, double errorMax = -1,  bool propagateCov = false,
                            const bool pixelFrame = true);

ARLCORE_API bool reconst3D( const std::vector<Point::csptr> &list2D, const std::vector<Camera> &camsList, SPTR(
                                Point ) point3D,
                            ARLCORE_RECONSTRUCTION3D, unsigned int& plane, std::vector<double> &log, double errorMax = -1,  bool propagateCov = false,
                            const bool pixelFrame = true);
/**
 ...
 *
 */
ARLCORE_API bool planarHomographyRegistration_3D_2D( const Camera&, const std::vector< Point::csptr >& points2D,
                                                     const std::vector< Point::csptr >& model3D,
                                                     vnl_rigid_matrix& T,
                                                     const std::vector<double>& optimiser_parameters,
                                                     std::vector<double>& log, bool verbose = false);
 ...
 *
 */
ARLCORE_API bool planarHomographyUnknownIntrinsic( const std::vector<Point::sptr>& points2D,
                                                   const std::vector< Point::sptr >& model3D, vnl_matrix_fixed<double,3,
                                                                                                               3>& H,
                                                   const std::vector<double>& optimiserParameters,
                                                   std::vector<double>& log, bool verbose);

ARLCORE_API bool planarHomographyUnknownIntrinsic( CSPTR( PointList ) points2D, CSPTR(
                                                       PointList ) model3D, vnl_matrix_fixed<double,3,3> &H, const std::vector<double> &optimiserParameters, std::vector<double> &log,
                                                   bool verbose);
...
static std::string ARLCORE_PROJECTIVE_REGISTRATION_NAMES[ARLCORE_PR_NBTYPES] = { "ARLCORE_PR_UNKNOWN",
                                                                                 ...
                                                                                 "ARLCORE_PR_ISPPC_LM",
                                                                                 "ARLCORE_PR_ISPPC_CG",
                                                                                 "ARLCORE_PR_ISPPC_ANISOTROP",
                                                                                 "ARLCORE_PR_ISPPC_ANISOTROP_LM",
                                                                                 "ARLCORE_PR_OSPPC",
                                                                                 "ARLCORE_PR_OSPPC_LM",
                                                                                 "ARLCORE_PR_OSPPC_CG",
                                                                                 "ARLCORE_PR_EPPC",
...
ARLCORE_API bool monoViewPointRegistration3D2D( const arlCore::Camera &,
                                                const std::vector<Point::csptr> &points2D, CSPTR(
                                                    PointList ) model3D,
...
ARLCORE_API bool multiViewPointRegistration3D2D ( const std::vector< const Camera* >&,
                                                  const std::vector< std::vector<Point::csptr> >&points2D, CSPTR(
                                                      PointList ) model3D,
                                                  arlCore::vnl_rigid_matrix &T, ARLCORE_PROJECTIVE_REGISTRATION,
                                                  const std::vector<double> &optimiserParameters, std::vector<double> &log,
                                                  bool verbose);
