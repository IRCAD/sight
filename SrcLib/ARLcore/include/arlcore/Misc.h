...
#ifndef __ARLCORE_MISC_H__
#define __ARLCORE_MISC_H__
...
//! @return if n is even
template <typename T> inline bool even( const T& n )
{
    long int a = n/2; return (T)(a*2)==n;
}
 ...
 **/
ARLCORE_API unsigned int fillGrid( SPTR(
                                       PointList ) pl, unsigned int nbPointsX, double squareSizeX, unsigned int nbPointsY, double squareSizeY, unsigned int nbPointsZ = 1,
                                   double squareSizeZ = 0 );
...
ARLCORE_API bool save( const std::vector< arlCore::vnl_rigid_matrix >&, const std::string& fileName,
                       bool overwrite = true );

//! @brief Calibrate tip and orientation of a needle
ARLCORE_API bool needleCalibration( const std::vector< arlCore::vnl_rigid_matrix >& tip,
...
ARLCORE_API unsigned int save( const std::vector< vnl_vector<double> >& q, vnl_vector<double> ratio,
                               const std::vector< std::string >& names, const std::string& fileName,
                               bool overwrite = true );
ARLCORE_API unsigned int save( const std::vector< vnl_vector<double> >& q, const std::string& filename,
                               bool overwrite = true );
...
ARLCORE_API unsigned int load( std::vector< vnl_vector<double> >&, std::vector<std::string>& names,
 ...
 **/
ARLCORE_API unsigned int statistic( const std::vector<double>& values, double& min, double& max, double& rms,
                                    double& mean, double& stddev, bool square = false );
 ...
 **/
ARLCORE_API unsigned int computeStat( const vnl_vector_fixed<double,5>& stat, double& min, double& max, double& mean,
...
ARLCORE_API bool computePolynomial( unsigned int degree, const vnl_vector<double>& parameters,
 ...
 **/
ARLCORE_API bool vgl_fit_line_3d( const std::vector< vgl_point_3d<double> >& points, vgl_line_3d_2_points<double>& line,
                                  double& RMS );
 ...
 **/
ARLCORE_API unsigned int computeNeedleROI( CSPTR( Point ) tip, CSPTR(
                                               Point ) top, unsigned int width, std::vector<arlCore::Point::sptr > &corners,
...
ARLCORE_API bool extractIP( const std::string& s, unsigned int& IP1, unsigned int& IP2, unsigned int& IP3,
                            unsigned int& IP4 );
