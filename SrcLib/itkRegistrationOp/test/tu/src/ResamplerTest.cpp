...
void ResamplerTest::rotateTest()
{
    ...
    for(int i = 0; i < 64; ++i)
    {
        for(int j = 0; j < 64; ++j)
        {
            imageHelper.setPixelBuffer( computeOffset(i, j, 32, imageIn), bufferValue.get() );
        }
    }
    ...
    for(int i = 0; i < 64; ++i)
    {
        for(int j = 0; j < 64; ++j )
        {
            for(int k = 0; k < 64; ++k)
            {
                ...
                if(i == 32)
                {
                    CPPUNIT_ASSERT_EQUAL(1.f, valueOut);
                }
                else
                {
                    CPPUNIT_ASSERT_EQUAL(0.f, valueOut);
                }
            }
        }
    }
