...
bool DicomReaderTest::checkPatientACHGenou( const ::fwData::Patient::sptr &patient )
{
    bool ok = true;
    bool notReallyChecked = true;

    // Study, Acquisition
    CHECK_VALUE(ok, "Study size doesn't match : ", 1, patient->getNumberOfStudies());
    ::fwData::Study::sptr study = patient->getStudies()[0];
    CHECK_VALUE(ok, "Acquisition size doesn't match : ", 1, study->getNumberOfAcquisitions());
    ...
    CHECK_VALUE(ok,"Acquisition Date doesn't match : ", "20081028",::fwTools::getDate(acq->getCreationDate()));
    ...
    CHECK_VALUE(ok,"Acquisition Time doesn't match : ", "174446",::fwTools::getTime(acq->getCreationDate()));
    //(0008,0033) TM [174502.095]                                       # 10,1 Content Time
    //(0008,0050) SH [12514 ]                                           # 6,1 Accession Number
    //(0008,0060) CS [CT]                                               # 2,1 Modality
    CHECK_VALUE(ok,"Modality doesn't match : ", "CT", study->getModality() );
    //(0008,0070) LO [TOSHIBA ]                                         # 8,1 Manufacturer
    //(0008,0080) LO [SCANNER DE LA MODER ]                             # 20,1 Institution Name
    CHECK_VALUE(ok,"Institution Name doesn't match : ", "SCANNER DE LA MODER", study->getHospital() );
    ...
    CHECK_VALUE(ok,"Patient's Firstname doesn't match : ", "", patient->getFirstname() );
    //(0010,0020) LO [12592 ARTHRO GENOU  G ]                           # 22,1 Patient ID
    CHECK_VALUE(ok,"Patient ID doesn't match : ", "12592 ARTHRO GENOU  G", patient->getIDDicom() );
    //(0010,0030) DA [19790618]                                         # 8,1 Patient's Birth Date
    CHECK_VALUE(ok,"Patient's Birth Date doesn't match : ", "19790618", ::fwTools::getDate(patient->getBirthdate()) );
    //(0010,0040) CS [M ]                                               # 2,1 Patient's Sex
    CHECK_VALUE(ok,"Patient's Sex doesn't match :", "M" , std::string(patient->getIsMale() ? "M" : "F" ) );
    ...
    CHECK_VALUE(ok,"Study Instance UID doesn't match :", "1.2.392.200036.9116.2.6.1.48.1211418863.1225183167.375775" , study->getUID() );
    //(0020,000e) UI [1.2.392.200036.9116.2.6.1.48.1211418863.1225184516.765855]         # 58,1 Series Instance UID
    CHECK_VALUE(ok,"Series Instance UID doesn't match :", "1.2.392.200036.9116.2.6.1.48.1211418863.1225184516.765855" , acq->getUID() );
