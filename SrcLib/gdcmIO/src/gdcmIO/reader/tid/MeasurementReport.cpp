...
{
    if(root->getCodedAttribute() == ::gdcmIO::container::DicomCodedAttribute("dd1dd1", "DCM", "Imaging Measurement Report"))
    {
        BOOST_FOREACH(const SPTR(::gdcmIO::container::sr::DicomSRNode)& node, root->getCRefSubNodeContainer())
        {
            ...
            else if(node->getCodedAttribute() == ::gdcmIO::container::DicomCodedAttribute("dd1d91", "DCM", "Imaging Measurements"))
            {
                BOOST_FOREACH(const SPTR(::gdcmIO::container::sr::DicomSRNode)& subNode, node->getCRefSubNodeContainer())
                {
