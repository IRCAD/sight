...
{
    // Value Type - Type 1
    ::gdcmIO::helper::DicomData::setTagValue< 0x0040, 0xa040 >(m_type, dataset);

    // Relationship Value - Type 1 (Shouldn't be there for root node)
    if(!m_relationship.empty())
    {
        ::gdcmIO::helper::DicomData::setTagValue< 0x0040, 0xa010 >(m_relationship, dataset);
    }
    ...
    ::gdcm::DataSet &dataset, const DicomCodedAttribute& codedAttribute) const
{
    ...
    // Code value - Type 1
    ::gdcmIO::helper::DicomData::setTagValue<0x0008,0x0100>(codedAttribute.getCodeValue(), itemDataset);

    // Coding Scheme Designator - Type 1
    ::gdcmIO::helper::DicomData::setTagValue<0x0008,0x0102>(codedAttribute.getCodingSchemeDesignator(), itemDataset);

    // Coding Scheme Version - Type 1C
    if (!m_codedAttribute.getCodingSchemeVersion().empty())
    {
        ::gdcmIO::helper::DicomData::setTagValue<0x0008,0x0103>(codedAttribute.getCodingSchemeVersion(), itemDataset);
    }

    // Code Meaning - Type 1
    ::gdcmIO::helper::DicomData::setTagValue<0x0008,0x0104>(codedAttribute.getCodeMeaning(), itemDataset);
