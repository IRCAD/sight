...
void Surface::writeSurfaceSegmentationModule(unsigned int segmentationNumber)
{
    ...
    segment->SetSegmentNumber(segmentationNumber + 1);
    OSLM_TRACE("Segment Number : " << segmentationNumber + 1);
    ...
    referencedSurfaceSequence->SetLengthToUndefined();
    {
        ...
        BOOST_FOREACH(const std::string& sopInstanceUID, referencedSOPInstanceUIDContainer)
        {
            ...
            ::gdcmIO::helper::DicomData::setTagValue< 0x0008, 0x1150 >(referencedSOPClassUID, imageSOPDataset);

            // SOP Instance UID - Type 1
            ::gdcmIO::helper::DicomData::setTagValue< 0x0008, 0x1155 >(sopInstanceUID, imageSOPDataset);
        ...
        ::gdcmIO::helper::DicomData::insertSQ< 0x0066, 0x002B >(referencedSurfaceSequence, segmentationDataset);
    }
    surfaceSequence->AddItem(surfaceItem);

    // Segment Sequence
    ::gdcmIO::helper::DicomData::insertSQ< 0x0062, 0x0002 >(surfaceSequence, dataset);
...
void Surface::writeSurfaceMeshModule(unsigned int segmentationNumber)
{
    ...
    surfaceSequence->SetLengthToUndefined();
    {
        ...
        pointsSequence->SetLengthToUndefined();
        {
            ...
            ::gdcmIO::helper::DicomData::insertSQ< 0x0066, 0x0011 >(pointsSequence, surfaceDataset);
        }
        ...
        ::gdcmIO::helper::DicomData::insertSQ< 0x0066, 0x0012 >(normalSequence, surfaceDataset);
        ...
        ::gdcmIO::helper::DicomData::insertSQ< 0x0066, 0x0013 >(primitivesSequence, surfaceDataset);

        surfaceSequence->AddItem(surfaceItem);
    }

    // Surface Sequence
    ::gdcmIO::helper::DicomData::insertSQ< 0x0066, 0x0002 >(surfaceSequence, dataset);
...
void Surface::writeContentIdentificationMacro(unsigned int segmentationNumber)
{
    ...
    ::gdcmIO::helper::DicomData::setTagValue< int, 0x0020, 0x0013 >(segmentationNumber, dataset);

    // Content Label - Type 1
    ::gdcmIO::helper::DicomData::setTagValue< 0x0070, 0x0080 >("SURFACE", dataset);

    // Content Description - Type 2
    ::gdcmIO::helper::DicomData::setTagValue< 0x0070, 0x0081 >("Surface Segmentation", dataset);
