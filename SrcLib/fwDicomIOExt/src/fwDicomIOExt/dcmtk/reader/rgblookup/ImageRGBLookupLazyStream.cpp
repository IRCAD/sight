...
#include "fwDicomIOExt/dcmtk/reader/rgblookup/ImageRGBLookupLazyReader.hpp"
...
std::streamsize ImageRGBLookupLazySource::read(char* stream, std::streamsize size)
{
    ...
    if(m_currentPosition + size > m_frameSize)
    {
        ...
        if(remainingBytes > 0)
        {
            std::copy( m_frame + m_currentPosition, m_frame + m_currentPosition + remainingBytes, stream);
