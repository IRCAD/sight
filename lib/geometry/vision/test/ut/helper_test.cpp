...
static inline void compare_chessboards(
    const std::vector<sight::data::point::point_coord_array_t>& _expected,
    const sight::data::point_list& _detected
)
{
    CHECK_EQ(_expected.size(), _detected.size());
    ...
    for(std::uint32_t i = 0 ; i < _expected.size() ; ++i)
    {
        ...
        CHECK(expected_coords[0] == doctest::Approx((*detected_coords)[0]).epsilon(0.1));
        CHECK(expected_coords[1] == doctest::Approx((*detected_coords)[1]).epsilon(0.1));
        CHECK_EQ(0.0, (*detected_coords)[2]);
    }
}
...
TEST_SUITE("sight::geometry::vision::helper")
{
    TEST_CASE("reprojection_identity")
    {
        ...
        sight::geometry::vision::helper::error_and_points_t result =
        ...
        CHECK(0. == doctest::Approx(result.first).epsilon(10e-6));
        ...
        for(unsigned int i = 0 ; i < 4 ; ++i)
        {
            ...
            CHECK(
                static_cast<double>(expected.x) == doctest::Approx(static_cast<double>(actual.x)).epsilon(10e-6)
            );
            CHECK(
                static_cast<double>(expected.y) == doctest::Approx(static_cast<double>(actual.y)).epsilon(10e-6)
            );
        }
    }
...
//------------------------------------------------------------------------------
    ...
    TEST_CASE("reprojection_real_dataset_test1")
    {
        ...
        CHECK(expected_err == doctest::Approx(result.first).epsilon(10e-6));
...
//------------------------------------------------------------------------------
    ...
    TEST_CASE("tool_calibration_basic")
    {
        ...
        {
            const double angle_in_degrees = sight::core::tools::random::safe_rand() % 180;
            const double angle            = glm::radians(angle_in_degrees);

            const double x = sight::core::tools::random::safe_rand() % 100 / 100.;
            const double y = sight::core::tools::random::safe_rand() % 100 / 100.;
            const double z = sight::core::tools::random::safe_rand() % 100 / 100.;
            ...
            sight::geometry::data::from_glm_mat(*mat, matrix);
        ...
        for(std::uint8_t i = 0 ; i < 3 ; ++i)
        {
            for(std::uint8_t j = 0 ; j < 3 ; ++j)
            {
                ...
                CHECK_MESSAGE(
                    val == doctest::Approx((*res_transform_matrix)(i, j)).epsilon(0.00001),
                    "transform[",
                    std::to_string(i),
                    "][",
                    std::to_string(j),
                ...
                CHECK_MESSAGE(
                    val == doctest::Approx((*res_center_matrix)(i, j)).epsilon(0.00001),
                    "center[",
                    std::to_string(i),
                    "][",
                    std::to_string(j),
            ...
            CHECK_MESSAGE(
                -translation[i] == doctest::Approx((*res_transform_matrix)(i, 3)).epsilon(0.00001),
                "transform[",
                std::to_string(i),
                "][3]"
            );
            CHECK_MESSAGE(
                center[i] == doctest::Approx((*res_center_matrix)(i, 3)).epsilon(0.00001),
                "center[",
                std::to_string(i),
                "][3]"
            );
...
//------------------------------------------------------------------------------
    ...
    TEST_CASE("tool_calibration")
    {
        ...
        for(std::size_t i = 0 ; i < nb_matrices ; ++i)
        {
            const double angle_in_degrees = sight::core::tools::random::safe_rand() % 360;
            const double angle            = glm::radians(angle_in_degrees);
...
//------------------------------------------------------------------------------
    ...
    TEST_CASE("chessboard_detection")
    {
        const auto calib_data_dir = sight::utest_data::dir() / "sight" / "calibration";
        {
            const cv::Mat chess_rgb0 = read_rgb_image((calib_data_dir / "chessboardRGB0.tiff").string());

            CHECK_EQ(false, chess_rgb0.empty());
            CHECK_EQ(3, chess_rgb0.channels());
            CHECK_EQ(static_cast<std::size_t>(1), chess_rgb0.elemSize1());

            const sight::data::point_list::csptr detected_chess = sight::geometry::vision::helper::detect_chessboard(
                chess_rgb0,
                9,
                6,
                0.25F
            ...
            const std::vector<sight::data::point::point_coord_array_t> expected_chessboard {
                {743.776, 429.954, 0.0},
                {822.537, 406.108, 0.0},
                {903.759, 383.811, 0.0},
                {984.949, 360.995, 0.0},
...

        {
            ...
            CHECK_EQ(false, chess_rgb1.empty());
            CHECK_EQ(3, chess_rgb1.channels());
            CHECK_EQ(static_cast<std::size_t>(1), chess_rgb1.elemSize1());
...

        {
            ...
            CHECK_EQ(false, chess_gray.empty());
            CHECK_EQ(1, chess_gray.channels());
            CHECK_EQ(static_cast<std::size_t>(1), chess_gray.elemSize1());
            ...
            const sight::data::point_list::csptr detected_chess2 = sight::geometry::vision::helper::detect_chessboard(
                chess_gray,
                8,
                5,
                0.25F
            );
            CHECK(!detected_chess2);
        }
    }

//------------------------------------------------------------------------------

    TEST_CASE("chessboard_detection_scale")
    {
        const auto calib_data_dir = sight::utest_data::dir() / "sight" / "calibration";
        ...
        const sight::data::point_list::csptr detected_chess_full_scale =
            sight::geometry::vision::helper::detect_chessboard(chess_rgb0, 9, 6, 1.F);

        const sight::data::point_list::csptr detected_chess_quarter_scale =
            sight::geometry::vision::helper::detect_chessboard(chess_rgb0, 9, 6, 0.25F);

        CHECK_EQ(detected_chess_full_scale->size(), detected_chess_quarter_scale->size());
        ...
        for(std::uint32_t i = 0 ; i < detected_chess_full_scale->size() ; ++i)
        {
            ...
            for(std::uint8_t j = 0 ; j < 3 ; ++j)
            {
                CHECK(full_scale_coords[j] == doctest::Approx(quarter_scale_coords[j]).epsilon(0.5));
            }
        }
    }
}
