...
#include <boost/multi_index_container.hpp> // NOLINT(misc-include-cleaner,unused-includes)
...
/**
 * @brief Helper type used to add a monitor with a custom weight through sight::core::notification::has_monitors::
 *        aggregate.
 *
 * It is implicitly built from a monitor::sptr, using a default weight of 1., so that plain monitors can be passed
 * directly. A custom weight can be specified by building it explicitly, e.g. weighted_monitor(_progress, 0.1).
 */
struct weighted_monitor
{
    core::notification::monitor::sptr progress;
    double weight {1.};

    template<typename T>
    /* *INDENT-OFF* */
    requires(std::is_convertible_v<T, core::notification::monitor::sptr>)
    /* *INDENT-ON* */
    weighted_monitor(T&& _progress, double _weight = 1.) :
        progress(std::forward<T>(_progress)),
        weight(_weight)
    {
    }
};
 ...
 */
class SIGHT_CORE_CLASS_API aggregator final : public monitor
{
public:
     ...
     * @name Typedefs
     * @{ */
    using sptr = std::shared_ptr<aggregator>;
    using wptr = std::weak_ptr<aggregator>;
    /**  @} */

    /// Default constructor. The name is initialized with an empty string
    SIGHT_CORE_API aggregator() = default;
    SIGHT_CORE_API ~aggregator() final;
     ...
     */
    SIGHT_CORE_API void add(const core::notification::monitor::sptr& _progress, double _weight = 1.);

private:

    /// aggregator container type
    using progress_seq = boost::multi_index_container<
        std::shared_ptr<monitor>,
        boost::multi_index::indexed_by<
            boost::multi_index::random_access<>,
            boost::multi_index::hashed_unique<
                boost::multi_index::identity<std::shared_ptr<monitor> >
            >
        >
    >;
    ...
    using progress_info_map = std::map<core::notification::monitor*, progress_info>;
