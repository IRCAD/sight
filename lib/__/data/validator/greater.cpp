...
SIGHT_REGISTER_DATA_VALIDATOR(sight::data::validator::greater);

//------------------------------------------------------------------------------

void greater::configure(const config_t& _config)
{
...
sight::data::validator::return_t greater::validate(const data::object::csptr& _object) const
{
    ...
    try
    {
        if(const auto real_obj = std::dynamic_pointer_cast<const data::real>(_object); real_obj)
        {
            ...
            {
                validation = {false,
                              "Object value " + real_obj->to_string() + " is not a valid number (NaN or Infinity)"
                };
            }
            else if(value > m_threshold)
            {
                validation = {true,
                              "Object value " + real_obj->to_string() + " is greater than "
            ...
            {
                validation = {false,
                              "Object value " + real_obj->to_string() + " is not greater than "
                              + std::to_string(m_threshold)
                };
            }
        }
        else if(const auto int_obj = std::dynamic_pointer_cast<const data::integer>(_object); int_obj)
        {
            ...
            if(!std::isfinite(value))
            {
                validation = {false, "Object value " + int_obj->to_string() + " is not a valid number (NaN or Infinity)"
                };
            }
            else if(value > m_threshold)
            {
                validation = {true,
                              "Object value " + int_obj->to_string() + " is greater than " + std::to_string(m_threshold)
                };
            }
            else
            {
                validation = {false,
                              "Object value " + int_obj->to_string() + " is not greater than "
                              + std::to_string(m_threshold)
                };
            }
        }
...
greater::auto_connect_signals_t greater::auto_connect_signals() const
{
