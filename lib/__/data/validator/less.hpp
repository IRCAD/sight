...
/**
 * @brief Defines a validator which checks if a numeric data (real or integer) is less than a specific threshold.
 ...
 */
class SIGHT_DATA_CLASS_API less final : public sight::data::validator::base
{
public:
    ...
    SIGHT_DECLARE_CLASS(
        ...
        sight::data::validator::factory::make<less>
    );
    SIGHT_DATA_API ~less() final = default;

    /** Configures the validator with the given configuration. */
    SIGHT_DATA_API void configure(const config_t& _config) final;

    /**
     * @brief Checks that the numeric object is less than the configured threshold
     * @note Given object should be a sight::data::real or sight::data::integer
     * @see data::validator::base::validate
     */
    SIGHT_DATA_API sight::data::validator::return_t validate(const sight::csptr<data::object>& _object) const final;
