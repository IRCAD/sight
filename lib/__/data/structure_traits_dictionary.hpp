 ...
 */
class SIGHT_DATA_CLASS_API structure_traits_dictionary : public object
{
public:
     ...
     */
    SIGHT_DATA_API void add_structure(structure_traits::sptr _structure_traits);

    /**
     * @brief Return the structure traits for given type
     * @pre given 'type' must exist in dictionary
     */
    SIGHT_DATA_API structure_traits::sptr get_structure(std::string _type);
    ...
    /// @{
    SIGHT_DATA_API bool operator==(const structure_traits_dictionary& _other) const noexcept;
    SIGHT_DATA_API bool operator!=(const structure_traits_dictionary& _other) const noexcept;
    /// @}
