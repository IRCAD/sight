 ...
 */
template<typename T>
T get_value(T min, T max, std::uint32_t seed_val = static_cast<std::uint32_t>(std::time(nullptr)))
{
...
void fill_container(
    ...
    std::uint32_t seed_val = static_cast<std::uint32_t>(std::time(nullptr))
)
{
    SIGHT_ASSERT("Wrong min/max value", min <= max);
    ...
    std::generate(rand_container.begin(), rand_container.end(), generator);
}

/// This is a thread safe "std::rand" implementation
CORE_API int safe_rand(bool reset = false);
