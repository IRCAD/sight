...
void ConfigParserTest::testObjectCreationWithConfig()
{
    ...
    auto configManager = service::app_config_manager::make();
    configManager->service::app_config_manager::setConfig(config);
    ...
    CPPUNIT_ASSERT(std::dynamic_pointer_cast<service::ut::TestConfigService>(srv1)->getIsUpdated());
    CPPUNIT_ASSERT(std::dynamic_pointer_cast<service::ut::TestConfigService>(srv2)->getIsUpdated() == false);
...
void ConfigParserTest::testTransferFunctionParser()
{
    ...
    auto tf = std::make_shared<sight::data::TransferFunction>();
...
service::config_t ConfigParserTest::buildObjectConfig()
{
    service::config_t config;
