...
{
    new_slot(ADD_OBJECTS_SLOT, &app_config_manager::addObjects, this);
    new_slot(REMOVE_OBJECTS_SLOT, &app_config_manager::removeObjects, this);
    new_slot(
...
void app_config_manager::setConfig(
...
void app_config_manager::launch()
{
    FW_PROFILE("launch");
...
void app_config_manager::stopAndDestroy()
{
    this->stop();
    this->destroy();
}
...
void app_config_manager::start_module()
{
    SIGHT_ERROR_IF("Module is not specified, it can not be started.", m_configId.empty());
    if(!m_configId.empty())
    {
        std::shared_ptr<core::runtime::module> module = extension::AppConfig::getDefault()->get_module(m_configId);
        module->start();
    }
}
...
void app_config_manager::create()
{
    SIGHT_ASSERT("Manager already running.", m_state == STATE_DESTROYED);

    m_addObjectConnection    = service::detail::Service::connectRegisterOut(this->slot(ADD_OBJECTS_SLOT));
    m_removeObjectConnection = service::detail::Service::connectUnregisterOut(this->slot(REMOVE_OBJECTS_SLOT));
...
void app_config_manager::update()
{
...
void app_config_manager::stop()
{
    ...
    std::vector<service::base::SharedFutureType> futures;
    {
        core::mt::scoped_lock lock(m_mutex);
...
void app_config_manager::destroy()
{
...
void app_config_manager::addExistingDeferredObject(const data::Object::sptr& _obj, const std::string& _uid)
{
    SIGHT_ASSERT(
...
data::Object::sptr app_config_manager::getConfigRoot() const
{
...
data::Object::sptr app_config_manager::findObject(const std::string& uid, std::string_view errMsgTail) const
{
#ifndef _DEBUG
...
data::Object::sptr app_config_manager::getNewObject(ConfigAttribute type, ConfigAttribute uid)
{
...
data::Object::sptr app_config_manager::getNewObject(ConfigAttribute type, const std::string& uid) const
{
...
data::Object::sptr app_config_manager::getObject(ConfigAttribute type, const std::string& uid) const
{
...
service::base::sptr app_config_manager::getNewService(const std::string& uid, const std::string& implType) const
{
...
void app_config_manager::destroyCreatedServices()
{
...
void app_config_manager::processStartItems()
{
    std::vector<service::base::SharedFutureType> futures;
...
void app_config_manager::processUpdateItems()
{
    std::vector<service::base::SharedFutureType> futures;
...
void app_config_manager::createObjects(const core::runtime::config_t& cfgElem)
{
...
void app_config_manager::createServices(const core::runtime::config_t& cfgElem)
{
    for(const auto& serviceCfg : boost::make_iterator_range(cfgElem.equal_range("service")))
    {
        ...
        else
        {
            ...
            const std::string msg = app_config_manager::getUIDListAsString(uids);
            SIGHT_DEBUG(
...
service::base::sptr app_config_manager::createService(const detail::ServiceConfig& srvConfig)
{
...
void app_config_manager::createConnections()
{
    for(const auto& elem : m_cfgElem)
    {
...
void app_config_manager::destroyProxy(
...
void app_config_manager::destroyProxies()
{
...
void app_config_manager::addObjects(data::Object::sptr _obj, const std::string& _id)
{
    core::mt::scoped_lock lock(m_mutex);
                    ...
                    if(objCfg.m_optional)
                    {
                        ...
                        if(registeredObj != object)
                        {
                            ...
                            if(srv->isStarted())
                            {
                                ...
                                srv->swapKey(objCfg.m_key, std::const_pointer_cast<data::Object>(registeredObj)).wait();
                            }
                        }
                    }
...
void app_config_manager::removeObjects(data::Object::sptr _obj, const std::string& _id)
{
    core::mt::scoped_lock lock(m_mutex);
    if(m_state != STATE_STARTED)
    {
        ...
        for(const auto& srvCfg : itDeferredObj->second.m_servicesCfg)
        {
            if(core::tools::id::exist(srvCfg.m_uid))
            {
                ...
                if(!optional)
                {
                    ...
                    SIGHT_ASSERT("Service " << srv->get_id() << " already stopped.", !srv->isStopped());
                    srv->stop().wait();
                    ...
                    service::base::wptr checkSrv = srv;
                    srv.reset();
...
void app_config_manager::connectProxy(const std::string& _channel, const ProxyConnections& _connectCfg)
{
    core::com::proxy::sptr proxy = core::com::proxy::get();
...
std::string app_config_manager::getUIDListAsString(const std::vector<std::string>& uidList)
{
