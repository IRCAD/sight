...
void FloatTest::methode1()
{
    ...
    {
        auto f0 = std::make_shared<data::Float>();
        f0->value() = VALUE;
        auto f1 = std::make_shared<data::Float>(VALUE);
        auto f2 = std::make_shared<data::Float>(VALUE + 0.1F);
        ...
        CPPUNIT_ASSERT_EQUAL(VALUE, std::make_shared<data::Float>(VALUE)->value());
    }
    ...
    for(float VALUE : NAN_VALUES)
    {
        ...
        CPPUNIT_ASSERT(!(VALUE == std::make_shared<data::Float>(VALUE)->value()));
        CPPUNIT_ASSERT(!(VALUE < std::make_shared<data::Float>(VALUE)->value()));
        CPPUNIT_ASSERT(!(VALUE > std::make_shared<data::Float>(VALUE)->value()));
    }
}
