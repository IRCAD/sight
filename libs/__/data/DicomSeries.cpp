...
void DicomSeries::shallow_copy(const Object::csptr& source)
{
    const auto& other = std::dynamic_pointer_cast<const DicomSeries>(source);
...
void DicomSeries::deep_copy(const Object::csptr& source, const std::unique_ptr<DeepCopyCacheType>& cache)
{
    const auto& other = std::dynamic_pointer_cast<const DicomSeries>(source);
...
void DicomSeries::addBinary(std::size_t _instanceIndex, const core::memory::buffer_object::sptr& _buffer)
{
