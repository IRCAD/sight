...
config_launcher::Parameters::Parameters(const service::config_t& config) :
...
void config_launcher::startConfig(
    service::base::sptr _service,
    const field_adaptor_t& _optReplaceMap
)
{
    field_adaptor_t replaceMap(_optReplaceMap);

    // Generate generic UID
    const std::string genericUidAdaptor = app::extension::config::getUniqueIdentifier(_service->get_id());
    replaceMap[config_launcher::s_GENERIC_UID_KEY] = genericUidAdaptor;
    ...
    config_manager->setConfig(appConfig.m_id, replaceMap);
...
void config_launcher::setConfig(const std::string& key)
{
