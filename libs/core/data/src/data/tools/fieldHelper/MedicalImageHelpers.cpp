...
bool MedicalImageHelpers::checkLandmarks( data::Image::sptr _pImg )
{
    ...
    if ( !_pImg->getField( data::tools::fieldHelper::Image::m_imageLandmarksId ) )
    {
        data::PointList::sptr pl = data::PointList::New();
        _pImg->setField( data::tools::fieldHelper::Image::m_imageLandmarksId, pl );
...
bool MedicalImageHelpers::checkImageSliceIndex( data::Image::sptr _pImg )
{
    ...
    if ( !(axialIdx && frontalIdx && sagittalIdx) )
    {
        ...
        _pImg->setField( data::tools::fieldHelper::Image::m_axialSliceIndexId, axialIdx );

        frontalIdx = data::Integer::New(-1);
        _pImg->setField( data::tools::fieldHelper::Image::m_frontalSliceIndexId, frontalIdx );

        sagittalIdx = data::Integer::New(-1);
        _pImg->setField( data::tools::fieldHelper::Image::m_sagittalSliceIndexId, sagittalIdx );
...
data::Point::sptr MedicalImageHelpers::getImageSliceIndices( data::Image::sptr _pImg )
{
    ...
    point->getCoord()[0] =
        static_cast<double>(_pImg->getField< data::Integer >( data::tools::fieldHelper::Image::m_sagittalSliceIndexId )
                            ->value());
    point->getCoord()[1] =
        static_cast<double>(_pImg->getField< data::Integer >( data::tools::fieldHelper::Image::m_frontalSliceIndexId  )
                            ->value());
    point->getCoord()[2] =
        static_cast<double>(_pImg->getField< data::Integer >( data::tools::fieldHelper::Image::m_axialSliceIndexId    )
                            ->value());
...
bool MedicalImageHelpers::checkComment( data::Image::sptr _pImg )
{
    ...
    if ( !_pImg->getField( data::tools::fieldHelper::Image::m_commentId ) )
    {
        // Set value
        data::String::sptr param = data::String::New("Original image");
        _pImg->setField( data::tools::fieldHelper::Image::m_commentId, param );
        fieldIsModified = true;
    }
...
bool MedicalImageHelpers::checkTransferFunctionPool(const data::Image::sptr& image)
{
    ...
    const std::string poolFieldName = data::tools::fieldHelper::Image::m_transferFunctionCompositeId;
    ...
    if ( !tfPool )
    {
        ...
        data::tools::helper::Field fieldHelper(image);
