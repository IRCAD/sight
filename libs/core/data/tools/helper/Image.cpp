...
{
    FW_DEPRECATED_MSG("::data::tools::helper::Image is no longer supported, the methods have been moved to "
...
bool Image::createLandmarks()
{
    ...
    if ( !m_image->getField( data::tools::fieldHelper::Image::m_imageLandmarksId ) )
    {
        data::PointList::sptr pl = data::PointList::New();
        m_image->setField( data::tools::fieldHelper::Image::m_imageLandmarksId, pl );
        fieldIsCreated = true;
    }
...
bool Image::createTransferFunctionPool()
{
    ...
    if ( !tfPool )
    {
        ...
        data::tools::helper::Field fieldHelper(m_image);
    ...
    if(tfPool->find(defaultTFName) == tfPool->end())
    {
        ...
        else if(data::tools::fieldHelper::MedicalImageHelpers::checkImageValidity(m_image))
        {
            double min, max;
            data::tools::fieldHelper::MedicalImageHelpers::getMinMax(m_image, min, max);
...
bool Image::createImageSliceIndex()
{
    ...
    if ( !(axialIdx && frontalIdx && sagittalIdx) )
    {
        ...
        m_image->setField( data::tools::fieldHelper::Image::m_axialSliceIndexId, axialIdx );

        frontalIdx = data::Integer::New(-1);
        m_image->setField( data::tools::fieldHelper::Image::m_frontalSliceIndexId, frontalIdx );

        sagittalIdx = data::Integer::New(-1);
        m_image->setField( data::tools::fieldHelper::Image::m_sagittalSliceIndexId, sagittalIdx );
