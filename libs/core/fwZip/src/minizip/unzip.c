...
extern int ZEXPORT unzOpenCurrentFile3(unzFile file, int* method, int* level, int raw, const char* password)
{
    ...
    pfile_in_zip_read_info->pos_in_zipfile = s->cur_file_info_internal.offset_curfile + SIZEZIPLOCALHEADER +
                                             size_variable;
...
extern int ZEXPORT unzReadCurrentFile(unzFile file, voidp buf, uint32_t len)
{
    ...
    do
    {
        ...
        else if (s->pfile_in_zip_read->compression_method == Z_BZIP2ED)
        {
            ...
            s->pfile_in_zip_read->crc32                   = crc32(s->pfile_in_zip_read->crc32, buf_before,
                                                                  (uint32_t)out_bytes);
