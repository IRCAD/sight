    ...
    const data::ActivitySeries::csptr& activitySeries) const
{
    ...
    for (std::string validatorImpl : info.validatorsImpl)
    {
        ...
        activities::IValidator::ValidationType validation = activityValidator->validate(activitySeries);
        if(!validation.first)
        {
            message += "\n" + validation.second;
            isValid  = false;
...
{
    activities::registry::ActivityInfo info;
    info = activities::registry::Activities::getDefault()->getInfo(m_mainActivityId);
