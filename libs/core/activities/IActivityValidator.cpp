...
const
{
    ...
    info = activities::registry::Activities::getDefault()->getInfo(activity->getActivityConfigId());

    data::Composite::sptr composite = activity->getData();

    for (activities::registry::ActivityRequirement req: info.requirements)
    {
            ...
            else
            {
                ...
                else
                {
                    ...
                    for (auto elt : *currentComposite)
                    {
                        ...
                        activities::registry::ActivityRequirementKey reqKey;
                        bool keyIsFound = false;
                        for (activities::registry::ActivityRequirementKey keyElt: req.keys)
                        {
                            if (key == keyElt.key)
                            {
                                reqKey     = keyElt;
...
IValidator::ValidationType IActivityValidator::checkParameters(const data::ActivitySeries::csptr& activity) const
{
    ...
    activities::registry::ActivityAppConfig appConfigInfo = info.appConfig;
