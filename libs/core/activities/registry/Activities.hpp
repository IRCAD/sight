...
struct ACTIVITIES_CLASS_API ActivityAppConfigParam
{
    ActivityAppConfigParam()
    {
    }
    ACTIVITIES_API ActivityAppConfigParam(const ConfigType& config);
...
struct ACTIVITIES_CLASS_API ActivityAppConfig
{
    ...
    ACTIVITIES_API ActivityAppConfig(const ConfigType& config);

    std::string id;
    ActivityAppConfigParamsType parameters;
};

struct ACTIVITIES_CLASS_API ActivityRequirementKey
{
    ActivityRequirementKey()
    {
    }
    ACTIVITIES_API ActivityRequirementKey(const ConfigType& config);

    std::string key;
    std::string path;
};

struct ACTIVITIES_CLASS_API ActivityRequirement
{
    ActivityRequirement()
    {
    }
    ACTIVITIES_API ActivityRequirement(const ConfigType& config);
 ...
 *   - default builder is activities::builder::ActivitySeries
 ...
 * @code{.xml}
    <extension implements="::activities::registry::Activities">
        ...
        <requirements>
            ...
            <requirement name="modelSeries" type="data::ModelSeries" minOccurs="1" maxOccurs="1">
                 ...
                 <validator>activities::validator::ImageProperties</validator>
            </requirement>
            <requirement name="transformationMatrix" type="data::TransformationMatrix3D" minOccurs="0" maxOccurs="1"
 * create="true" />
            <!--# ...-->
        </requirements>
        <builder>activities::builder::ActivitySeries</builder>
        <validators>
            <validator>activities::validator::RelatedStudy</validator>
        </validators>
 ...
 * @endcode
 */
struct ACTIVITIES_CLASS_API ActivityInfo
{
    ...
    ACTIVITIES_API ActivityInfo(const SPTR(core::runtime::Extension)& ext);

    ACTIVITIES_API bool usableWith(DataCountType dataCount) const;
 ...
 *
 * @see activities::registry::ActivityInfo
 */
class ACTIVITIES_CLASS_API Activities : public core::BaseObject
{
...
public:
    ...
    ACTIVITIES_API static Activities::sptr getDefault();

    /// Destructor
    ACTIVITIES_API virtual ~Activities();
     ...
     * @{
     **/
    ACTIVITIES_API void parseBundleInformation();
    ///@}
     ...
     * @note This method is thread safe.
     **/
    ACTIVITIES_API const ActivityInfo getInfo( const std::string& extensionId ) const;

    /**
     * @brief Tests if we have information about operator
     * @note This method is thread safe.
     */
    ACTIVITIES_API bool hasInfo( const std::string& extensionId ) const;

    /**
     * @brief Get the number of vector objects in the same type.
     */
    ACTIVITIES_API ActivityInfo::DataCountType getDataCount( const CSPTR(data::Vector)& data ) const;

    /**
     * @brief Get all infos
     * @note This method is thread safe.
     */
    ACTIVITIES_API ActivitiesType getInfos() const;

    /**
     * @brief Get available activities for given data.
     * @note This method is thread safe.
     */
    ACTIVITIES_API ActivitiesType getInfos( const CSPTR(data::Vector)& data ) const;

    /**
     * @brief Get all keys
     * @note This method is thread safe.
     */
    ACTIVITIES_API std::vector< std::string > getKeys() const;

    /**
     * @brief Clear the registry.
     * @note This method is thread safe.
     */
    ACTIVITIES_API void clearRegistry();
