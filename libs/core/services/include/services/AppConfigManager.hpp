 ...
 */
class SERVICES_CLASS_API AppConfigManager :  public services::IAppConfigManager,
                                             public core::com::HasSlots
{
public:

    fwCoreClassMacro(AppConfigManager, services::IAppConfigManager, std::make_shared< AppConfigManager >)

    fwCoreAllowSharedFromThis()

    /// Creates slots.
    SERVICES_API AppConfigManager();

    /// Does nothing.
    SERVICES_API virtual ~AppConfigManager();
     ...
     */
    SERVICES_API virtual void setConfig(const std::string& _configId,
     ...
     */
    SERVICES_API virtual data::Object::sptr getConfigRoot() const override;

    /// Calls methods : create, start then update.
    SERVICES_API virtual void launch() override;

    /// Stops and destroys services specified in config, then resets the configRoot sptr.
    SERVICES_API virtual void stopAndDestroy() override;

    /// Creates objects and services from config.
    SERVICES_API virtual void create() override;

    /// Starts services specified in config.
    SERVICES_API virtual void start() override;

    /// Updates services specified in config.
    SERVICES_API virtual void update() override;

    /// Stops services specified in config.
    SERVICES_API virtual void stop() override;

    /// Destroys services specified in config.
    SERVICES_API virtual void destroy() override;

    /**
     * @brief Sets if we are testing the class.
     * @param _isUnitTest Use true to set it as a test.
     */
    SERVICES_API void setIsUnitTest(bool _isUnitTest);
     ...
     */
    SERVICES_API void addExistingDeferredObject(const data::Object::sptr& _obj, const std::string& _uid);

private:

    typedef ::std::pair< std::string, bool > ConfigAttribute;
    typedef services::helper::ProxyConnections ProxyConnections;
    typedef services::IService::Config ServiceConfig;
     ...
     */
    SERVICES_API virtual void startModule();
    ...
    services::IService::sptr getNewService(const std::string& uid, const std::string& implType) const;
    ...
    services::IService::sptr createService(const ServiceConfig& srvConfig);
    ...
    typedef std::pair< data::Object::sptr, services::IXMLParser::sptr> CreatedObjectType;
    ...
    typedef std::vector< services::IService::wptr > ServiceContainer;
