 ...
 */
class SERVICES_CLASS_API Proxy : public core::BaseObject
{
...
public:
    ...
    SERVICES_API virtual ~Proxy();

    /// Returns an instance of Proxy.
    SERVICES_API static Proxy::sptr getDefault();

    /// Registers a signal in the channel. It will be connected to all slots in the channel.
    SERVICES_API void connect(ChannelKeyType channel, core::com::SignalBase::sptr signal);

    /// Registers a slot in the channel. It will be connected to all signals in the channel.
    SERVICES_API void connect(ChannelKeyType channel, core::com::SlotBase::sptr slot);

    /// Unregisters the signal. Disconnects it from the slots in channel
    SERVICES_API void disconnect(ChannelKeyType channel, core::com::SignalBase::sptr signal);

    /// Unregisters the slot. Disconnects it from the signals in channel
    SERVICES_API void disconnect(ChannelKeyType channel, core::com::SlotBase::sptr slot);
