 ...
 *
 * @code{.cpp}
    m_appMgr = std::make_unique< services::AppManager >();
 ...
 * @code{.cpp}
    ...
    void MyManager::initialize()
    {
        ...
        if (this->checkInputs())
        {
            ...
            services::IService::ConfigType mesherConfig;
            mesherConfig.put("config.percentReduction", reduction);
            mesher->configure(mesherConfig);

            services::helper::ProxyConnections connection(this->getInputID(s_VALIDATION_CHANNEL));
            connection.addSignalConnection(mesher->getID(), services::IService::s_UPDATED_SIG);
   ...
   @endcode
 */
class SERVICES_CLASS_API AppManager : public core::com::HasSlots
{
public:

    SERVICES_API AppManager();
    SERVICES_API ~AppManager();

    typedef services::IService::AccessType AccessType;

    /// Initialize the Manager
    SERVICES_API virtual void create();

    /// Uninitialize the Manager
    SERVICES_API virtual void destroy();
     ...
     */
    SERVICES_API services::IService::sptr addService(const std::string& type, const std::string& uid,
     ...
     * @return Return the created service
     */
    SERVICES_API services::IService::sptr addService(const std::string& type, bool autoStart = false,
     ...
     * @param autoStart if true, the service will be started when all its required inputs are present.
     */
    SERVICES_API void addService(const services::IService::sptr& srv, bool autoStart = false,
                                 bool autoUpdate = false);
     ...
     */
    SERVICES_API void startService(const services::IService::sptr& srv);
     ...
     */
    SERVICES_API void stopService(const services::IService::sptr& srv);
     ...
     */
    SERVICES_API void startServices();

    /// Stop all the started service and unregister all the registered service
    SERVICES_API void stopAndUnregisterServices();
     ...
     */
    SERVICES_API void addObject(data::Object::sptr obj, const std::string& id);
     ...
     *        AppManager, it is different from the uid.
     */
    SERVICES_API void removeObject(data::Object::sptr obj, const std::string& id);
     ...
     */
    SERVICES_API data::Object::sptr getObject(const std::string& id) const;

    /// Add a proxy connection
    SERVICES_API void addProxyConnection(const helper::ProxyConnections& proxy);
     ...
     */
    SERVICES_API void replaceInput(const std::string& key, const std::string& value);

    /// Check if all the required inputs are present and add the object in the manager
    SERVICES_API bool checkInputs();

    /// Return the AppManager unique identifier
    SERVICES_API const std::string& getID() const;
...
protected:
     ...
     */
    SERVICES_API std::string getInputID(const std::string& id) const;
     ...
     */
    SERVICES_API void requireInput(const std::string& key, const InputType& type,
...
private:
    ...
    {
        /// constructor
        SERVICES_API ServiceInfo(const services::IService::sptr& srv, const bool autoStart,
                                 const bool autoUpdate);

        /// service
        services::IService::wptr m_service;
    ...
    void internalAddService(const services::IService::sptr& srv, const bool autoStart, const bool autoUpdate);

    /// Return the service information
    ServiceInfo& getServiceInfo(const services::IService::sptr& srv);

    /// Return the service information
    const ServiceInfo& getServiceInfo(const services::IService::sptr& srv) const;

    /// Start the service and connect its slots and signals
    services::IService::SharedFutureType start(const ServiceInfo& info);

    /// Stop the service and connect its slots and signals
    services::IService::SharedFutureType stop(const ServiceInfo& info);
    ...
    std::vector< services::IService::sptr > m_startedService;
