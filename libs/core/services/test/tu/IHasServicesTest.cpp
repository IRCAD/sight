...
CPPUNIT_TEST_SUITE_REGISTRATION( ::sight::services::ut::IHasServicesTest );
...
static const std::string srvType = "::sight::services::ut::TestSrvAutoconnect";

struct TestIHasServices : public services::IHasServices
{
    ...
    void testConnection()
    {
        ...
        {
            auto testService = this->registerService< services::ut::TestSrvAutoconnect >(srvType);
        ...
        // The destructor of services::IHasServices would assert if unregister is not done properly
    ...
    void testRegistration()
    {
        ...
        {
            services::IService::wptr refService1;
            {
            ...
            services::IService::wptr refService2;
            {
        ...
        }
        {
            ...
            services::IService::wptr refService2;
            {
                auto testService2 = this->registerService("::sight::services::ut::TestServiceImplementation");
                testService2->start();
                refService2 = testService2;
            }
            ...
            // The destructor of services::IHasServices would assert if unregister is not done properly
            // So if the test passes, that means we are ok with the unregistering
            this->unregisterServices("::sight::services::ut::TestSrvAutoconnect");
            CPPUNIT_ASSERT(refService1.expired());
            CPPUNIT_ASSERT(!refService2.expired());

            this->unregisterServices("::sight::services::ut::TestServiceImplementation");
            CPPUNIT_ASSERT(refService1.expired());
            CPPUNIT_ASSERT(refService2.expired());
        }
        {
            auto testService1 = this->registerService< services::ut::TestSrvAutoconnect >(srvType);
            testService1->registerInOut(data1, "data1", true);
            testService1->start();

            auto testService2 = this->registerService< services::ut::TestSrvAutoconnect >(srvType);
            testService2->start();

            auto testService3 = this->registerService< services::ut::TestSrvAutoconnect >(srvType);
    ...
    void testOptionalInputs()
    {
        ...
        {
            auto testService = this->registerService< services::ut::TestServiceImplementation >(
                "::sight::services::ut::TestServiceImplementation");
            ...
            services::OSR::unregisterService("data2", services::IService::AccessType::INPUT, testService);
            testService->swapKey("data2", nullptr);
            CPPUNIT_ASSERT_EQUAL(std::string("data2"), testService->getSwappedObjectKey());
            CPPUNIT_ASSERT(nullptr == testService->getSwappedObject());
        }
