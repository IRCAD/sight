...
CPPUNIT_TEST_SUITE_REGISTRATION( ::sight::services::ut::LockTest );
...
class LockedService : public services::IService
{
public:

    static const services::IService::KeyType s_INPUT;
    static const services::IService::KeyType s_INOUT;
    static const services::IService::KeyType s_OUTPUT;

    fwCoreServiceMacro(LockedService, services::IService)
...
const services::IService::KeyType LockedService::s_INPUT  = "input";
const services::IService::KeyType LockedService::s_INOUT  = "inout";
const services::IService::KeyType LockedService::s_OUTPUT = "output";

fwServicesRegisterMacro( ::sight::services::IService, ::sight::services::ut::LockedService)
...
void LockTest::tearDown()
{
    ...
    const auto& services = services::OSR::getServices< services::IService >();
...
{
    // Add the service
    services::IService::sptr lockedService = services::add("::sight::services::ut::LockedService");
    ...
    // Register the data
    lockedService->registerInput(input, services::ut::LockedService::s_INPUT);
    lockedService->registerInOut(inout, services::ut::LockedService::s_INOUT);
    lockedService->setOutput(services::ut::LockedService::s_OUTPUT, output);
    ...
    weakInput = lockedService->getWeakInput< data::Integer >(services::ut::LockedService::s_INPUT);
    {
        auto sharedInput = weakInput.lock();
        CPPUNIT_ASSERT_EQUAL(input, sharedInput.get_shared());
    }

    weakInOut = lockedService->getWeakInOut< data::Integer >(services::ut::LockedService::s_INOUT);
    {
        auto sharedInOut = weakInOut.lock();
        CPPUNIT_ASSERT_EQUAL(inout, sharedInOut.get_shared());
    }

    weakOutput = lockedService->getWeakOutput< data::Integer >(services::ut::LockedService::s_OUTPUT);
    {
        auto sharedOutput = weakOutput.lock();
        CPPUNIT_ASSERT_EQUAL(output, sharedOutput.get_shared());
    }
    ...
    {
        auto sharedInput = lockedService->getLockedInput< data::Integer >(services::ut::LockedService::s_INPUT);
        CPPUNIT_ASSERT_EQUAL(input, sharedInput.get_shared());
    }

    {
        auto sharedInOut = lockedService->getLockedInOut< data::Integer >(services::ut::LockedService::s_INOUT);
        CPPUNIT_ASSERT_EQUAL(inout, sharedInOut.get_shared());
    }

    {
        auto sharedOutput = lockedService->getLockedOutput< data::Integer >(
            services::ut::LockedService::s_OUTPUT);
        CPPUNIT_ASSERT_EQUAL(output, sharedOutput.get_shared());
    }

    // cleanup
    services::OSR::unregisterService(lockedService);
}
...
void LockTest::testDumpLock()
{
    ...
    lockedService->registerInput(image, services::ut::LockedService::s_INPUT);

    {
        auto sharedInput = lockedService->getLockedInput< data::Image >(services::ut::LockedService::s_INPUT);
    ...
    lockedService->registerInput(mesh, services::ut::LockedService::s_INPUT);

    {
        auto sharedInput = lockedService->getLockedInput< data::Mesh >(services::ut::LockedService::s_INPUT);
...
{
    // Add the service
    services::ut::LockedService::sptr lockedService = services::ut::LockedService::dynamicCast(services::add(
                                                                                                   "::sight::services::ut::LockedService"));
    CPPUNIT_ASSERT(lockedService);
    ...
    // Test that inputLock doesn't block other reader
    {
        auto weakInput = lockedService->getWeakInput< const data::Integer >(
            services::ut::LockedService::s_INPUT);
        auto sharedInput = weakInput.lock();
        CPPUNIT_ASSERT_EQUAL(input, sharedInput.get_shared());

        std::thread t1(&::sight::services::ut::LockedService::starting, lockedService);
    ...
    // Test that outputLock is blocking
    {
        ...
        std::thread t2(&::sight::services::ut::LockedService::stopping, lockedService);
        std::this_thread::sleep_for(std::chrono::milliseconds(500));
