...
CPPUNIT_TEST_SUITE_REGISTRATION( ::sight::services::ut::ServiceConfigTest );
...
{
    services::registry::ServiceConfig::sptr currentServiceConfig;
    currentServiceConfig = services::registry::ServiceConfig::getDefault();

    const std::string configId(services::registry::AppConfig::getUniqueIdentifier());
    const std::string service("::sight::services::ut::TestService");
