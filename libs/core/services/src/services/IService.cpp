...
void IService::setOutput(const IService::KeyType& key, const data::Object::sptr& object, size_t index)
{
    ...
    if(services::OSR::isRegistered(outKey, services::IService::AccessType::OUTPUT, this->getSptr()))
    {
        services::OSR::unregisterServiceOutput(outKey, this->getSptr());
    }

    if(object != nullptr)
    {
        services::OSR::registerServiceOutput(object, outKey, this->getSptr());
    }
}
                             ...
                             const bool optional)
{
    ...
    services::OSR::registerServiceInput(obj, key, this->getSptr());
}
                              ...
                              AccessType access, const bool autoConnect, const bool optional)
{
    ...
    services::OSR::registerService(obj, key, access, this->getSptr());
}

//------------------------------------------------------------------------------

void IService::registerObject(const std::string& objId,
                              const services::IService::KeyType& key,
                              const services::IService::AccessType access,
...
{
    services::OSR::unregisterService(key, access, this->getSptr());
...
IService::ConfigType IService::getConfigTree() const
{
    ...
    // This is in case we get the configuration from a services::registry::ServiceConfig
...
{
    services::IService::KeyConnectionsMap connectionMap = this->getAutoConnections();
    ...
    for(const auto& objectCfg : m_serviceConfig.m_objects)
    {
        ...
        {
            services::IService::KeyConnectionsType connections;
...
bool IService::hasAllRequiredObjects() const
{
    ...
    for (const auto& objectCfg : m_serviceConfig.m_objects)
    {
        ...
        {
            if (objectCfg.m_access == services::IService::AccessType::INPUT)
            {
            ...
            else if (objectCfg.m_access == services::IService::AccessType::INOUT)
            {
...
void IService::registerObject(const services::IService::KeyType& key,
                              const services::IService::AccessType access,
                              const bool autoConnect, const bool optional)
{
    auto itr = std::find_if( m_serviceConfig.m_objects.begin(), m_serviceConfig.m_objects.end(),
