...
void AppConfigManager::create()
{
    ...
    m_addObjectConnection    = services::OSR::getRegisterSignal()->connect( this->slot(s_ADD_OBJECTS_SLOT) );
    m_removeObjectConnection = services::OSR::getUnregisterSignal()->connect( this->slot(s_REMOVE_OBJECTS_SLOT) );
...
void AppConfigManager::stop()
{
    ...
    services::OSR::getRegisterSignal()->disconnect( this->slot(s_ADD_OBJECTS_SLOT) );
    services::OSR::getUnregisterSignal()->disconnect( this->slot(s_REMOVE_OBJECTS_SLOT) );
    ...
    SLM_DEBUG("Parsing OSR after stopping the config :" << std::endl
                                                        << services::OSR::getRegistryInformation());
...
services::IService::sptr AppConfigManager::getNewService(const std::string& uid, const std::string& implType) const
{
    services::registry::ServiceFactory::sptr srvFactory = services::registry::ServiceFactory::getDefault();

    services::IService::sptr srv = srvFactory->create(implType);
...
void AppConfigManager::stopStartedServices()
{
    ...
    BOOST_REVERSE_FOREACH(services::IService::wptr w_srv, m_startedSrv)
    {
        SLM_ASSERT("Service expired.", !w_srv.expired());

        const services::IService::sptr srv = w_srv.lock();
...
{
    BOOST_REVERSE_FOREACH(services::IService::wptr w_srv, m_createdSrv)
    {
        SLM_ASSERT("Service expired.", !w_srv.expired());
...
void AppConfigManager::processStartItems()
{
    ...
    for(const auto& elem :  m_cfgElem->getElements())
    {
        if (elem->getName() == "start")
        {
            ...
            {
                const services::IService::sptr srv = services::get(uid);
        ...
        if (elem->getName() == "object")
        {
            ...
            else
            {
                ...
                std::string srvImpl = srvFactory->getDefaultImplementationIdFromObjectAndType(
                    obj->getClassname(), "::sight::services::IXMLParser");

                services::IService::sptr srv = srvFactory->create("::sight::services::IXMLParser", srvImpl);
                auto objectParser            = services::IXMLParser::dynamicCast(srv);
...
void AppConfigManager::createServices(core::runtime::ConfigurationElement::csptr cfgElem)
{
    for(const auto& elem : cfgElem->getElements())
    {
        if (elem->getName() == "service")
        {
            ...
            ServiceConfig srvConfig = services::helper::Config::parseService(elem, this->msgHead());
            ...
            for(const auto& objectCfg : srvConfig.m_objects)
            {
                ...
                if(objectCfg.m_access == services::IService::AccessType::OUTPUT)
                {
...
services::IService::sptr AppConfigManager::createService(const services::IService::Config& srvConfig)
{
    // Create and bind service
    const services::IService::sptr srv = this->getNewService(srvConfig.m_uid, srvConfig.m_type);
    ...
    for(const auto& objectCfg : srvConfig.m_objects)
    {
        ...
        if((obj || !objectCfg.m_optional) && objectCfg.m_access != services::IService::AccessType::OUTPUT)
        {
...
void AppConfigManager::createConnections()
{
    for(const auto& elem : m_cfgElem->getElements())
    {
        if (elem->getName() == "connect")
        {
            ...
            ProxyConnections connectionInfos = services::helper::Config::parseConnections2(elem, this->msgHead(),
                                                                                           genIdFn);
...
void AppConfigManager::addObjects(data::Object::sptr _obj, const std::string& _id)
{
    ...
    std::unordered_map<std::string, services::IService::sptr> newServices;
    ...
    for(const auto& itService : servicesCfg)
    {
        ...
        for(const auto& objCfg : srvCfg->m_objects)
        {
            ...
            if(m_createdObjects.find(objCfg.m_uid) == m_createdObjects.end())
            {
                ...
                {
                    services::IService::sptr srv = services::get(uid);
        ...
        for(const auto& srvCfg : itDeferredObj->second.m_servicesCfg)
        {
            if( core::tools::fwID::exist(srvCfg.m_uid) )
            {
                ...
                for(const auto& objCfg : srvCfg.m_objects)
                {
                    if(_id == objCfg.m_uid)
                    {
                        services::IService::sptr srv = services::get(srvCfg.m_uid);
                ...
                if(!optional)
                {
                    ...
                    services::IService::wptr checkSrv = srv;
                    srv.reset();
