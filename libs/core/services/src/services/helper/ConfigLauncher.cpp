...
void ConfigLauncher::parseConfig(const services::IService::ConfigType& _config,
                                 const services::IService::sptr& _service)
{
    services::IService::ConfigType srvCfg;
    const services::IService::ConfigType* curConfig = &_config;

    const services::IService::ConfigType& oldConfig = _config;
    SLM_ASSERT("There must be only one <appConfig/> element.", oldConfig.count("appConfig") == 1 );

    const services::IService::ConfigType& appConfig = oldConfig.get_child("appConfig");
    const std::string appCfgId                      = appConfig.get<std::string>("<xmlattr>.id");

    srvCfg.add("config.appConfig.<xmlattr>.id", appCfgId);
    services::IService::ConfigType& newCfg = srvCfg.get_child("config.appConfig");
    curConfig = &srvCfg;
    ...
    const services::IService::ConfigType& srvconfig = curConfig->get_child("config");

    SLM_ASSERT("There must be only one <appConfig/> element.", srvconfig.count("appConfig") == 1 );

    const services::IService::ConfigType& appConfigCfg = srvconfig.get_child("appConfig");
    m_appConfig = activities::registry::ActivityAppConfig(appConfigCfg);
}

//------------------------------------------------------------------------------

void ConfigLauncher::startConfig(services::IService::sptr _srv,
                                 const FieldAdaptorType& _optReplaceMap )
{
    ...
    const std::string genericUidAdaptor = services::registry::AppConfig::getUniqueIdentifier( _srv->getID() );
    ...
    // Init manager
    m_appConfigManager = services::AppConfigManager::New();
