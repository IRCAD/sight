...
#include "services/parser/Composite.hpp"
...
fwServicesRegisterMacro( ::sight::services::IXMLParser, ::sight::services::parser::Composite, ::sight::data::Composite )
    ...
    for( core::runtime::ConfigurationElement::csptr elem :  m_cfg->getElements() )
    {
        if( elem->getName() == "item" )
        {
            ...
            if( buildMode == BUILD_OBJECT )
            {
                ...
                services::IAppConfigManager::sptr ctm = services::IAppConfigManager::New();
                ctm->services::IAppConfigManager::setConfig( elem );
...
{
    for( services::IAppConfigManager::sptr ctm :  m_ctmContainer )
    {
        ctm->start();
    }
}
...
{
    BOOST_REVERSE_FOREACH( services::IAppConfigManager::sptr ctm, m_ctmContainer )
    {
        ctm->stop();
    }
}
