...
#include "services/registry/AppInfo.hpp"
 ...
 * @deprecated This class is no longer supported, use services::AppConfigManager.
 */
class SERVICES_CLASS_API IAppConfigManager : public core::tools::Object
{
public:
    ...
    [[deprecated("will be removed in sight 22.0, use services::AppConfigManager")]]
    SERVICES_API IAppConfigManager();

    /// Destructor. Do nothing.
    SERVICES_API virtual ~IAppConfigManager();
    ...
    SERVICES_API static SPTR(IAppConfigManager) New();
     ...
     */
    SERVICES_API virtual void setConfig(const std::string& configId,
    ...
    /// Get config root
    SERVICES_API virtual data::Object::sptr getConfigRoot() const = 0;

    /// Calls methods : create, start then update.
    SERVICES_API virtual void launch() = 0;

    /// Stops and destroys services specified in config, then resets the configRoot sptr.
    SERVICES_API virtual void stopAndDestroy() = 0;

    /// Creates objects and services from config
    SERVICES_API virtual void create() = 0;

    /// Starts services specified in config
    SERVICES_API virtual void start() = 0;

    /// Updates services specified in config
    SERVICES_API virtual void update() = 0;

    /// Stops services specified in config
    SERVICES_API virtual void stop() = 0;

    /// Destroys services specified in config
    SERVICES_API virtual void destroy() = 0;
