...
auto_stereo_compositor_listener::auto_stereo_compositor_listener(std::uint8_t _viewpointNumber) :
...
)
{
    ...
    if(_schemeName.find("auto_stereo") != std::string::npos)
    {
        ...
        auto* const pass = newTech->getPass(0);
        {
            ...
            const auto vpNewName        = vpBaseName + "+auto_stereo";
        ...
        if(Ogre::StringUtil::startsWith(fpBaseName, "RTV_FP"))
        {
            ...
            const auto fpNewName        = fpBaseName + "+auto_stereo";
