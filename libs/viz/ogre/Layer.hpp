...
#include "viz/ogre/compositor/listener/AutoStereo.hpp"
#include "viz/ogre/compositor/types.hpp"
 ...
 */
class VIZ_OGRE_CLASS_API Layer :
...
public:
    ...
    fwCoreClassMacro(Layer, viz::ogre::Layer, new Layer)
    ...
    VIZ_OGRE_API static const core::com::Signals::SignalKeyType s_INIT_LAYER_SIG;
    typedef core::com::Signal<void (viz::ogre::Layer::sptr)> InitLayerSignalType;

    /// Contains the signal sent when the layer is resized.
    VIZ_OGRE_API static const core::com::Signals::SignalKeyType s_RESIZE_LAYER_SIG;
    ...
    VIZ_OGRE_API static const core::com::Signals::SignalKeyType s_CAMERA_UPDATED_SIG;

    /// Contains signals sent when the camera is modified.
    VIZ_OGRE_API static const core::com::Signals::SignalKeyType s_CAMERA_RANGE_UPDATED_SIG;
    typedef core::com::Signal<void ()> CameraUpdatedSignalType;

    typedef core::com::Slot< void (viz::ogre::IWindowInteractor::InteractionInfo) >
        InteractionSlotType;
    ...
    VIZ_OGRE_API static const core::com::Slots::SlotKeyType s_INTERACTION_SLOT;

    /// Contains the slot name taht request the reset of camera.
    VIZ_OGRE_API static const core::com::Slots::SlotKeyType s_RESET_CAMERA_SLOT;

    /// @deprecated please used s_DEFAULT_CAMERA_NAME.
    [[deprecated("Removed in sight 22.0.")]]
    VIZ_OGRE_API static const std::string DEFAULT_CAMERA_NAME;

    /// @deprecated please used s_DEFAULT_LIGHT_NAME
    [[deprecated("Removed in sight 22.0.")]]
    VIZ_OGRE_API static const std::string DEFAULT_LIGHT_NAME;

    /// Defines the default camera name.
    VIZ_OGRE_API static const std::string s_DEFAULT_CAMERA_NAME;

    /// Defines the default light name.
    VIZ_OGRE_API static const std::string s_DEFAULT_LIGHT_NAME;

    /// Defines the default camera node name.
    VIZ_OGRE_API static const std::string s_DEFAULT_CAMERA_NODE_NAME;

    /// Initializes signals and slots.
    VIZ_OGRE_API Layer();

    /// Destoyres Ogre resources.
    VIZ_OGRE_API virtual ~Layer();

    /// Sets the render window containing this layer.
    VIZ_OGRE_API void setRenderTarget(::Ogre::RenderTarget* _renderTarget);

    /// Set the associated scene manager ID of this viewport
    VIZ_OGRE_API void setID(const std::string& id);

    /// @returns the name of this layer.
    VIZ_OGRE_API const std::string getName() const;

    /// @returns the ID of this layer.
    VIZ_OGRE_API const std::string& getLayerID() const;

    /// @returns the scene manager associated to this viewport.
    VIZ_OGRE_API ::Ogre::SceneManager* getSceneManager() const;

    /// Creates the scene.
    VIZ_OGRE_API void createScene();

    /// Destroys the scene.
    VIZ_OGRE_API void destroyScene();

    /// @returns true if the scene is created.
    VIZ_OGRE_API bool isSceneCreated() const;

    /// Adds a disabled compositor name to the ChainManager.
    VIZ_OGRE_API void addAvailableCompositor(std::string compositorName);

    /// Enables/Disables a compositor according to the isEnabled flag.
    VIZ_OGRE_API void updateCompositorState(std::string compositorName, bool isEnabled);

    /// Places and align camera's focal with the world boundingBox.
    VIZ_OGRE_API void resetCameraCoordinates();

    /// Computes camera's focal with the world boundingBox.
    VIZ_OGRE_API void computeCameraParameters();

    /// Resets the camera clipping range (near and far).
    VIZ_OGRE_API void resetCameraClippingRange() const;

    /// Resets the camera clipping range (near and far).
    VIZ_OGRE_API void resetCameraClippingRange(const ::Ogre::AxisAlignedBox& worldCoordBoundingBox) const;

    /// Appends a new interactor with the given priority. Interactors with higher priorities are executed first.
    VIZ_OGRE_API void addInteractor(const viz::ogre::interactor::IInteractor::sptr& _interactor,
                                    int _priority = 0);

    /// Removes the given interactor.
    VIZ_OGRE_API void removeInteractor(const viz::ogre::interactor::IInteractor::sptr& _interactor);

    /// @return the order of the layer.
    VIZ_OGRE_API int getOrder() const;

    /// Sets the order of the layer.
    VIZ_OGRE_API void setOrder(int _order);

    /// Sets the worker used by slots.
    VIZ_OGRE_API void setWorker( const core::thread::Worker::sptr& _worker);

    /// @returns the render service.
    VIZ_OGRE_API SPTR(viz::ogre::SRender) getRenderService() const;

    /// Sets the render service.
    VIZ_OGRE_API void setRenderService( const SPTR(viz::ogre::SRender)& _service );

    /// Requests render.
    VIZ_OGRE_API void requestRender();

    /// Sets stereoscopic rendering.
    VIZ_OGRE_API void setStereoMode(compositor::Core::StereoModeType mode);

    /// Sets background color : specific to background Layer.
    VIZ_OGRE_API void setBackgroundColor(std::string topColor, std::string botColor);

    /// Sets background scale : specific to background Layer.
    VIZ_OGRE_API void setBackgroundScale(float topScale, float botScale);

    /// Sets if this layer need a layer's 3D scene.
    VIZ_OGRE_API void setCoreCompositorEnabled(bool enabled, std::string transparencyTechnique = "",
                                               std::string numPeels                        = "",
    ...
    VIZ_OGRE_API void setCompositorChainEnabled(const std::string& compositorChain);

    /// Sets the viewport parameters for this layer: left, top, width, height.
    VIZ_OGRE_API void setViewportConfig(const ViewportConfigType& _vpCfg);

    /// @returns true if this layer needs a layer's 3D scene.
    VIZ_OGRE_API bool isCoreCompositorEnabled();

    /// @returns true if there is an XML configured compositor chain.
    VIZ_OGRE_API bool isCompositorChainEnabled();

    /// @returns true if stereoscopic rendering is enabled.
    VIZ_OGRE_API bool is3D() const;

    /// @returns the stereoscopic mode.
    VIZ_OGRE_API compositor::Core::StereoModeType getStereoMode() const;
    ...
    VIZ_OGRE_API ::Ogre::Camera* getDefaultCamera() const;

    /// Gets the projection matrix used to define nth viewpoint. The index must be lower than the number of viewpoints.
    VIZ_OGRE_API ::Ogre::Matrix4 getCameraProjMat(const std::uint8_t cameraIdx) const;

    /// @returns the number of cameras (viewpoints) used by this layer. Defined by the stereo mode.
    VIZ_OGRE_API std::uint8_t getNumberOfCameras() const;

    /// Sets default light flag.
    VIZ_OGRE_API void setHasDefaultLight(bool hasDefaultLight);

    /// @returns the number of lights adaptors used in this layer.
    VIZ_OGRE_API int getLightsNumber() const;

    /// @returns the light adaptors used in this layer.
    VIZ_OGRE_API std::vector<SPTR(viz::ogre::ILight)> getLightAdaptors() const;

    /// @returns the computed bounding box of the scene.
    VIZ_OGRE_API ::Ogre::AxisAlignedBox computeWorldBoundingBox() const;

    /// @returns the OIT selected.
    VIZ_OGRE_API compositor::transparencyTechnique getTransparencyTechnique();

    /// @returns the number of peels computed by Depth Peeling or x2 Dual Depth Peeling.
    VIZ_OGRE_API int getTransparencyDepth();

    /// Sets the OIT desired. Deactivate OIT compositor.
    VIZ_OGRE_API bool setTransparencyTechnique(compositor::transparencyTechnique technique);

    /// Sets the number of peels computed by Depth Peeling or x2 Dual Depth Peeling. Deactivate OIT compositor.
    VIZ_OGRE_API void setTransparencyDepth(int depth);

    /// Sets the camera calibrations for stereo rendering.
    VIZ_OGRE_API void setCameraCalibrations(const CameraCalibrationsType& calibrations);

    /// @returns true if a specified light is the default light in the layer.
    VIZ_OGRE_API bool isDefaultLight(const CSPTR(viz::ogre::ILight)&) const;

    /// Removes the default light in the layer.
    VIZ_OGRE_API void removeDefaultLight();

    /// @returns or create the overlay panel in which adaptors can render 2D text.
    VIZ_OGRE_API ::Ogre::OverlayContainer* getOverlayTextPanel();

    /// Defines the overlay scripts to enable on this layer's viewport.
    VIZ_OGRE_API void setEnabledOverlays(const std::vector<std::string>& _overlayScripts);

    /// @returns the overlays enabled on this layer's viewport.
    VIZ_OGRE_API const OverlaySetType& getEnabledOverlays() const;

    /// Cancels interaction for all interactors with a lower priority than the one calling this.
    VIZ_OGRE_API void cancelFurtherInteraction();

private:

    /// Slot: interacts with the scene.
    void interaction(viz::ogre::IWindowInteractor::InteractionInfo);
    ...
    SPTR(viz::ogre::compositor::Core) m_coreCompositor { nullptr };

    /// Contains the Ogre default compositor default transparency technique.
    viz::ogre::compositor::transparencyTechnique m_transparencyTechnique { viz::ogre::compositor::DEFAULT };
    ...
    viz::ogre::compositor::ChainManager::uptr m_compositorChainManager;
    ...
    std::multimap< int, viz::ogre::interactor::IInteractor::wptr, std::greater<int> > m_interactors;
    ...
    WPTR(viz::ogre::SRender) m_renderService;
    ...
    SPTR(viz::ogre::ILight) m_lightAdaptor { nullptr };
