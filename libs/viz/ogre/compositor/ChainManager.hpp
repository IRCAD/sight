...
public:
    ...
    VIZ_OGRE_API ChainManager(::Ogre::Viewport* viewport);
    VIZ_OGRE_API virtual ~ChainManager();

    /// Inserts the new compositor in the compositor chain vector
    VIZ_OGRE_API void addAvailableCompositor(CompositorIdType _compositorName);
    /// Clears the compositor chain
    VIZ_OGRE_API void clearCompositorChain(const std::string& _layerId,
                                           SPTR(viz::ogre::SRender) _renderService);
    /// Enables or disables the target compositor
    VIZ_OGRE_API void updateCompositorState(CompositorIdType _compositorName, bool _isEnabled,
                                            const std::string& _layerId,
                                            SPTR(viz::ogre::SRender) _renderService);

    VIZ_OGRE_API void setCompositorChain(const std::vector<CompositorIdType>&_compositors,
                                         const std::string& _layerId, SPTR(viz::ogre::SRender) _renderService);

    VIZ_OGRE_API CompositorChainType getCompositorChain();

    VIZ_OGRE_API void setOgreViewport(::Ogre::Viewport* _viewport);
    ...
    VIZ_OGRE_API static const CompositorIdType FINAL_CHAIN_COMPOSITOR;

    /// Class used to find a compositor by its name in the compositor chain
    class VIZ_OGRE_CLASS_API FindCompositorByName
    {
    public:
