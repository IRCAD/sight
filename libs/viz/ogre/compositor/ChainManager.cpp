...
#include "viz/ogre/compositor/SaoListener.hpp"
...
void ChainManager::clearCompositorChain(const std::string& /*_layerId*/,
                                        viz::ogre::SRender::sptr /*_renderService*/)
{
    ::Ogre::CompositorManager& compositorManager = ::Ogre::CompositorManager::getSingleton();
...
void ChainManager::updateCompositorState(CompositorIdType _compositorName, bool _isEnabled,
                                         const std::string& _layerId, viz::ogre::SRender::sptr _renderService)
{
    ::Ogre::CompositorManager& compositorManager = ::Ogre::CompositorManager::getSingleton();
                                            ...
                                            const std::string& _layerId, viz::ogre::SRender::sptr _renderService)
{
    ...
    for(const auto targetPass : targetPasses)
    {
        ...
        for(const auto pass : passes)
        {
            ...
            if(material)
            {
                const auto constants = viz::ogre::helper::Shading::findMaterialConstants(*material);
                for(const auto& constant : constants)
                {
                    ...
                    if(_isEnabled && this->getRegisteredService(id) == nullptr)
                    {
                        ...
                        auto obj = viz::ogre::helper::Shading::createObjectFromShaderParameter(constantType,
                                                                                               constantValue);
                        ...
                        if(obj != nullptr)
                        {
                            ...
                            auto srv = this->registerService(
                                "::sight::modules::viz::ogre::adaptor::SCompositorParameter", id);
