 ...
 */
class VIZ_OGRE_CLASS_API Core   //TODO : Manage occlusion query
...
public:
    ...
    VIZ_OGRE_API Core(::Ogre::Viewport* viewport);

    /// Destructor of default compositor
    VIZ_OGRE_API ~Core();

    /// Return the OIT selected
    VIZ_OGRE_API transparencyTechnique getTransparencyTechnique();
    ...
    VIZ_OGRE_API bool setTransparencyTechnique(transparencyTechnique technique);
    ...
    VIZ_OGRE_API void setStereoMode(StereoModeType stereoMode);

    /// Return the enabled stereo mode.
    VIZ_OGRE_API StereoModeType getStereoMode() const;

    /// Re/Activate OIT compositor
    VIZ_OGRE_API void update();
...
private:
    ...
    VIZ_OGRE_API void setTransparencyDepthOfDepthPeeling(int depth);

    /// Set number of ping pong peels for Dual Depth Peeling compositor
    VIZ_OGRE_API void setTransparencyDepthOfDualDepthPeeling(int depth);
    ...
    VIZ_OGRE_API void setTransparencyDepthOfHybridTransparency(int depth);

    /// Setup Default compositor (without OIT)
    VIZ_OGRE_API void setupDefaultTransparency();

    /// Setup OIT current compositor
    VIZ_OGRE_API void setupTransparency();

    //VIZ_OGRE_API void setupQueries();

    //VIZ_OGRE_API virtual bool frameRenderingQueued(const Ogre::FrameEvent& evt);
