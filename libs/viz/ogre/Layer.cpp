...
#include "viz/ogre/factory/Text.hpp"
...
struct Layer::LayerCameraListener : public ::Ogre::Camera::Listener
{
    ...
    void cameraPreRenderScene(::Ogre::Camera*) final
    {
        ...
        if(m_layer->getStereoMode() != viz::ogre::compositor::Core::StereoModeType::NONE)
        {
...
{
    namespace fwc = viz::ogre::compositor;

    auto renderService = m_renderService.lock();

    auto root = viz::ogre::Utils::getOgreRoot();
    m_sceneManager = root->createSceneManager("DefaultSceneManager", renderService->getID() + "_" + m_id);
    m_sceneManager->addRenderQueueListener( viz::ogre::Utils::getOverlaySystem() );
    ...
    if(m_hasDefaultLight)
    {
        ...
        m_lightAdaptor = viz::ogre::ILight::createLightAdaptor(m_defaultLightDiffuseColor,
...
void Layer::destroyScene()
{
    if (m_order == 0)
    {
        ...
        ::Ogre::MaterialManager::getSingleton().remove(
            this->getName() + "backgroundMat", viz::ogre::RESOURCE_GROUP);
    }
    if(m_lightAdaptor)
    {
        viz::ogre::ILight::destroyLightAdaptor(m_lightAdaptor);
    ...
    if(m_sceneManager)
    {
        viz::ogre::Utils::getOgreRoot()->destroySceneManager(m_sceneManager);
...
void Layer::interaction(viz::ogre::IWindowInteractor::InteractionInfo info)
{
    this->getRenderService()->makeCurrent();

    switch(info.interactionType)
    {
        case viz::ogre::IWindowInteractor::InteractionInfo::MOUSEMOVE:
        {
        ...
        case viz::ogre::IWindowInteractor::InteractionInfo::WHEELMOVE:
        {
            this->forAllInteractors([&info](const interactor::IInteractor::sptr& _i)
                {
        ...
        case viz::ogre::IWindowInteractor::InteractionInfo::RESIZE:
        {
            ...
            if(m_dpi != newDpi)
            {
                ...
                for(auto& [name, movObj] : movableTextObjects)
                {
                    ...
                    SLM_ASSERT("Movable object should be of type '::viz::ogre::Text'", textObj);
        ...
        case viz::ogre::IWindowInteractor::InteractionInfo::KEYPRESS:
        {
            this->forAllInteractors([&info](const interactor::IInteractor::sptr& _i)
                {
        ...
        case viz::ogre::IWindowInteractor::InteractionInfo::KEYRELEASE:
        {
            this->forAllInteractors([&info](const interactor::IInteractor::sptr& _i)
                {
        ...
        case viz::ogre::IWindowInteractor::InteractionInfo::BUTTONRELEASE:
        {
            this->forAllInteractors([&info](const interactor::IInteractor::sptr& _i)
                {
        ...
        case viz::ogre::IWindowInteractor::InteractionInfo::BUTTONPRESS:
        {
            this->forAllInteractors([&info](const interactor::IInteractor::sptr& _i)
                {
        ...
        case viz::ogre::IWindowInteractor::InteractionInfo::BUTTONDOUBLEPRESS:
        {
            this->forAllInteractors([&info](const interactor::IInteractor::sptr& _i)
                {
...
viz::ogre::SRender::sptr Layer::getRenderService() const
{
...
void Layer::setRenderService( const viz::ogre::SRender::sptr& _service)
{
...
void Layer::addInteractor(const viz::ogre::interactor::IInteractor::sptr& _interactor, int _priority)
{
...
void Layer::removeInteractor(const viz::ogre::interactor::IInteractor::sptr& _interactor)
{
...
bool Layer::isDefaultLight(const viz::ogre::ILight::csptr& _light) const
{
...
void Layer::resetCameraClippingRange(const ::Ogre::AxisAlignedBox& worldCoordBoundingBox) const
{
    if(m_camera && m_camera->getProjectionType() == ::Ogre::PT_PERSPECTIVE)
    {
        ...
        const auto saoCompositorIt = std::find_if(chain.begin(), chain.end(),
                                                  viz::ogre::compositor::ChainManager::FindCompositorByName("SAO"));
                                     ...
                                     compositor::Core::StereoModeType stereoMode)
{
    ...
    if(transparencyTechnique != "")
    {
        ...
        {
            m_transparencyTechnique = viz::ogre::compositor::DEPTHPEELING;
        }
        else if(transparencyTechnique == "CelShadingDepthPeeling")
        {
            m_transparencyTechnique = viz::ogre::compositor::CELSHADING_DEPTHPEELING;
        }
        else if(transparencyTechnique == "DualDepthPeeling")
        {
            m_transparencyTechnique = viz::ogre::compositor::DUALDEPTHPEELING;
        }
        else if(transparencyTechnique == "WeightedBlended")
        {
            m_transparencyTechnique = viz::ogre::compositor::WEIGHTEDBLENDEDOIT;
        }
        else if(transparencyTechnique == "HybridTransparency")
        {
            m_transparencyTechnique = viz::ogre::compositor::HYBRIDTRANSPARENCY;
        }
        else
        {
...
void Layer::setupCore()
{
    ...
    m_coreCompositor = std::make_shared< viz::ogre::compositor::Core>(m_viewport);
...
viz::ogre::compositor::ChainManager::CompositorChainType Layer::getCompositorChain() const
{
...
Ogre::Matrix4 Layer::getCameraProjMat(const uint8_t cameraIdx) const
{
    ...
    if(m_stereoMode == viz::ogre::compositor::Core::StereoModeType::AUTOSTEREO_5)
    {
        const float eyeAngle = 0.02321f;
        const float angle    = eyeAngle * (-2.f + float(cameraIdx));

        extrinsicTransform = viz::ogre::helper::Camera::computeFrustumShearTransform(*m_camera, angle);
    }
    else if(m_stereoMode == viz::ogre::compositor::Core::StereoModeType::AUTOSTEREO_8)
    {
    ...
    else if(m_stereoMode == viz::ogre::compositor::Core::StereoModeType::STEREO)
    {
...
{
    return m_stereoMode == viz::ogre::compositor::Core::StereoModeType::AUTOSTEREO_8 ? 8 :
           m_stereoMode == viz::ogre::compositor::Core::StereoModeType::AUTOSTEREO_5 ? 5 :
           m_stereoMode == viz::ogre::compositor::Core::StereoModeType::STEREO       ? 2 : 1;
}
...
{
    auto lightAdaptors = this->getRenderService()->getAdaptors< viz::ogre::ILight >();
    int lightsNumber(0);

    std::for_each(lightAdaptors.begin(), lightAdaptors.end(), [&](viz::ogre::ILight::sptr adaptor)
        {
...
std::vector< viz::ogre::ILight::sptr > Layer::getLightAdaptors() const
{
    auto lightAdaptors = this->getRenderService()->getAdaptors< viz::ogre::ILight >();
    std::vector< viz::ogre::ILight::sptr > layerLightAdaptors;

    std::for_each(lightAdaptors.begin(), lightAdaptors.end(), [&](viz::ogre::ILight::sptr lightAdaptor)
        {
