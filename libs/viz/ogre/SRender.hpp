...
#include "viz/ogre/overlay/ViewportListener.hpp"
 ...
 * @code{.xml}
    <service uid="..." type="::sight::viz::ogre::SRender" autoconnect="yes">
 ...
 */
class VIZ_OGRE_CLASS_API SRender final : public viz::base::IRender
{
...
public:
    ...
    typedef std::map< SceneIdType, SPTR(viz::ogre::Layer) > LayerMapType;

    /// Contains the signal sent when the compositor chain has been modified.
    VIZ_OGRE_API static const core::com::Signals::SignalKeyType s_COMPOSITOR_UPDATED_SIG;
    typedef core::com::Signal<void (std::string, bool, viz::ogre::Layer::sptr)> CompositorUpdatedSignalType;

    /// Contains the signal sent when fullscreen was enabled/disabled.
    VIZ_OGRE_API static const core::com::Signals::SignalKeyType s_FULLSCREEN_SET_SIG;
    ...
    VIZ_OGRE_API static const core::com::Slots::SlotKeyType s_COMPUTE_CAMERA_ORIG_SLOT;

    /// Contains the slot name that resets all layers camera.
    VIZ_OGRE_API static const core::com::Slots::SlotKeyType s_RESET_CAMERAS_SLOT;

    /// Contains the slot name that computes the parameters to reset the camera.
    VIZ_OGRE_API static const core::com::Slots::SlotKeyType s_COMPUTE_CAMERA_CLIPPING_SLOT;

    /// Contains the slot name that request the picker to do a ray cast according to the passed position.
    VIZ_OGRE_API static const core::com::Slots::SlotKeyType s_DO_RAY_CAST_SLOT;

    /// Contains the slot name that requests a rendering.
    VIZ_OGRE_API static const core::com::Slots::SlotKeyType s_REQUEST_RENDER_SLOT;

    /// Contains the slot name that disables fullscreen rendering if it was enabled.
    VIZ_OGRE_API static const core::com::Slots::SlotKeyType s_DISABLE_FULLSCREEN;

    /// Contains the slot name that enables fullscreen rendering on a specific screen.
    VIZ_OGRE_API static const core::com::Slots::SlotKeyType s_ENABLE_FULLSCREEN;

    /// Defines the layer ID of the background.
    VIZ_OGRE_API static const std::string s_OGREBACKGROUNDID;

    /// Initialiazes slots.
    VIZ_OGRE_API SRender() noexcept;

    /// Destroys the service.
    VIZ_OGRE_API ~SRender() noexcept override;

    /// Sets this render service as the current OpenGL context.
    VIZ_OGRE_API void makeCurrent();

    /// Requests a render from the Ogre render engine.
    VIZ_OGRE_API void requestRender();

    /// @returns true if the scene is shown on screen.
    VIZ_OGRE_API bool isShownOnScreen();

    /// @returns the scene manager corresponding to the sceneID.
    VIZ_OGRE_API ::Ogre::SceneManager* getSceneManager(const ::std::string& sceneID);

    /// @returns the layer corresponding to the sceneID.
    VIZ_OGRE_API viz::ogre::Layer::sptr getLayer(const ::std::string& sceneID);

    /// @returns this render layers.
    VIZ_OGRE_API LayerMapType getLayers();

    /// @returns m_interactorManager.
    VIZ_OGRE_API viz::ogre::IWindowInteractor::sptr getInteractorManager() const;

    /// Resets camera parameters with the actual global bounding box.
    VIZ_OGRE_API void resetCameraCoordinates(const std::string& _layerId);

    /// Resets all layers camera parameters with the actual global bounding box.
    VIZ_OGRE_API void resetCameras();

    /// Computes camera parameters with the actual global bounding box.
    VIZ_OGRE_API void computeCameraClipping();
...
protected:
    ...
    VIZ_OGRE_API void starting() override;

    /// Stops all adaptors
    VIZ_OGRE_API void stopping() override;

    /// Configures the adaptor.
    VIZ_OGRE_API virtual void configuring() override;

    /// Does nothing.
    VIZ_OGRE_API void updating() override;
...
private:
    ...
    viz::ogre::IWindowInteractor::sptr m_interactorManager;
...
{
    auto servicesVector = services::OSR::getServices("::viz::ogre::IAdaptor");
