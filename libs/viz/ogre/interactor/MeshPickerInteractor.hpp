 ...
 */
class VIZ_OGRE_CLASS_API MeshPickerInteractor final : public ::sight::viz::ogre::interactor::IInteractor
{
...
public:
    ...
    VIZ_OGRE_API MeshPickerInteractor(SPTR(Layer)_layer = nullptr, bool _layerOrderDependant = true) noexcept;

    /// Destroys the picker.
    VIZ_OGRE_API ~MeshPickerInteractor() noexcept override;

    /// Runs a picking query when a mouse button is released @see MeshPickerInteractor::pick().
    VIZ_OGRE_API void buttonReleaseEvent(MouseButton _button, Modifier _mods, int _x, int _y) override;

    /// Runs a picking query when a mouse button is pressed @see MeshPickerInteractor::pick().
    VIZ_OGRE_API void buttonPressEvent(MouseButton _button, Modifier _mods, int _x, int _y) override;

    /// Sets the signal to be called when picking succeeded.
    VIZ_OGRE_API void setPointClickedSig(const PointClickedSigType::sptr& _sig);

    /// Sets the mask to filter out entities with mismathing query flags.
    VIZ_OGRE_API void setQueryMask(std::uint32_t _queryMask);
