...
#include "viz/ogre/widget/ClippingBox.hpp"
...
class VIZ_OGRE_CLASS_API ClippingBoxInteractor final : public IInteractor
{

public:

    /// Creates the widget.
    VIZ_OGRE_API ClippingBoxInteractor(SPTR(Layer)_layer,
    ...
    /// Destroys the widget.
    VIZ_OGRE_API ~ClippingBoxInteractor() noexcept override;

    /// Interacts with the widget if it was previously picked, behaves like a trackball otherwise.
    VIZ_OGRE_API void mouseMoveEvent(MouseButton, Modifier, int, int, int, int) override;

    /// Ends all interactions with the widget.
    VIZ_OGRE_API void buttonReleaseEvent(MouseButton, Modifier, int, int) override;

    /// Picks the object at the (x,y) position on a left click, scales or translates the widget otherwise.
    VIZ_OGRE_API void buttonPressEvent(MouseButton, Modifier, int, int) override;

    /// Sets the visibility of the box widget.
    VIZ_OGRE_API void setBoxVisibility(bool _visibility);

    /// Retrieves the positions of the box in model space.
    VIZ_OGRE_API ::Ogre::AxisAlignedBox getClippingBox() const;

    /// Retrieves the current clipping transform.
    VIZ_OGRE_API ::Ogre::Matrix4 getClippingTransform() const;

    /// Sets the current clipping box positions from the input transform.
    VIZ_OGRE_API void updateFromTransform(const ::Ogre::Matrix4& _clippingTrf);
...
private:
    ...
    viz::ogre::picker::IPicker m_picker;
