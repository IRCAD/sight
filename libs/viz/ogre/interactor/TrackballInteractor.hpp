...
namespace sight::viz::ogre::interactor
{
 ...
 */
class VIZ_OGRE_CLASS_API TrackballInteractor final : public ::sight::viz::ogre::interactor::IInteractor
{

public:

    /// Initializes the trackball.
    VIZ_OGRE_API TrackballInteractor(SPTR(Layer)_layer = nullptr, bool _layerOrderDependant = true);

    /// Destroys the trackball.
    VIZ_OGRE_API ~TrackballInteractor() override;
     ...
     */
    VIZ_OGRE_API void mouseMoveEvent(MouseButton _button, Modifier, int _x, int _y,
    ...
    VIZ_OGRE_API void buttonPressEvent(MouseButton _button, Modifier, int _x, int _y) override;

    /// Disables mouse movements.
    VIZ_OGRE_API void buttonReleaseEvent(MouseButton _button, Modifier, int, int) override;
     ...
     */
    VIZ_OGRE_API void wheelEvent(Modifier, int _delta, int, int) override;
     ...
     */
    VIZ_OGRE_API void keyPressEvent(int _key, Modifier, int _mouseX, int _mouseY) override;

    /**
     * @brief Recomputes the camera's aspect ratio when the render window is resized.
     */
    VIZ_OGRE_API void resizeEvent(int, int) override;

    /// Recomputes the mouse's scale and focus point from the updated scene length.
    VIZ_OGRE_API void setSceneLength(float _sceneLength) override;
