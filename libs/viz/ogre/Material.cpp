...
void Material::updateOptionsMode(int _optionsMode)
{
    ...
    if(_optionsMode != data::Material::STANDARD)
    {
        for(const auto currentTechnique : techniques)
        {
            ...
            const bool depthOnly = viz::ogre::helper::Shading::isDepthOnlyTechnique(*currentTechnique);

            if( viz::ogre::helper::Shading::isGeometricTechnique(*currentTechnique) || depthOnly )
            {
                ...
                *normalsPass              = *firstPass;
                ...
                if(!depthOnly)
                {
                    ...
                    fpName = viz::ogre::helper::Shading::setPermutationInProgramName(fpName, "Edge_Normal");
...
void Material::updateShadingMode( int _shadingMode, int _numLights, bool _hasDiffuseTexture, bool _useTextureAlpha )
{
    ...
    const ::Ogre::String permutation = viz::ogre::helper::Shading::getPermutation(mode, needDiffuseTexture,
                                                                                  m_hasVertexColor);
    const ::Ogre::String r2vbGSName = viz::ogre::helper::Shading::getR2VBGeometryProgramName(m_primitiveType,
                                                                                             needDiffuseTexture,
                                                                                             m_hasVertexColor,
                                                                                             m_hasPrimitiveColor);
        ...
        for(const auto ogrePass : passes)
        {
            ...
            else
            {
                if(m_templateName == DEFAULT_MATERIAL_TEMPLATE_NAME)
                {
                    ...
                    vpName = viz::ogre::helper::Shading::setPermutationInProgramName(vpName, permutation);
                    ogrePass->setVertexProgram(vpName);

                    std::string fpName = ogrePass->getFragmentProgramName();
                    fpName = viz::ogre::helper::Shading::setPermutationInProgramName(fpName, permutation);
                    ogrePass->setFragmentProgram(fpName);

                    const bool colorPass = viz::ogre::helper::Shading::isColorTechnique(*tech);
...
void Material::setDiffuseTexture(const ::Ogre::TexturePtr& _texture)
{
    ...
    for(const auto technique : techniques)
    {
        ...
        if(viz::ogre::helper::Shading::isColorTechnique(*technique))
        {
