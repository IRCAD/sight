...
public:
    ...
    /// Constructor
    VIZ_OGRE_API Mesh(const std::string& _name);
    ...
    VIZ_OGRE_API void setDynamicVertices(bool _isDynamic);
    /// Set meshes and indices buffers to dynamic state (only has effect if called before service starting/update)
    VIZ_OGRE_API void setDynamic(bool _isDynamic);

    VIZ_OGRE_API void setVisible(bool _visible);
    VIZ_OGRE_API void updateMesh(const data::Mesh::sptr& _mesh, bool _pointsOnly = false);
    VIZ_OGRE_API void updateMesh(const data::PointList::csptr& _pointList);
    VIZ_OGRE_API std::pair<bool, std::vector<R2VBRenderable*> > updateR2VB(const data::Mesh::sptr& _mesh,
                                                                           ::Ogre::SceneManager& _sceneMgr,
                                                                           const std::string& _materialName,
                                                                           bool _hasTexture);

    /// Updates the vertices position
    VIZ_OGRE_API void updateVertices(const data::Mesh::csptr& _mesh);
    /// Updates the vertices position
    VIZ_OGRE_API void updateVertices(const data::PointList::csptr& mesh);
    /// Updates the vertices colors.
    VIZ_OGRE_API void updateColors(const data::Mesh::csptr& _mesh);
    /// Updates the vertices texture coordinates.
    VIZ_OGRE_API void updateTexCoords(const data::Mesh::csptr& _mesh);
    /// Erase the mesh data, called when the configuration change (new layer, etc...), to simplify modifications.
    VIZ_OGRE_API void clearMesh(::Ogre::SceneManager& _sceneMgr);

    VIZ_OGRE_API void updateMaterial(viz::ogre::Material* _material, bool _isR2VB) const;

    VIZ_OGRE_API bool hasColorLayerChanged(const data::Mesh::csptr& _mesh);

    VIZ_OGRE_API ::Ogre::Entity* createEntity(::Ogre::SceneManager& _sceneMgr);

    VIZ_OGRE_API void invalidateR2VB();
...
private:
    ...
    std::map< data::Mesh::CellType, viz::ogre::R2VBRenderable*> m_r2vbObject;
