 ...
 */
class VIZ_OGRE_CLASS_API R2VBRenderable : public ::Ogre::SimpleRenderable
{
public:
    [[deprecated("Will be removed in sight 22.0")]]
    VIZ_OGRE_API static R2VBRenderable* New(const std::string& _name, ::Ogre::SubEntity* _sourceObject,
    ...
    VIZ_OGRE_API R2VBRenderable(const ::Ogre::String& name);
    virtual VIZ_OGRE_API ~R2VBRenderable();
    ...
    VIZ_OGRE_API void setOutputSettings(size_t _vertexCount, bool _hasColor, bool _hasTexCoord,
    ...
    VIZ_OGRE_API virtual const ::Ogre::String& getMovableType(void) const override;

    /** @copydoc SimpleRenderable::getRenderOperation. */
    VIZ_OGRE_API virtual void getRenderOperation(::Ogre::RenderOperation& _op) override;

    /// Delegate to the subentity.
    VIZ_OGRE_API virtual ::Ogre::Real getBoundingRadius(void) const override;

    /// @copydoc Renderable::getSquaredViewDepth
    VIZ_OGRE_API virtual ::Ogre::Real getSquaredViewDepth(const Ogre::Camera* _cam) const override;

    /// Return the parent mesh.
    VIZ_OGRE_API virtual const ::Ogre::MeshPtr& getMesh(void) const;

    /// Mark the output verex buffer as dirty, the r2vb process will be run on next update
    VIZ_OGRE_API void setDirty();

    /// Runs the R2VB process.
    VIZ_OGRE_API void manualUpdate();

    /// Set the material used to process the geometry pass.
    VIZ_OGRE_API void setRenderToBufferMaterial(const std::string& _mtlName);

    viz::ogre::IAdaptor::wptr m_materialAdaptor;
