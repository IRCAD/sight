...
Mesh::Mesh(const std::string& _name)
{
    ...
    m_ogreMesh = meshMgr.createManual(meshName, viz::ogre::RESOURCE_GROUP);
        ...
        *m_r2vbMesh->sharedVertexData->vertexDeclaration   = *m_ogreMesh->sharedVertexData->vertexDeclaration;
        ...
        for(size_t i = 0; i < numSubEntities; ++i)
        {
            ...
            if(m_r2vbObject.find(cellType) == m_r2vbObject.end())
            {
                ...
                m_r2vbObject[cellType] = viz::ogre::R2VBRenderable::New(r2vbObjectName, subEntity, &_sceneMgr,
...
void Mesh::updateColors(const data::Mesh::csptr& _mesh)
{
    ...
    if(hasPrimitiveColor)
    {
        ...
        if(!m_perPrimitiveColorTexture)
        {
            ...
            m_perPrimitiveColorTexture     = ::Ogre::TextureManager::getSingleton().create(
                m_perPrimitiveColorTextureName, viz::ogre::RESOURCE_GROUP, true);
        }
        ...
        if(m_perPrimitiveColorTexture->getWidth() != width || m_perPrimitiveColorTexture->getHeight() != height )
        {
            ...
            viz::ogre::Utils::allocateTexture(m_perPrimitiveColorTexture.get(), width, height, 1,
    ...
    if (hasVertexColor)
    {
        ...
        viz::ogre::helper::Mesh::copyColors(pColor, colors, _mesh->getNumberOfPoints(), nbComponents);
    ...
    if(hasPrimitiveColor)
    {
        ...
        viz::ogre::helper::Mesh::copyColors(pColorDest, colors, _mesh->getNumberOfCells(), nbComponents);
