 ...
 */
class VIZ_OGRE_CLASS_API TransferFunction : public ::boost::noncopyable
{
public:
    typedef std::shared_ptr<TransferFunction> sptr;
    typedef std::weak_ptr<TransferFunction> wptr;

    VIZ_OGRE_API TransferFunction();
    VIZ_OGRE_API virtual ~TransferFunction();

    /// Initialize the textures attributes.
    VIZ_OGRE_API void createTexture(const ::Ogre::String& _parentId);

    /// Destroy the texture.
    VIZ_OGRE_API void removeTexture();

    /// Update the TF texture according to the transfer function data.
    VIZ_OGRE_API void updateTexture(const data::TransferFunction::csptr& _tf);

    /// Return the TF texture.
    VIZ_OGRE_API ::Ogre::TexturePtr getTexture() const;

    /// Set the sample distance.
    VIZ_OGRE_API void setSampleDistance(const float& _sampleDistance);
