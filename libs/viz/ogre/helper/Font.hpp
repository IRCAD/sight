...
public:
     ...
     */
    static VIZ_OGRE_API ::Ogre::FontPtr getFont(const std::string& _trueTypeFileName, const size_t _size,
                                                const std::uint32_t _trueTypeResolution = 220);

    /// Creates or retrieves the material used to render the specified font.
    static VIZ_OGRE_API ::Ogre::MaterialPtr getFontMtl(const std::string& _fontName);

private:

    /// Returns the font's texture map.
    static VIZ_OGRE_API ::Ogre::TexturePtr getFontMap(const std::string& _fontName);
