...
fwServicesRegisterMacro( ::sight::viz::base::IRender, ::sight::viz::ogre::SRender, ::sight::data::Composite )
...
{
    m_ogreRoot = viz::ogre::Utils::getOgreRoot();
...
void SRender::starting()
{
    ...
    if(!bHasBackground)
    {
        ...
        viz::ogre::Layer::sptr ogreLayer = viz::ogre::Layer::New();
        ogreLayer->setRenderService(viz::ogre::SRender::dynamicCast(this->shared_from_this()));
    ...
    if(m_offScreen)
    {
        ...
        m_interactorManager = viz::ogre::IWindowInteractor::createOffscreenManager(m_width, m_height);
    ...
    {
        // Instantiate the manager that help to communicate between this service and the widget
        m_interactorManager = viz::ogre::IWindowInteractor::createManager();
        m_interactorManager->setRenderService(this->getSptr());
    ...
    {
        viz::ogre::Layer::sptr layer = it.second;
...
void SRender::requestRender()
{
    ...
    if(m_offScreen)
    {
        ...
        const auto image = this->getLockedInOut< data::Image >(s_OFFSCREEN_INOUT);
        {
            ...
            viz::ogre::Utils::convertFromOgreTexture(renderTexture, image.get_shared(), m_flip);
        }
...
{
    viz::ogre::Layer::sptr layer = this->getLayer(sceneID);
    return layer->getSceneManager();
}

// ----------------------------------------------------------------------------

viz::ogre::Layer::sptr SRender::getLayer(const ::std::string& sceneID)
{
    SLM_ASSERT("Empty sceneID", !sceneID.empty());
    SLM_ASSERT("Layer ID "<< sceneID <<" does not exist", m_layers.find(sceneID) != m_layers.end());

    viz::ogre::Layer::sptr layer = m_layers.at(sceneID);
...
viz::ogre::SRender::LayerMapType SRender::getLayers()
{
    return m_layers;
}
...
viz::ogre::IWindowInteractor::sptr SRender::getInteractorManager() const
{
