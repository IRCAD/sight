 ...
 */
class VIZ_OGRE_CLASS_API Material
{
public:
    typedef std::unique_ptr<Material> uptr;

    /// Name of the default used as a base for the instance created by this adaptor
    VIZ_OGRE_API static const std::string DEFAULT_MATERIAL_TEMPLATE_NAME;

    /// Constructor
    VIZ_OGRE_API Material(const std::string& _name, const std::string& _templateName);

    /// Destructor
    VIZ_OGRE_API ~Material();

    /// Updates material options mode (standard, point normals or cells normals)
    VIZ_OGRE_API void updateOptionsMode(int _optionsMode );

    /// Updates material polygon mode (surface, point or wireframe)
    VIZ_OGRE_API void updatePolygonMode( int _polygonMode );

    /// Update material shading mode (flat/gouraud/phong) in fixed function pipeline
    VIZ_OGRE_API void updateShadingMode(int _shadingMode, int _numLights, bool _hasDiffuseTexture,
                                        bool useTextureAlpha);

    /// Update material color in fixed function pipeline
    VIZ_OGRE_API void updateRGBAMode( data::Material::sptr _sightMaterial );
    ...
    VIZ_OGRE_API void setDiffuseTexture(const ::Ogre::TexturePtr& _texture);

    /// Set the material template used
    VIZ_OGRE_API void setTemplate(const std::string& _templateName);
