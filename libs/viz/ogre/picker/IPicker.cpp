...
#include "viz/ogre/collisionTools/CollisionTools.hpp"
...
{
    const auto* const camera = m_sceneManager->getCamera(viz::ogre::Layer::s_DEFAULT_CAMERA_NAME);
    ...
    viz::ogre::CollisionTools tool = viz::ogre::CollisionTools(m_sceneManager, _queryMask);
...
::Ogre::SceneNode* IPicker::getCameraSceneNode() const
{
    ...
    return m_sceneManager->getCamera(viz::ogre::Layer::s_DEFAULT_CAMERA_NAME)->getParentSceneNode();
}
...
{
    ::Ogre::Camera* cam        = m_sceneManager->getCamera(viz::ogre::Layer::s_DEFAULT_CAMERA_NAME);
