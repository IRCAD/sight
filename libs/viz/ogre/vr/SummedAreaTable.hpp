 ...
 */
class VIZ_OGRE_CLASS_API SummedAreaTable
{
public:

    /// Constructor, creates an SAT with the given resolution.
    VIZ_OGRE_API SummedAreaTable(std::string _parentId, ::Ogre::SceneManager* _sceneManager,
                                 float _sizeRatio = 0.25f);

    /// Destructor, does nothing.
    VIZ_OGRE_API ~SummedAreaTable();

    /// Computes the SAT sequentially on the CPU based on the given image and TF.
    VIZ_OGRE_API void computeSequential(data::Image::sptr _image, data::TransferFunction::sptr _tf);

    /// Computes the SAT using Hensley's recursive doubling algorithm.
    VIZ_OGRE_API void computeParallel(::Ogre::TexturePtr _imgTexture,
                                      const viz::ogre::TransferFunction::sptr& _gpuTf,
                                      float _sampleDistance);

    /// Returns the texture holding the SAT.
    VIZ_OGRE_API ::Ogre::TexturePtr getTexture() const;

    /// Returns the texture used as a ping-pong buffer during SAT computation allowing it to be repurposed.
    VIZ_OGRE_API ::Ogre::TexturePtr getSpareTexture() const;

    /// Updates the current size of the image according to the passed texture and updates the SAT
    VIZ_OGRE_API void updateSatFromTexture(::Ogre::TexturePtr _imgTexture);

    /// Updates the SAT size ratio and updates the SAT.
    VIZ_OGRE_API void updateSatFromRatio(float _sizeRatio);
