...
public:
    ...
    VIZ_OGRE_API PreIntegrationTable();

    /// Destructor, does nothing.
    VIZ_OGRE_API ~PreIntegrationTable();

    /// Initializes the texture resource.
    VIZ_OGRE_API void createTexture(const std::string& _parentId);

    /// Destroy the texture
    VIZ_OGRE_API void removeTexture();

    /// Sets the table resolution based on the new image and recomputes it.
    VIZ_OGRE_API void imageUpdate(const data::Image::sptr& _img, const data::TransferFunction::sptr& _tf,
                                  float _samplingRate);

    /// Computes the table based on the given TF and slice distance.
    VIZ_OGRE_API void tfUpdate(const data::TransferFunction::sptr& _tf, float _sampleDistance);

    /// Get the texture holding the pre-integration table.
    VIZ_OGRE_API inline ::Ogre::TexturePtr getTexture() const;

    /// Returns the image value interval.
    VIZ_OGRE_API inline std::pair<int, int> getMinMax() const;
