...
SummedAreaTable::~SummedAreaTable()
{
    ...
    textureManager.remove(m_parentId + SOURCE_BUFFER_NAME, viz::ogre::RESOURCE_GROUP);
    textureManager.remove(m_parentId + TARGET_BUFFER_NAME, viz::ogre::RESOURCE_GROUP);
}

//-----------------------------------------------------------------------------

void SummedAreaTable::computeParallel(::Ogre::TexturePtr _imgTexture,
                                      const viz::ogre::TransferFunction::sptr& _gpuTf, float _sampleDistance)
{
    if(!m_sourceBuffer)
    {
