...
#include "viz/ogre/vr/GridProxyGeometry.hpp"
#include "viz/ogre/vr/IllumAmbientOcclusionSAT.hpp"
#include "viz/ogre/vr/IVolumeRenderer.hpp"
#include "viz/ogre/vr/RayEntryCompositor.hpp"
 ...
 */
class VIZ_OGRE_CLASS_API RayTracingVolumeRenderer : public IVolumeRenderer
{
public:
     ...
     */
    VIZ_OGRE_API RayTracingVolumeRenderer(std::string parentId,
                                          Layer::sptr layer,
    ...
    /// Does nothing.
    VIZ_OGRE_API virtual ~RayTracingVolumeRenderer();

    /// Function called when a new image is being rendered.
    VIZ_OGRE_API virtual void imageUpdate(const data::Image::sptr image,
                                          const data::TransferFunction::sptr tf) override;

    /// Called when the transfer function is updated.
    VIZ_OGRE_API virtual void updateVolumeTF() override;

    /// Sets the number of samples per view ray.
    VIZ_OGRE_API virtual void setSampling(uint16_t nbSamples) override;

    /// Sets the opacity correction factor.
    VIZ_OGRE_API void setOpacityCorrection(int opacityCorrection);

    /// Ambient occlusion factor setter.
    VIZ_OGRE_API virtual void setAOFactor(double aoFactor);

    /// Color bleeding factor setter.
    VIZ_OGRE_API virtual void setColorBleedingFactor(double colorBleedingFactor);

    /**
     * @brief Sets the ambient occlsuion and soft shadows SAT.
     * @param _ambientOcclusionSAT SAT that store importances values.
     */
    VIZ_OGRE_API virtual void setAmbientOcclusionSAT(IllumAmbientOcclusionSAT::sptr _ambientOcclusionSAT);

    /// Sets pre-integrated mode.
    VIZ_OGRE_API virtual void setPreIntegratedRendering(bool preIntegratedRendering) override;

    /// Sets ambient occlusion usage.
    VIZ_OGRE_API virtual void setAmbientOcclusion(bool ambientOcclusion);

    /// Sets color bleeding usage.
    VIZ_OGRE_API virtual void setColorBleeding(bool colorBleeding);

    /// Sets soft shadows usage.
    VIZ_OGRE_API virtual void setShadows(bool shadows);
    ...
    VIZ_OGRE_API void setFocalLength(float focalLength);

    /// Computes image positions, updates the proxy geometry.
    VIZ_OGRE_API virtual void clipImage(const ::Ogre::AxisAlignedBox& clippingBox) override;

    /// IllumVolume getter.
    VIZ_OGRE_API IllumAmbientOcclusionSAT::sptr getIllumVolume();

    /// Returns whether or not the volume is visible.
    VIZ_OGRE_API bool isVisible() const;

    /// Layer getter
    viz::ogre::Layer::sptr getLayer() const;

    /// Sets the texture holding the image to be displayed. Doesn't recompute the proxy geometry.
    VIZ_OGRE_API void set3DTexture(const ::Ogre::TexturePtr& _texture );
...
protected:
    ...
    VIZ_OGRE_API virtual std::tuple<std::string, std::string, size_t> computeRayTracingDefines() const;

    /// Sets all texture units needed by the material during the ray casting pass.
    VIZ_OGRE_API virtual void setRayCastingPassTextureUnits(Ogre::Pass* const _rayCastingPass,
     ...
     */
    VIZ_OGRE_API virtual void createRayTracingMaterial(const std::string& _sourceFile = "RayTracedVolume_FP.glsl");
...
private:
    ...
    viz::ogre::vr::GridProxyGeometry* m_proxyGeometry;
    ...
    viz::ogre::Layer::wptr m_layer;
};

//-----------------------------------------------------------------------------
// Inline method(s)

inline viz::ogre::vr::IllumAmbientOcclusionSAT::sptr RayTracingVolumeRenderer::getIllumVolume()
{
...
inline viz::ogre::Layer::sptr RayTracingVolumeRenderer::getLayer() const
{
    return m_layer.lock();
}
