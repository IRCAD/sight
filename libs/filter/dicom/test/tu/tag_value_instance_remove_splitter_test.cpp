...
#include "tag_value_instance_remove_splitter_test.hpp"
...
CPPUNIT_TEST_SUITE_REGISTRATION(sight::filter::dicom::ut::tag_value_instance_remove_splitter_test);
...
void tag_value_instance_remove_splitter_test::setUp()
{
...
void tag_value_instance_remove_splitter_test::tearDown()
{
...
void tag_value_instance_remove_splitter_test::simple_application()
{
    ...
    // Apply filter
    sight::filter::dicom::splitter::tag_value_instance_remove_splitter::sptr filter =
        std::dynamic_pointer_cast<sight::filter::dicom::splitter::tag_value_instance_remove_splitter>(
            sight::filter::dicom::factory::make("sight::filter::dicom::splitter::tag_value_instance_remove_splitter")
        );
    CPPUNIT_ASSERT(filter);
    filter->set_tag(DCM_AcquisitionNumber);
    filter->set_tag_value("1");
    ...
    CPPUNIT_ASSERT_EQUAL(std::size_t(275), dicom_series->get_dicom_container().size());
}
