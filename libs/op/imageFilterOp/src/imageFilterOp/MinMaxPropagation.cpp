...
class MinMaxPropagCriterion : public itk::ImageFunction<TImage, bool>
{
public:
    ...
    void setParams(::fwData::Image::csptr roi, const std::vector<IndexType>& seeds, double radius, bool overwrite,
                   MinMaxPropagation::Mode mode)
    {
        std::vector< PixelType > seedValues(seeds.size());
        std::transform(seeds.begin(), seeds.end(), seedValues.begin(),
    ...
    bool isInROI(const IndexType& index) const
    {
        ...
        return !::fwDataTools::fieldHelper::MedicalImageHelpers::isBufNull(roiVal, m_roi->getType().sizeOf());
    }
...
struct MinMaxPropagator
{
    ...
    void operator()(Parameters& params)
    {
        ...
        const typename ImageType::Pointer itkImage = ::fwItkIO::itkImageFactory< ImageType >(params.inputImage);
        ...
        for(; !iter.IsAtEnd(); ++iter)
        {
            ...
            const ::fwData::Image::BufferType* pixBuf =
                reinterpret_cast< ::fwData::Image::BufferType* >(params.outputImage->getPixelBuffer(bufferIndex));
...
MinMaxPropagation::MinMaxPropagation(const fwData::Image::csptr& inImage, const fwData::Image::sptr& outImage,
...
fwDataTools::ImageDiff MinMaxPropagation::propagate(SeedsType& seeds, ::fwData::Image::BufferType* value,
                                                    const double radius, const bool overwrite, const Mode mode)
{
    const ::fwTools::Type type     = m_inImage->getType();
    ...
    params.roi         = m_roi;
    params.diff        = ::fwDataTools::ImageDiff(outImagePixelSize);
    params.seeds       = seeds;
    params.value       = value;
    ...
    ::fwTools::Dispatcher< ::fwTools::SupportedDispatcherTypes, MinMaxPropagator >::invoke(type, params);
