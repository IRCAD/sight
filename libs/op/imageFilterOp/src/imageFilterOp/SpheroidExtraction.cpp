...
#include "imageFilterOp/SpheroidExtraction.hpp"
...
struct SpheroidExtractor
{
    struct Parameters
    {
        ...
        ::fwData::PointList::sptr outputPointList;
        double threshold;
        double radiusMin;
        double radiusMax;
    ...
    void operator()(Parameters& params)
    {
        ...
        typename ImageType::Pointer inputImage = ::fwItkIO::itkImageFactory< ImageType >(params.inputImage);
        ...
        for(labelsIt = labels.begin(); labelsIt != labels.end(); labelsIt++)
        {
            ...
            if(mahalanobisDistance < criteriaThreshold)
            {
                ...
                ::fwData::Point::sptr point = ::fwData::Point::New(realPointCoords);
...
fwData::PointList::sptr SpheroidExtraction::extract(const fwData::Image::csptr& _image,
                                                    const double _threshold,
                                                    const double _radiusMin, const double _radiusMax,
                                                    const double _elongationMin, const double _elongationMax)
{
    ::fwData::PointList::sptr outputPointList = ::fwData::PointList::New();
    ...
    ::fwTools::Dispatcher< ::fwTools::SupportedDispatcherTypes, SpheroidExtractor >::invoke(_image->getType(), params);
