...
LineDrawer::LineDrawer(const ::fwData::Image::sptr& img, const ::fwData::Image::csptr& roi) :
    m_image(img),
    m_roiImage(roi)
{
    m_useROI = ::fwDataTools::fieldHelper::MedicalImageHelpers::checkImageValidity( m_roiImage );
...
bool LineDrawer::drawEllipse( const LineDrawer::CoordinatesType& c, ::fwData::Image::BufferType* value,
                              ...
                              ::fwDataTools::ImageDiff& diff)
{
    ...
    for(int y = hbegin; y <= hend; y++)
    {
        for(int x = wbegin; x <= wend; x++)
        {
            ...
            if(dx*dx+dy*dy <= 1)
            {
                point[firstDim]  = static_cast< ::fwData::Image::IndexType >(origX+x);
                point[secondDim] = static_cast< ::fwData::Image::IndexType >(origY+y);

                const ::fwData::Image::IndexType index = point[0] + point[1]*m_yPitch + point[2]*m_zPitch;
...
bool LineDrawer::drawPixel( const ::fwData::Image::IndexType index, ::fwData::Image::BufferType* value,
                            const bool overwrite, ::fwDataTools::ImageDiff& diff)
{
    const ::fwData::Image::BufferType* pixBuf =
        reinterpret_cast< ::fwData::Image::BufferType* >(m_image->getPixelBuffer(index));

    if (m_useROI)
    {
        ::fwData::Image::BufferType* roiVal =
            reinterpret_cast< ::fwData::Image::BufferType* >(m_roiImage->getPixelBuffer(index));
        if (::fwDataTools::fieldHelper::MedicalImageHelpers::isBufNull(roiVal, m_roiTypeSize))
        {
            return false;
        }
    }
    ...
    if (!overwrite && !::fwDataTools::fieldHelper::MedicalImageHelpers::isBufNull(pixBuf, m_imageTypeSize) )
    {
        return false;
    }
...
::fwDataTools::ImageDiff LineDrawer::draw(const OrientationType orientation, const CoordinatesType& startCoord,
                                          const CoordinatesType& endCoord, ::fwData::Image::BufferType* value,
                                          const double thickness, const bool overwrite)
{
    ::fwDataTools::ImageDiff diff(m_imageTypeSize, 128);
