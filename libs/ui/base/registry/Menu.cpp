...
ui::base::container::fwMenuItem::sptr Menu::getFwMenuItem(std::string actionSid,
                                                          std::vector< ui::base::container::fwMenuItem::sptr > menuItems)
{
    SIGHT_ASSERT("The action '" + actionSid + "' declared by the Menu '" + m_sid + "' is not found",
...
void Menu::initialize( core::runtime::ConfigurationElement::sptr configuration)
{
    ...
    for( ConfigurationType menuItem :  vectMenuItems)
    {
        ...
        if(menuItem->hasAttribute("sid"))
        {
            ...
            SIGHT_ASSERT("The action '" + sid + "' already exists for '" + m_sid + "' menu.",
    ...
    for( ConfigurationType menu :  vectMenus)
    {
        ...
        if(menu->hasAttribute("sid"))
        {
            ...
            SIGHT_ASSERT("The menu '" + sid + "' already exists for this menu '" + m_sid + "'",
                         m_actionSids.find( sid) == m_actionSids.end());
...
void Menu::manage(std::vector< ui::base::container::fwMenuItem::sptr > menuItems )
{
    ...
    {
        SIGHT_ASSERT("The menu '" << m_sid << "' contains more menuItems in <registry> than in <layout>: "
        ...
        {
            SIGHT_ASSERT("The action '"+sid.first +"' does not exist, but is declared in '" + m_sid + "' menu, "
...
void Menu::manage(std::vector< ui::base::container::fwMenu::sptr > menus )
{
    ...
    {
        SIGHT_ASSERT("The menu '" << m_sid << "' contains more menus in <registry> than in <layout>: "
        ...
        if(sid.second.second) //service is auto started?
        {
            SIGHT_ASSERT("The menu '"+sid.first +"' does not exist, but is declared in '" + m_sid + "' menu.",
                         core::tools::fwID::exist(sid.first ));
            service::IService::sptr service = service::get( sid.first );
            SIGHT_ASSERT("The service '"+sid.first +"' cannot be started by '" + m_sid + "' because it is not stopped."
                         , service->isStopped() );
...
void Menu::unmanage()
{
    for( SIDMenuMapType::value_type sid :  m_actionSids)
    {
        if(sid.second.second) //service is auto started?
        {
            SIGHT_ASSERT("The menu '" + m_sid + "' try to stop the service '" + sid.first + "' but it does not exist. "
