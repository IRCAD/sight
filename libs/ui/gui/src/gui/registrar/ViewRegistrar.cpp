...
gui::container::fwContainer::sptr ViewRegistrar::getParent()
{
    gui::container::fwContainer::sptr parentContainer;
    if(!m_parentWid.empty())
    {
        parentContainer = gui::GuiRegistry::getWIDContainer(m_parentWid);
    ...
    {
        parentContainer = gui::GuiRegistry::getSIDContainer(m_sid);
...
void ViewRegistrar::manage(std::vector< gui::container::fwContainer::sptr > subViews )
{
...
void ViewRegistrar::manageMenuBar(gui::container::fwMenuBar::sptr menuBar )
{
    gui::GuiRegistry::registerSIDMenuBar(m_menuBarSid.first, menuBar);
    if(m_menuBarSid.second) //service is auto started?
    {
        ...
        services::IService::sptr service = services::get( m_menuBarSid.first );
        service->start();
    }
}
...
void ViewRegistrar::manageToolBar(gui::container::fwToolBar::sptr toolBar )
{
    gui::GuiRegistry::registerSIDToolBar(m_toolBarSid.first, toolBar);
    if(m_toolBarSid.second) //service is auto started?
    {
        ...
        services::IService::sptr service = services::get( m_toolBarSid.first );
        service->start();
    }
}
...
void ViewRegistrar::unmanageToolBar()
{
    if ( !m_toolBarSid.first.empty() )
    {
        ...
        gui::GuiRegistry::unregisterSIDToolBar(m_toolBarSid.first);
    }
}
...
void ViewRegistrar::unmanageMenuBar()
{
    if ( !m_menuBarSid.first.empty() )
    {
        ...
        gui::GuiRegistry::unregisterSIDMenuBar(m_menuBarSid.first);
    }
}
