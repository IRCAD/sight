...
gui::container::fwToolBar::sptr ToolBarRegistrar::getParent()
{
    return gui::GuiRegistry::getSIDToolBar(m_sid);
}

//-----------------------------------------------------------------------------

gui::container::fwMenuItem::sptr ToolBarRegistrar::getFwMenuItem(std::string actionSid,
                                                                 std::vector< gui::container::fwMenuItem::sptr > menuItems)
{
    SLM_ASSERT("The action '" + actionSid + "' declared by the toolBar '" + m_sid + "' is not found",
               m_actionSids.find(actionSid) != m_actionSids.end());
    gui::container::fwMenuItem::sptr menuItem = menuItems.at( m_actionSids[actionSid].first );
    return menuItem;
}
...
void ToolBarRegistrar::initialize( core::runtime::ConfigurationElement::sptr configuration)
{
    ...
    for( ConfigurationType menuItem :  vectMenuItems)
    {
        ...
        if(menuItem->hasAttribute("sid"))
        {
            ...
            gui::ActionCallbackBase::sptr callback;
            gui::GuiBaseObject::sptr guiObj = gui::factory::New(ActionCallbackBase::REGISTRY_KEY);
            callback = gui::ActionCallbackBase::dynamicCast(guiObj);
            SLM_ASSERT("Cannot create action callback, factory failed for '"
                       + gui::ActionCallbackBase::REGISTRY_KEY + "'", callback);
            callback->setSID(sid);
...
void ToolBarRegistrar::manage(std::vector< gui::container::fwMenuItem::sptr > menuItems )
{
    gui::container::fwMenuItem::sptr menuItem;
    for( SIDToolBarMapType::value_type sid :  m_actionSids)
    {
        ...
        gui::GuiRegistry::registerActionSIDToParentSID(sid.first, m_sid);
        ...
        else
        {
            ...
            {
                gui::GuiRegistry::actionServiceStopping(sid.first);
            }
            else
            {
                gui::GuiRegistry::actionServiceStarting(sid.first);
            }
        }
    }
}
...
void ToolBarRegistrar::manage(std::vector< gui::container::fwMenu::sptr > menus )
{
    gui::container::fwMenu::sptr menu;
    for( SIDToolBarMapType::value_type sid :  m_menuSids)
    {
        ...
        gui::GuiRegistry::registerSIDMenu(sid.first, menu);
        if(sid.second.second) //service is auto started?
        {
...
void ToolBarRegistrar::manage(std::vector< gui::container::fwContainer::sptr > containers )
{
    gui::container::fwContainer::sptr container;
    for( SIDToolBarMapType::value_type sid :  m_editorSids)
    {
        ...
        gui::GuiRegistry::registerSIDContainer(sid.first, container);
        if(sid.second.second) //service is auto started?
        {
    ...
    for( WIDToolBarMapType::value_type wid :  m_editorWids)
    {
        ...
        gui::GuiRegistry::registerWIDContainer(wid.first, container);
    }
}
...
void ToolBarRegistrar::unmanage()
{
    for( SIDToolBarMapType::value_type sid :  m_actionSids)
    {
        ...
        gui::GuiRegistry::unregisterActionSIDToParentSID(sid.first, m_sid);
    }
    for( SIDToolBarMapType::value_type sid :  m_menuSids)
    {
        if(sid.second.second) //service is auto started?
        ...
        gui::GuiRegistry::unregisterSIDMenu(sid.first);
    }
    for( SIDToolBarMapType::value_type sid :  m_editorSids)
    {
        if(sid.second.second) //service is auto started?
        ...
        gui::GuiRegistry::unregisterSIDContainer(sid.first);
    }
    for( WIDToolBarMapType::value_type wid :  m_editorWids)
    {
        gui::GuiRegistry::unregisterWIDContainer(wid.first);
    }
}
