...
gui::container::fwMenuBar::sptr MenuBarRegistrar::getParent()
{
    return gui::GuiRegistry::getSIDMenuBar(m_sid);
}

//-----------------------------------------------------------------------------

gui::container::fwMenu::sptr MenuBarRegistrar::getFwMenu(std::string menuSid,
                                                         std::vector< gui::container::fwMenu::sptr > menus)
{
    SLM_ASSERT("The menu '" + menuSid + "'declared in '"+ m_sid +"' is not found",
               m_menuSids.find(menuSid) != m_menuSids.end());
    gui::container::fwMenu::sptr menu = menus.at( m_menuSids[menuSid].first );
    return menu;
}
...
void MenuBarRegistrar::manage(std::vector< gui::container::fwMenu::sptr > menus )
{
