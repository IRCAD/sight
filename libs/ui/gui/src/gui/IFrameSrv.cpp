...
gui::container::fwContainer::wptr IFrameSrv::m_progressWidget =
    std::weak_ptr< gui::container::fwContainer >();
...
void IFrameSrv::initialize()
{
    ...
    m_viewRegistrar = gui::registrar::ViewRegistrar::New(this->getID());
...
void IFrameSrv::create()
{
    ...
    gui::container::fwContainer::sptr frame = m_frameLayoutManager->getFrame();
    if ( m_progressWidget.expired() )
    {
        m_progressWidget = frame;
    }

    gui::container::fwContainer::sptr container = m_frameLayoutManager->getContainer();
    std::vector< gui::container::fwContainer::sptr > subViews;
    subViews.push_back(container);
    m_viewRegistrar->manage(subViews);

    gui::layoutManager::IFrameLayoutManager::CloseCallback fct;

    if (m_closePolicy == CLOSE_POLICY_EXIT)
    {
        fct = std::bind( &gui::IFrameSrv::onCloseExit, this);
    }
    else if (m_closePolicy == CLOSE_POLICY_NOTIFY)
    {
        fct = std::bind( &gui::IFrameSrv::onCloseNotify, this);
    }
    else if(m_closePolicy == CLOSE_POLICY_MESSAGE)
    {
        fct = std::bind( &gui::IFrameSrv::onCloseMessage, this);
        auto app = gui::Application::New();
        app->setConfirm(true);
    }
...
void IFrameSrv::initializeLayoutManager(ConfigurationType frameConfig)
{
    ...
    gui::GuiBaseObject::sptr guiObj = gui::factory::New(
        gui::layoutManager::IFrameLayoutManager::REGISTRY_KEY);
    m_frameLayoutManager = gui::layoutManager::IFrameLayoutManager::dynamicCast(guiObj);
    SLM_ASSERT("ClassFactoryRegistry failed for class "<< gui::layoutManager::IFrameLayoutManager::REGISTRY_KEY,
...
void IFrameSrv::initializeMenuBarBuilder(ConfigurationType menuBarConfig)
{
    ...
    gui::GuiBaseObject::sptr guiObj = gui::factory::New(gui::builder::IMenuBarBuilder::REGISTRY_KEY);
    m_menuBarBuilder = gui::builder::IMenuBarBuilder::dynamicCast(guiObj);
    SLM_ASSERT("ClassFactoryRegistry failed for class "<< gui::builder::IMenuBarBuilder::REGISTRY_KEY,
               m_menuBarBuilder);
...
void IFrameSrv::initializeToolBarBuilder(ConfigurationType toolBarConfig)
{
    ...
    gui::GuiBaseObject::sptr guiObj = gui::factory::New(gui::builder::IToolBarBuilder::REGISTRY_KEY);
    m_toolBarBuilder = gui::builder::IToolBarBuilder::dynamicCast(guiObj);
    SLM_ASSERT("ClassFactoryRegistry failed for class "<< gui::builder::IToolBarBuilder::REGISTRY_KEY,
               m_toolBarBuilder);
...
gui::container::fwContainer::sptr IFrameSrv::getProgressWidget()
{
...
{
    gui::container::fwContainer::sptr container = m_frameLayoutManager->getFrame();
