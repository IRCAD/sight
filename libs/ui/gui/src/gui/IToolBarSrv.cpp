...
{
    m_registrar = gui::registrar::ToolBarRegistrar::New(this->getID());
...
{
    gui::container::fwToolBar::sptr toolBar               = m_registrar->getParent();
    std::vector< gui::IMenuItemCallback::sptr > callbacks = m_registrar->getCallbacks();
...
{
    gui::container::fwMenuItem::sptr menuItem = m_registrar->getFwMenuItem(actionSrvSID,
...
void IToolBarSrv::actionServiceStarting(std::string actionSrvSID)
{
    ...
    const gui::IActionSrv::csptr actionSrv  = gui::IActionSrv::dynamicCast(service);
...
void IToolBarSrv::initializeLayoutManager(ConfigurationType layoutConfig)
{
    ...
    gui::GuiBaseObject::sptr guiObj = gui::factory::New(
        gui::layoutManager::IToolBarLayoutManager::REGISTRY_KEY);
    m_layoutManager = gui::layoutManager::IToolBarLayoutManager::dynamicCast(guiObj);
    SLM_ASSERT("ClassFactoryRegistry failed for class "<< gui::layoutManager::IToolBarLayoutManager::REGISTRY_KEY,
               m_layoutManager);
