...
{
    m_registrar = gui::registrar::MenuRegistrar::New(this->getID());
...
{
    gui::container::fwMenu::sptr menu                     = m_registrar->getParent();
...
void IMenuSrv::initializeLayoutManager(ConfigurationType layoutConfig)
{
    ...
    gui::GuiBaseObject::sptr guiObj = gui::factory::New(
        gui::layoutManager::IMenuLayoutManager::REGISTRY_KEY);
    m_layoutManager = gui::layoutManager::IMenuLayoutManager::dynamicCast(guiObj);
    SLM_ASSERT("ClassFactoryRegistry failed for class "<< gui::layoutManager::IMenuLayoutManager::REGISTRY_KEY,
               m_layoutManager);
