...
{
    gui::dialog::InputDialog inputBox(title, message, text);
...
InputDialog::InputDialog()
{
    services::registry::ActiveWorkers::getDefaultWorker()->postTask<void>(std::function< void() >([&]
            {
                gui::GuiBaseObject::sptr guiObj = gui::factory::New(IInputDialog::REGISTRY_KEY);
                m_implementation = gui::dialog::IInputDialog::dynamicCast(guiObj);
            }));
}
