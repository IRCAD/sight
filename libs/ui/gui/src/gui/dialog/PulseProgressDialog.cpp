...
PulseProgressDialog::PulseProgressDialog(
    ...
    gui::dialog::IPulseProgressDialog::Stuff stuff,
    const std::string& msg,
    gui::dialog::IPulseProgressDialog::MilliSecond frequenceRefresh )
{

    services::registry::ActiveWorkers::getDefaultWorker()->postTask<void>(std::function<void()>(
                                                                              [&]
            {
                gui::GuiBaseObject::sptr guiObj = gui::factory::New(IPulseProgressDialog::REGISTRY_KEY);
                m_implementation = gui::dialog::IPulseProgressDialog::dynamicCast(guiObj);
                if (m_implementation)
                {
