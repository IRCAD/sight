...
#include "gui/dialog/MultiSelectorDialog.hpp"
...
MultiSelectorDialog::MultiSelectorDialog()
{
    services::registry::ActiveWorkers::getDefaultWorker()->postTask< void >(std::function< void() >(
                                                                                [this]
            {
                gui::GuiBaseObject::sptr guiObj = gui::factory::New(IMultiSelectorDialog::REGISTRY_KEY);
                m_implementation = gui::dialog::IMultiSelectorDialog::dynamicCast(guiObj);
            })).wait();
}
