...
{
    gui::dialog::SelectorDialog selector(title, message, _selections);
...
void SelectorDialog::create()
{
    services::registry::ActiveWorkers::getDefaultWorker()->postTask<void>(std::function< void() >([&]
            {
                gui::GuiBaseObject::sptr guiObj = gui::factory::New(ISelectorDialog::REGISTRY_KEY);
                m_implementation = gui::dialog::ISelectorDialog::dynamicCast(guiObj);
            })).wait();
}
