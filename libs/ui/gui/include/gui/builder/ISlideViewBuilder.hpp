 ...
 */
class GUI_CLASS_API ISlideViewBuilder : public gui::GuiBaseObject
{
public:

    fwCoreClassMacro(ISlideViewBuilder, gui::GuiBaseObject)

    GUI_API const static std::string REGISTRY_KEY;
    ...
    GUI_API ISlideViewBuilder();

    /// Destroyes the builder.
    GUI_API virtual ~ISlideViewBuilder();

    /// Returns the generic container.
    GUI_API virtual gui::container::fwContainer::sptr getContainer() const;

    /// Parses the configuration.
    GUI_API virtual void initialize(core::runtime::ConfigurationElement::sptr _config);

    /// Creates the container.
    GUI_API virtual void createContainer(gui::container::fwContainer::sptr _parent) = 0;
