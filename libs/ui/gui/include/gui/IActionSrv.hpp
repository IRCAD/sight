 ...
 */
class GUI_CLASS_API IActionSrv : public ::sight::services::IService
{
...
public:
    ...
    GUI_API virtual void activate();

    /// Set the action service is not active.
    GUI_API virtual void deactivate();

    /// Return true if action service is active.
    GUI_API bool getIsActive() const;

    /// Set the action service executable or not.
    GUI_API virtual void setIsExecutable(bool isExecutable);

    /// Set the action service executable.
    GUI_API virtual void setExecutable();

    /// Set the action service not executable.
    GUI_API virtual void setInexecutable();

    /// Return action service is executable.
    GUI_API bool getIsExecutable() const;
     ...
     */
    GUI_API bool confirmAction();

protected:

    GUI_API IActionSrv();

    GUI_API virtual ~IActionSrv();
     ...
     * @code{.xml}
           <service uid="item" type="::modules::gui::action::SDefault" autoConnect="no" >
...
private:
    ...
    gui::registrar::ActionRegistrar::sptr m_registrar;
