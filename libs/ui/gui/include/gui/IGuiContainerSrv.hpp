 ...
 * @code{.xml}
   ...
   @endcode
 *  - \<layout type="::gui::LineLayoutManager" \> : give the type of layout.
 *    - \b type {::gui::LineLayoutManager | gui::CardinalLayoutManager | gui::TabLayoutManager |
 *              gui::ToolboxLayoutManager} :
 *     - \b gui::LineLayoutManager : all views will be on the same line or column (it depends of the orientation
 *           value attribute)
 *           @see gui::layoutManager::LineLayoutManagerBase
 *     - \b gui::CardinalLayoutManager : all views will be added around a central view define by the align
 * attribute.
 *           @see gui::layoutManager::CardinalLayoutManagerBase
 *     - \b gui::TabLayoutManager : all views will be draw as tab.
 *           @see gui::layoutManager::TabLayoutManagerBase
 *     - \b gui::ToolboxLayoutManager : all views will be draw in toolbox.
 *           @see gui::layoutManager::ToolboxLayoutManagerBase
 *  - \b toolBar: defines the toolBar configuration.
 *           @see gui::builder::IToolBarBuilder
 *  - \b slideView: defines a slide view.
 *           @see gui::builder::ISlideViewBuilder
 *
 ...
 *
 */
class GUI_CLASS_API IGuiContainerSrv : public services::IService
{
...
public:
    ...
    GUI_API SPTR(gui::container::fwContainer) getContainer();

    GUI_API void setParent(std::string wid);

protected:

    GUI_API IGuiContainerSrv();

    GUI_API virtual ~IGuiContainerSrv();
     ...
     *
       @see gui::registrar::ViewRegistrar::initialize(), gui::layoutManager::IViewLayoutManager::initialize(),
     *::gui::builder::IToolBarBuilder::initialize()
     */
    ...
    GUI_API static const core::com::Slots::SlotKeyType s_SET_ENABLED_SLOT;

    /// Slot to enable the container
    GUI_API static const core::com::Slots::SlotKeyType s_ENABLE_SLOT;

    /// Slot to disable the container
    GUI_API static const core::com::Slots::SlotKeyType s_DISABLE_SLOT;

    /// Slot to show/hide the container
    GUI_API static const core::com::Slots::SlotKeyType s_SET_VISIBLE_SLOT;

    /// Slot to show the container
    GUI_API static const core::com::Slots::SlotKeyType s_SHOW_SLOT;

    /// Slot to hide the container
    GUI_API static const core::com::Slots::SlotKeyType s_HIDE_SLOT;

    /// SLOT: enable/disable the container
    GUI_API void setEnabled(bool isEnabled);
    /// SLOT: enable the container
    GUI_API void enable();
    /// SLOT: disable the container
    GUI_API void disable();
    /// SLOT: show/hide the container
    GUI_API void setVisible(bool isVisible);
...
private:
    ...
    typedef std::vector< SPTR(gui::builder::ISlideViewBuilder) > SlideViewContainerType;
    ...
    SPTR( gui::layoutManager::IViewLayoutManager) m_viewLayoutManager;

    SPTR( gui::registrar::ViewRegistrar) m_viewRegistrar;
    SPTR( gui::builder::IToolBarBuilder) m_toolBarBuilder;
    SPTR( gui::builder::IContainerBuilder) m_containerBuilder;
