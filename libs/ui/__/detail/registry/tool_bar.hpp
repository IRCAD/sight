 ...
 */
class tool_bar : public ui::object
{
public:

    SIGHT_DECLARE_CLASS(tool_bar, ui::object);

    //------------------------------------------------------------------------------

    static sptr make(const std::string& _sid)
    {
        return std::make_shared<tool_bar>(_sid);
    }

    typedef std::vector<ui::menu_item_callback::sptr> callbacks_t;

    /// Constructor.
    tool_bar(std::string _sid);

    /// Destructor. Do nothing
    ~tool_bar() override = default;
...
protected:
    ...
    typedef std::map<std::string, std::pair<unsigned int, bool> > SIDtool_barMapType;
    typedef std::map<std::string, unsigned int> WIDtool_barMapType;
