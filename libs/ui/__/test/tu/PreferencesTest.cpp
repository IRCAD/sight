...
CPPUNIT_TEST_SUITE_REGISTRATION(sight::ui::ut::PreferencesTest);
...
void PreferencesTest::setUp()
{
    ...
    const std::string& profileName = core::tools::UUID::generate();
    core::runtime::get_current_profile()->set_name(profileName);

    // Compute the expected preferences file path
    m_preferencesPath = core::tools::os::get_user_config_dir(profileName) / "preferences.json";
    m_encryptedPath   = core::tools::os::get_user_config_dir(profileName) / "preferences.sight";
}
