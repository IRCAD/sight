 ...
 */
class GUIQT_CLASS_API LoggerDialog : public QDialog,
...
public:
    ...
    /// Constructor
    GUIQT_API LoggerDialog(gui::GuiBaseObject::Key key);

    /// Destructor
    GUIQT_API virtual ~LoggerDialog();
     ...
     */
    GUIQT_API virtual void setMessage(const std::string& message) override;

    /**
     * @brief Set the dialog logger.
     * @param[in] logger Dialog logger
     */
    GUIQT_API virtual void setLogger(const core::log::Logger::sptr& logger) override;

    /**
     * @brief Show the dialog and return whether the user has selected the Ok or Cancel button
     */
    GUIQT_API virtual bool show() override;
