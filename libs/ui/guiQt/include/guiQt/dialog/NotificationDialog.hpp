...
class GUIQT_CLASS_API Container : public QWidget
{
...
class GUIQT_CLASS_API ClickableQLabel : public QLabel
{
 ...
 *
 */
class GUIQT_CLASS_API NotificationDialog : public gui::dialog::INotificationDialog
{

public:

    fwCoreClassMacro(NotificationDialog, gui::dialog::INotificationDialog,
                     gui::factory::New< NotificationDialog > )

    /// Initializes members.
    GUIQT_API NotificationDialog(gui::GuiBaseObject::Key key);

    /// Destroys the dialog.
    GUIQT_API virtual ~NotificationDialog() override;

    /// Shows the notification relative to the active window.
    GUIQT_API virtual void show() override;

    /**
     * @brief Returns whether the popup is displayed or not.
     * @return boolean (true is visible, false otherwise).
     */
    GUIQT_API virtual bool isVisible() const override;

    /// Closes the popup (use a fadeout effect).
    GUIQT_API virtual void close() const override;
