...
#include "guiQt/dialog/LocationDialog.hpp"
...
fwGuiRegisterMacro( ::sight::guiQt::dialog::LocationDialog, ::sight::gui::dialog::ILocationDialog::REGISTRY_KEY );
...
LocationDialog::LocationDialog(gui::GuiBaseObject::Key key) :
    m_style(gui::dialog::ILocationDialog::NONE),
    m_type(gui::dialog::ILocationDialog::SINGLE_FILE)
{
}
...
data::location::ILocation::sptr LocationDialog::show()
{
    ...
    else if (m_type == gui::dialog::ILocationDialog::SINGLE_FILE)
    {
        ...
        else if ( m_style & gui::dialog::ILocationDialog::WRITE )
        {
            if (dialog.exec() && !dialog.selectedFiles().empty())
            {
                fileName   = dialog.selectedFiles()[0];
...
{
    if ( option == gui::dialog::ILocationDialog::WRITE )
    {
        m_style = (gui::dialog::ILocationDialog::Options) (m_style & ~gui::dialog::ILocationDialog::READ);
        m_style = (gui::dialog::ILocationDialog::Options) (m_style | gui::dialog::ILocationDialog::WRITE);
    }
    else if ( option == gui::dialog::ILocationDialog::READ )
    {
        m_style = (gui::dialog::ILocationDialog::Options) (m_style & ~gui::dialog::ILocationDialog::WRITE);
        m_style = (gui::dialog::ILocationDialog::Options) (m_style | gui::dialog::ILocationDialog::READ);
    }
    else if ( option == gui::dialog::ILocationDialog::FILE_MUST_EXIST )
    {
        m_style =
            (gui::dialog::ILocationDialog::Options) (m_style |
                                                     gui::dialog::ILocationDialog::FILE_MUST_EXIST);
    }
