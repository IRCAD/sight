...
namespace sight::io::base::services
{
...
{
    FW_RAISE_IF("This reader doesn't manage files", !(this->getIOPathType() & io::base::services::FILE));
...
const io::base::services::LocationsType& IReader::getFiles() const
{
    FW_RAISE_IF("This reader doesn't manage files", !(this->getIOPathType() & io::base::services::FILES));
...
void IReader::setFiles(const io::base::services::LocationsType& files)
{
    FW_RAISE_IF("This reader doesn't manage files", !(this->getIOPathType() & io::base::services::FILES));
    m_locations = files;
}
...
{
    FW_RAISE_IF("This reader doesn't manage folders", !(this->getIOPathType() & io::base::services::FOLDER));
...
void IReader::setFileFolder(std::filesystem::path folder)
{
    FW_RAISE_IF("This reader doesn't manage file or files",
                !(this->getIOPathType() & io::base::services::FILE) &&
                !(this->getIOPathType() & io::base::services::FILES));
...
const io::base::services::LocationsType& IReader::getLocations() const
{
...
void IReader::configuring()
{
    ...
    SLM_ASSERT("Generic configuring method is only available for io service that use paths.",
               !( this->getIOPathType() & io::base::services::TYPE_NOT_DEFINED ) );

    SLM_ASSERT("This reader does not manage folders and a folder path is given in the configuration",
               ( this->getIOPathType() & io::base::services::FOLDER ) || (config.count("folder") == 0) );

    SLM_ASSERT("This reader does not manage files and a file path is given in the configuration",
               ( this->getIOPathType() & io::base::services::FILE || this->getIOPathType() & io::base::services::FILES ) ||
               (config.count("file") == 0));

    m_windowTitle = config.get("windowTitle", "");

    if ( this->getIOPathType() & io::base::services::FILE )
    {
        FW_RAISE_IF("This reader cannot manages FILE and FILES.", this->getIOPathType() & io::base::services::FILES );
    ...
    if ( this->getIOPathType() & io::base::services::FILES )
    {
        FW_RAISE_IF("This reader cannot manage FILE and FILES.", this->getIOPathType() & io::base::services::FILE );

        io::base::services::LocationsType locations;
    ...
    if ( this->getIOPathType() & io::base::services::FOLDER )
    {
...
io::base::services::IOPathType IReader::getIOPathType() const
{
    return io::base::services::TYPE_NOT_DEFINED;
}
...
void IReader::readFiles(io::base::services::LocationsType files)
{
    this->setFiles(files);
    this->updating();
}
