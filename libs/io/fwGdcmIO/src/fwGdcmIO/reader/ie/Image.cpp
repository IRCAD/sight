...
void Image::readImagePlaneModule()
{
    ...
    SLM_TRACE("Image's dimension : " << dimension);
...
void Image::readVOILUTModule()
{
    ...
    SLM_TRACE("Image's window center : " << m_object->getWindowCenter());
    ...
    SLM_TRACE("Image's window width : " << m_object->getWindowWidth());
...
void Image::readImagePixelModule()
{
    ...
    SLM_TRACE("Image dimensions : [" << dimensions[0] << "," <<dimensions[1] << "," << dimensions[2] << "]");
    ...
    SLM_TRACE("Image buffer size : " << imageBufferSize);
    SLM_TRACE("New image buffer size : " << newImageBufferSize);
