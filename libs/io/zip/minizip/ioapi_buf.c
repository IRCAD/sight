...
voidpf fopen_buf_internal_func(ZIP_UNUSED voidpf opaque, voidpf stream, ZIP_UNUSED uint32_t number_disk,
                               ZIP_UNUSED int mode)
{
...
voidpf ZCALLBACK fopendisk64_buf_func(voidpf opaque, voidpf stream_cd, uint32_t number_disk, int mode)
{
    ourbuffer_t* bufio    = (ourbuffer_t*)opaque;
    ourstream_t* streamio = (ourstream_t*)stream_cd;
    voidpf stream         = bufio->filefunc64.zopendisk64_file(bufio->filefunc64.opaque, streamio->stream, number_disk,
                                                               mode);
    return fopen_buf_internal_func(opaque, stream, number_disk, mode);
}
...
long fflush_buf(voidpf opaque, voidpf stream)
{
    ...
    while (bytes_left_to_write > 0)
    {
        ...
        {
            bytes_written = bufio->filefunc64.zwrite_file(bufio->filefunc64.opaque, streamio->stream,
                                                          streamio->writebuf + (bytes_to_write - bytes_left_to_write),
                                                          bytes_left_to_write);
        }
        else
        {
            bytes_written =
                bufio->filefunc.zwrite_file(bufio->filefunc.opaque, streamio->stream,
                                            streamio->writebuf + (bytes_to_write - bytes_left_to_write),
                                            bytes_left_to_write);
        }
...
uint32_t ZCALLBACK fread_buf_func(voidpf opaque, voidpf stream, void* buf, uint32_t size)
{
    ...
    while (bytes_left_to_read > 0)
    {
        if ((streamio->readbuf_len == 0) || (streamio->readbuf_pos == streamio->readbuf_len))
        {
            ...
            {
                bytes_read = bufio->filefunc64.zread_file(bufio->filefunc64.opaque, streamio->stream,
                                                          streamio->readbuf + streamio->readbuf_pos, bytes_to_read);
            }
            else
            {
                bytes_read = bufio->filefunc.zread_file(bufio->filefunc.opaque, streamio->stream,
                                                        streamio->readbuf + streamio->readbuf_pos, bytes_to_read);
            }
            ...
            print_buf(opaque, stream, "filled [read %d/%d buf %d:%d pos %lld]\n", bytes_read, bytes_to_read,
                      streamio->readbuf_pos, streamio->readbuf_len, streamio->position);
        ...
        if ((streamio->readbuf_len - streamio->readbuf_pos) > 0)
        {
            ...
            print_buf(opaque, stream, "emptied [copied %d remaining %d buf %d:%d pos %lld]\n", bytes_to_copy,
                      bytes_left_to_read, streamio->readbuf_pos, streamio->readbuf_len, streamio->position);
        }
    }
...
uint32_t ZCALLBACK fwrite_buf_func(voidpf opaque, voidpf stream, const void* buf, uint32_t size)
{
    ...
    if (streamio->readbuf_len > 0)
    {
        ...
        {
            ret = bufio->filefunc64.zseek64_file(bufio->filefunc64.opaque, streamio->stream, streamio->position,
                                                 ZLIB_FILEFUNC_SEEK_SET);
        }
        else
        {
            ret = bufio->filefunc.zseek_file(bufio->filefunc.opaque, streamio->stream, (uint32_t)streamio->position,
                                             ZLIB_FILEFUNC_SEEK_SET);
        }
    ...
    while (bytes_left_to_write > 0)
    {
        ...
        memcpy(streamio->writebuf + streamio->writebuf_pos, (char*)buf + (bytes_to_write - bytes_left_to_write),
               bytes_to_copy);

        print_buf(opaque, stream, "write copy [remaining %d write %d:%d len %d]\n", bytes_to_copy, bytes_to_write,
                  bytes_left_to_write, streamio->writebuf_len);
...
uint64_t ftell_buf_internal_func(ZIP_UNUSED voidpf opaque, voidpf stream, uint64_t position)
{
    ...
    print_buf(opaque, stream, "tell [pos %llu readpos %d writepos %d err %d]\n", streamio->position,
              streamio->readbuf_pos, streamio->writebuf_pos, errno);
    if (streamio->readbuf_len > 0)
    {
...
int fseek_buf_internal_func(voidpf opaque, voidpf stream, uint64_t offset, int origin)
{
    ...
    switch (origin)
    {
        case ZLIB_FILEFUNC_SEEK_SET:
            ...
            if ((streamio->readbuf_len > 0) && (offset < streamio->position) &&
                (offset >= streamio->position - streamio->readbuf_len))
            {
